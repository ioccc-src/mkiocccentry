...
files might seem strange or out of place, especially as they are obfuscated.
Also they should not be modified by anyone but those who have access to a
private tool as doing so will break something. These files are for a fun thing
for people to discover but as the rest of the repo is not obfuscated (except for
the parts of the JSON parser that are generated by tools :-) ) it seems like
these files should be briefly explained.
