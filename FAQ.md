...
13. <a href="foo">What are the foo.h, foo.c and oebxergfB.h files under soup/?</a>
...
At the risk of stating the obvious, please **DO NOT** modify the files
_soup/foo.h_ or _soup/foo.c_ as doing so is very likely to break something,
especially in code but even not in code. This is because the code (or part of
it) corresponds to a private tool that we asked Cody Boone Ferguson (@xexyl) to
write. As it happens at this time the only place the word might be found is in
comments but nevertheless this should **NOT** be changed as doing so would ruin
something fun. :-) See below for more files.


## 13. <a name="foo">What are the foo.h, foo.c and oebxergfB.h files under soup/?</a>

Whereas the rest of the repo is not obfuscated (except perhaps for the parts of
the JSON parser that are generated by tools :-) ), these files are. This is very
intended to obscure something fun for people to discover.

Please **DO NOT** even try and modify these files as doing so will break
something even if it appears not to. Even comments are meant to be the way they
are.

The code and the data correspond to code and data in a private tool written by
Cody Boone Ferguson (@xexyl) at our request and as for _oebxergfB.h_ it is
generated by that tool from other data that is also private.

Instead of trying to understand it you might enjoy trying to figure out how it's
used. :-)
