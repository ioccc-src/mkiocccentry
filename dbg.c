...
"usage: %s [-h] [-v level] work_dir iocccsize_path [tar_path]\n"
"\n"
...
err(int exitcode, char const *name, char const *fmt, ...)
{
    ...
    ret = fprintf(stderr, "FATAL: %s: ", name);
    if (ret < 0) {
...
main(int argc, char *argv[])
{
     ...
     * simulate an error
     */
    err(4, __FUNCTION__, "simulated error, work_dir: %s iocccsize_path: %s", work_dir, iocccsize_path);
    /* exit(4); */
    /*NOTREACHED*/
