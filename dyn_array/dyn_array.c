...
dyn_array_grow(struct dyn_array *array, intmax_t elms_to_allocate)
{
    ...
    if (array->elm_size <= 0) {
	err(53, __func__, "array->elm_size in dynamic array must be > 0: %zu", array->elm_size);
	not_reached();
    }
    if (array->chunk <= 0) {
	...
	dbg(DBG_V5_HIGH, "in %s(array, %jd): moved-place: allocated: %jd elements of size: %zu in use: %jd",
			  __func__, elms_to_allocate,
			  dyn_array_alloced(array),
			  array->elm_size,
			  dyn_array_tell(array));
    } else if (dbg_allowed(DBG_V5_HIGH)) {
	dbg(DBG_V5_HIGH, "in %s(array, %jd): in-place: allocated: %jd elements of size: %zu in use: %jd",
			   __func__, elms_to_allocate,
			   dyn_array_alloced(array),
			   array->elm_size,
			   dyn_array_tell(array));
...
determine_move_case(void *first_alloc, void *last_alloc, void *first_add, void *last_add)
{
	...
	dbg(DBG_V7_HIGH, "in %s: first_alloc: %p last_alloc: %p first_add: %p last_add: %p",
	 ...
	 */
	if (dbg_allowed(DBG_V7_HIGH)) {
	    dbg(DBG_V7_HIGH, "in %s: first_add: %p < first_alloc: %p",
	...
	case PTR_BELOW_ADDR:	/* all of data is before allocated */
	    ...
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: ret #0: %s: last_add: %p < first_alloc: %p",
	...
	case PTR_EQ:		/* only last byte of data is allocated */
	    ...
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: ret #1: %s: last_add: %p == first_alloc: %p",
			       __func__, move_case_name(ret), last_add, first_alloc);
	    }
	    break;
	...
	case PTR_ABOVE_ADDR:	/* last byte to add is beyond first allocated */
	     ...
	     */
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: last_add: %p > first_alloc: %p",
		...
		if (dbg_allowed(DBG_V7_HIGH)) {
		    dbg(DBG_V7_HIGH, "in %s: last_add: %p < last_alloc: %p",
			  __func__, last_add, last_alloc);
		}
		ret = MOVE_CASE_BEFORE_INTO;
		if (dbg_allowed(DBG_V7_HIGH)) {
		    dbg(DBG_V7_HIGH, "in %s: ret #2: %s: last_add: %p < last_alloc: %p",
		...
		ret = MOVE_CASE_BEFORE_INTO;
		if (dbg_allowed(DBG_V7_HIGH)) {
		    dbg(DBG_V7_HIGH, "in %s: ret #3: %s: last_add: %p == last_alloc: %p",
			  __func__, move_case_name(ret), last_add, last_alloc);
		}
		break;
		...
		if (dbg_allowed(DBG_V7_HIGH)) {
		    dbg(DBG_V7_HIGH, "in %s: ret #4: %s: last_add: %p > last_alloc: %p",
			  __func__, move_case_name(ret), last_add, last_alloc);
		}
		break;
	...
	case PTR_BELOW_ADDR:	/* data ends before end of allocated */
	    ...
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: ret #5: %s: last_add: %p < last_alloc: %p",
			      __func__, move_case_name(ret), last_add, last_alloc);
	    }
	    break;
	...
	case PTR_EQ:		/* data ends at end of allocated */
	    ...
	    ret = MOVE_CASE_INSIDE;
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: ret #6: %s: last_add: %p == last_alloc: %p",
			       __func__, move_case_name(ret), last_add, last_alloc);
	    }
	    break;
	...
	case PTR_ABOVE_ADDR:	/* data ends beyond end of allocated */
	    ...
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: ret #7: %s: last_add: %p > last_alloc: %p",
			      __func__, move_case_name(ret), last_add, last_alloc);
	    }
	    break;
	...
	case PTR_BELOW_ADDR:	/* data starts before end of allocated */
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: first_add: %p < last_alloc: %p",
		...
		ret = MOVE_CASE_INSIDE;
		if (dbg_allowed(DBG_V7_HIGH)) {
		    dbg(DBG_V7_HIGH, "in %s: ret #8: %s: last_add: %p < last_alloc: %p",
			   __func__, move_case_name(ret), last_add, last_alloc);
		}
		break;
		...
		ret = MOVE_CASE_INSIDE;
		if (dbg_allowed(DBG_V7_HIGH)) {
		    dbg(DBG_V7_HIGH, "in %s: ret #9: %s: last_add: %p == last_alloc: %p",
			   __func__, move_case_name(ret), last_add, last_alloc);
		}
		break;
		...
		ret = MOVE_CASE_IN_BEYOND;
		if (dbg_allowed(DBG_V7_HIGH)) {
		    dbg(DBG_V7_HIGH, "in %s: ret #10: %s: last_add: %p > last_alloc: %p",
			   __func__, move_case_name(ret), last_add, last_alloc);
		}
		break;
	...
	case PTR_EQ:		/* data start matches end of allocated */
	    ret = MOVE_CASE_INSIDE;
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: ret #11: %s: first_add: %p == last_alloc: %p",
	...
	case PTR_ABOVE_ADDR:	/* data start beyond end of allocated */
	    ...
	    ret = MOVE_CASE_OUTSIDE;
	    if (dbg_allowed(DBG_V7_HIGH)) {
		dbg(DBG_V7_HIGH, "in %s: ret #12: %s: first_add: %p > last_alloc: %p",
			      __func__, move_case_name(ret), first_add, last_alloc);
	    }
	    break;
...
dyn_array_create(size_t elm_size, intmax_t chunk, intmax_t start_elm_count, bool zeroize)
{
	...
	dbg(DBG_V5_HIGH, "in %s(%zu, %jd, %jd, %s): initialized empty dynamic array, allocated: %jd elements of size: %zu",
		      __func__, elm_size, chunk, start_elm_count,
		      booltostr(zeroize),
...
dyn_array_append_set(struct dyn_array *array, void *array_to_add_p, intmax_t count_of_elements_to_add)
{
	...
	if (dbg_allowed(DBG_V6_HIGH)) {
	    dbg(DBG_V6_HIGH, "in %s(array: %p, array_to_add_p: %p, %jd)", __func__,
	...
	if (dbg_allowed(DBG_V6_HIGH)) {
	    dbg(DBG_V6_HIGH, "in %s: array->data: %p", __func__, (void *)array->data);
	    dbg(DBG_V6_HIGH, "in %s: alloc_size: %jd", __func__, alloc_size);
	    dbg(DBG_V6_HIGH, "in %s: last_alloc_byte: %p", __func__, (void *)last_alloc_byte);
	    dbg(DBG_V6_HIGH, "in %s: array_to_add_p: %p", __func__, (void *)array_to_add_p);
	    dbg(DBG_V6_HIGH, "in %s: data_size: %jd", __func__, data_size);
	    dbg(DBG_V6_HIGH, "in %s: last_add_byte: %p", __func__, (void *)last_add_byte);
	    dbg(DBG_V6_HIGH, "in %s: data_first_offset: %ju", __func__, data_first_offset);
	    dbg(DBG_V6_HIGH, "in %s: data_last_offset: %ju", __func__, data_last_offset);
	}
	mv_case = determine_move_case(array->data, last_alloc_byte, array_to_add_p, last_add_byte);
	if (dbg_allowed(DBG_V6_HIGH)) {
	    dbg(DBG_V6_HIGH, "in %s: move case: %s", __func__, move_case_name(mv_case));
	}
	...
	uint8_t *dest = NULL;		/* next destination byte for multi-part moves */
	...
	case MOVE_CASE_INSIDE:
	    ...
	    if (dbg_allowed(DBG_V6_HIGH)) {
		dbg(DBG_V6_HIGH, "mv_case: %s special "
	...
	case MOVE_CASE_BEFORE_INTO:
	    ...
	    if (dbg_allowed(DBG_V6_HIGH)) {
		dbg(DBG_V6_HIGH, "mv_case: %s special "
			     "memmove(dest: %p, array_to_add_p: %p, pre_length: %ju)",
			     move_case_name(mv_case),
			     (void *)dest, (void *)array_to_add_p, pre_length);
	    }
	    memmove((void *)dest, (void *)array_to_add_p, pre_length);

	    /* append data that moved with the dynamic array allocated data */
	    dest = beyond + pre_length;
	    if (dbg_allowed(DBG_V6_HIGH)) {
		dbg(DBG_V6_HIGH, "mv_case: %s special "
			     "memmove(dest: %p, array->data: %p, in_length: %ju)",
			     move_case_name(mv_case),
			     (void *)dest, array->data, in_length);
	    }
	    memmove((void *)dest, array->data, in_length);
	    break;
	...
	case MOVE_CASE_BEFORE_IN_BEYOND:
	    ...
	    if (dbg_allowed(DBG_V6_HIGH)) {
		dbg(DBG_V6_HIGH, "mv_case: %s special "
			     "memmove(dest: %p array->data: %p, alloc_size: %ju)",
			     move_case_name(mv_case),
			     (void *)dest, array->data, alloc_size);
	    }
	    memmove((void *)dest, array->data, alloc_size);

	    /* append the data that was beyond the end of the previous dynamic array allocated data */
	    dest = beyond + pre_length + alloc_size;
	    if (dbg_allowed(DBG_V6_HIGH)) {
		dbg(DBG_V6_HIGH, "mv_case: %s special "
			     "memmove(dest: %p array_to_add_p+pre_length+alloc_size: %p, post_length: %ju)",
			     move_case_name(mv_case),
			     (void *)dest,
			     (void *)((uint8_t *)(array_to_add_p) + pre_length + alloc_size),
			     post_length);
	    }
	    memmove((void *)dest,
		    (void *)((uint8_t *)(array_to_add_p) + pre_length + alloc_size),
	...
	case MOVE_CASE_IN_BEYOND:
	    ...
	    /* append the data that was beyond the end of the previous dynamic array allocated data */
	    dest = beyond + in_length;
	    if (dbg_allowed(DBG_V6_HIGH)) {
		dbg(DBG_V6_HIGH, "mv_case: %s special "
			     "memmove(dest: %p array_to_add_p+in_length: %p, post_length: %ju)",
			     move_case_name(mv_case),
			     (void *)dest, (void *)((uint8_t *)(array_to_add_p) + in_length), post_length);
	    }
	    memmove((void *)dest, (void *)((uint8_t *)(array_to_add_p) + in_length), post_length);
	    break;
    ...
    if (dbg_allowed(DBG_V7_HIGH)) {
	dbg(DBG_V7_HIGH, "in %s(array: %p, array_to_add_p %p, %jd): %s: allocated: %jd elements of size: %zu in use: %jd",
		       __func__, (void *)array, (void *)array_to_add_p,
		       count_of_elements_to_add,
