 ...
 */
bool
is_jprint_match(struct jprint *jprint, struct jprint_pattern *pattern, struct json *node, char *str)
{
    /* firewall */
    if (jprint == NULL) {
	...
	err(40, __func__, "pattern is NULL");
	not_reached();
    } else if (node == NULL) {
	...
	for (pattern = jprint->patterns; pattern != NULL; pattern = pattern->next) {
		...
		case JTYPE_NUMBER:	/* JSON item is number - see struct json_number */
			...
			if (item != NULL && item->converted) {
			    ...
			    if (str != NULL) {
				if (is_jprint_match(jprint, pattern, node, str)) {
...
jprint_sanity_chks(struct jprint *jprint, char const *program, int *argc, char ***argv)
{
	...
	err(57, __func__, "NULL argc");
	not_reached();
    } else if (argv == NULL || *argv == NULL || **argv == NULL) {
	err(58, __func__, "NULL argv");
	not_reached();
    } else if (program == NULL) {
	err(59, __func__, "NULL program");
	not_reached();
    }
	...
	free_jprint(&jprint);
	err(60, __func__, "-Y requires exactly one name_arg");
	not_reached();
...
run_jprint_check_tool(struct jprint *jprint, char **argv)
{
    ...
    /* firewall */
    if (jprint == NULL) {
	err(61, __func__, "NULL jprint");
	not_reached();
    } else if (jprint->file_contents == NULL) {
	err(62, __func__, "NULL jprint->file_contents");
	not_reached();
    }
