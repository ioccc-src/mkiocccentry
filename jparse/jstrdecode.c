...
static struct jstring *jstrdecode_stream(FILE *in_stream, bool skip_enclosing, bool ignore_first, bool remove_last,
	bool ignore_nl);
 ...
 */
static struct jstring *
jstrdecode_stream(FILE *in_stream, bool skip_enclosing, bool ignore_first, bool remove_last, bool ignore_nl)
{
     ...
     */
    dbg(DBG_LOW, "about to decode data on input stream");
     ...
     */
    buf = json_decode(input, inputlen, &bufsiz, skip_enclosing);
    if (buf == NULL) {
	/* free input */
	if (input != NULL) {
	    free(input);
	...
	warn(__func__, "error while decoding buffer");
	return NULL;
    }
...
main(int argc, char **argv)
{
	...
	for (i=optind; i < argc; ++i) {
		...
		if (!jstr) {
		    ...
		    dbg(DBG_LOW, "decoded buf size: %ju", (uintmax_t)jstr->bufsiz);
		}
	     ...
	     * decode arg
	     */
	    buf = json_decode_str(input, &bufsiz, skip_each);
	    if (buf == NULL) {
	...
	dbg(DBG_LOW, "decoding from stdin");
	/*
	 * NOTE: jstrdecode_stream() adds the allocated struct jstring * to the
	 * decoded JSON strings list
	 */
	jstr = jstrdecode_stream(stdin, skip_concat_quotes, skip_each, skip_each, ignore_nl);
	if (jstr != NULL) {
