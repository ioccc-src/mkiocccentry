...
prompt(char const *str, size_t *lenp)
{
     ...
     */
    if (str == NULL) {
	err(32, __func__, "called with NULL str");
	not_reached();
    }
...
get_contest_id(bool *testp, bool *read_answers_flag_used)
{
     ...
     */
    if (testp == NULL) {
	err(43, __func__, "called with NULL testp");
	not_reached();
    } else if (read_answers_flag_used == NULL) {
	err(44, __func__, "called with NULL read_answers_flag_used");
	not_reached();
    }
	     ...
	     char **tarball_path, time_t tstamp, bool test_mode)
{
    ...
    *tarball_path = form_tar_filename(ioccc_id, entry_num, test_mode, tstamp);
    if (*tarball_path == NULL) {
	errp(54, __func__, "failed to form compressed tarball path");
	not_reached();
    }
