...
check_prog_c(struct info *infop, char const *entry_dir, char const *cp, char const *prog_c)
{
    ...
    if (exit_code != 0) {
	err(90, __func__, "%s -- %s %s/prog.c failed with exit code: %d",
...
inspect_Makefile(char const *Makefile, struct info *infop)
{
    ...
    if (stream == NULL) {
	errp(93, __func__, "cannot open Makefile: %s", Makefile);
	not_reached();
    }
...
warn_Makefile(char const *Makefile, struct info *infop)
{
	...
	if (!yorn) {
	    err(96, __func__, "Use a different Makefile or modify this file: %s", Makefile);
	    not_reached();
	}
    }
}
...
check_Makefile(struct info *infop, char const *entry_dir, char const *cp, char const *Makefile)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(98, __func__, "Makefile does not exist: %s", Makefile);
	not_reached();
    }
    if (!is_file(Makefile)) {
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(99, __func__, "Makefile is not a file: %s", Makefile);
	not_reached();
    }
    if (!is_read(Makefile)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(100, __func__, "Makefile is not readable file: %s", Makefile);
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(103, __func__, "failed to cmdprintf: cp Makefile entry_dir/Makefile");
	not_reached();
    }
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(104, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    ...
    ret = fflush(stderr);
    if (ret < 0) {
	errp(105, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    if (exit_code < 0) {
	errp(106, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(107, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(108, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
	...
	errp(109, __func__, "malloc #1 of %lu bytes failed", (unsigned long)(LITLEN("Makefile") + 1));
	not_reached();
    }
...
check_remarks_md(struct info *infop, char const *entry_dir, char const *cp, char const *remarks_md)
{
	...
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(111, __func__, "remarks.md does not exist: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr, "",
	      ...
	      "",
	      NULL);
	err(112, __func__, "remarks_md is not a file: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(113, __func__, "remarks_md is not readable file: %s", remarks_md);
	not_reached();
    }
    filesize = file_size(remarks_md);
    if (filesize < 0) {
	err(114, __func__, "file_size error: %ld on remarks_md %s", (long)filesize, remarks_md);
	not_reached();
    } else if (filesize == 0) {
	err(115, __func__, "remarks.md cannot be empty: %s", remarks_md);
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(116, __func__, "failed to cmdprintf: cp remarks.md entry_dir/remarks.md");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(117, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    clearerr(stderr);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ret = fflush(stderr);
    if (ret < 0) {
	errp(118, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    exit_code = system(cmd);
    if (exit_code < 0) {
	errp(119, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(120, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(121, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
	...
	errp(122, __func__, "malloc #1 of %lu bytes failed", (unsigned long)(LITLEN("remarks.md") + 1));
	not_reached();
    }
...
check_extra_data_files(struct info *infop, char const *entry_dir, char const *cp, int count, char **args)
{
    ...
    if (count < 0) {
	err(124, __func__, "count :%d < 0", count);
	not_reached();
    }
	...
	errp(125, __func__, "calloc #0 of %d char * pointers failed", count + 1);
	not_reached();
    }
	...
	if (!exists(args[i])) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(126, __func__, "extra[%i] does not exist: %s", i, args[i]);
	    not_reached();
	}
	if (!is_file(args[i])) {
	    fpara(stderr,
		   ...
		   "",
		   NULL);
	    err(128, __func__, "extra[%i] is not a file: %s", i, args[i]);
	    not_reached();
	}
	if (!is_read(args[i])) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(129, __func__, "extra[%i] is not readable file: %s", i, args[i]);
	    not_reached();
	}
	...
	if (base_len == 0) {
	    err(130, __func__, "basename of extra data file: %s has a length of 0", args[i]);
	    not_reached();
	    ...
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(131, __func__, "basename of extra data file: %s is %lu characters an is > the limit: %lu",
	...
	if (!isascii(base[0]) || !isalnum(base[0])) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(132, __func__, "basename of extra data file: %s start with in invalid character: %s", args[i], base);
	    not_reached();
	}
	...
	for (j=0; j < base_len; ++j) {
		...
		fpara(stderr,
		      ...
		      "",
		      NULL);
		err(133, __func__, "basename of %s character %lu is NOT a POSIX Fully portable character nor +",
	...
	if (dest == NULL) {
	    errp(134, __func__, "malloc #0 of %lu bytes failed", (unsigned long)(dest_len + 1));
	    not_reached();
	}
	ret = snprintf(dest, dest_len, "%s/%s", entry_dir, base);
	if (ret <= 0) {
	    errp(135, __func__, "snprintf #0 error: %d", ret);
	    not_reached();
	}
	...
	if (exists(dest)) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(136, __func__, "for extra file: %s destination already exists: %s", args[i], dest);
	    not_reached();
	}
	...
	if (cmd == NULL) {
	    err(137, __func__, "failed to cmdprintf: cp extra_file[%d]: %s dest: %s", i, args[i], dest);
	    not_reached();
	}
	dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
	...
	ret = fflush(stdout);
	if (ret < 0) {
	    errp(138, __func__, "fflush(stdout) error code: %d", ret);
	    not_reached();
	}
	clearerr(stderr);		/* pre-clear ferror() status */
	errno = 0;			/* pre-clear errno for errp() */
	ret = fflush(stderr);
	if (ret < 0) {
	    errp(139, __func__, "fflush(stderr) #1: error code: %d", ret);
	    not_reached();
	}
	...
	exit_code = system(cmd);
	if (exit_code < 0) {
	    errp(140, __func__, "error calling system(%s)", cmd);
	    not_reached();
	} else if (exit_code == 127) {
	    errp(141, __func__, "execution of the shell failed for system(%s)", cmd);
	    not_reached();
	} else if (exit_code != 0) {
	    err(142, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	    not_reached();
	}
...
verify_entry_dir(char const *entry_dir, char const *ls)
{
    ...
    if (cmd == NULL) {
	err(163, __func__, "failed to cmdprintf: cd entry_dir && ls -lak .");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(164, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    clearerr(stderr);		/* pre-clear ferror() status */
    ...
    ret = fflush(stderr);
    if (ret < 0) {
	errp(165, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    exit_code = system(cmd);
    if (exit_code < 0) {
	errp(166, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(167, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(168, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(169, __func__, "fflush(stdout) #0: error code: %d", ret);
	not_reached();
    }
    clearerr(stderr);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ...
    if (ls_stream == NULL) {
	errp(171, __func__, "popen for reading failed for: %s", cmd);
	not_reached();
    }
    ...
    if (readline_len < 0) {
	err(172, __func__, "EOF while reading 1st line from ls: %s", ls);
	not_reached();
    } else {
    ...
    if (ret != 1) {
	err(173, __func__, "failed to parse block line from ls: %s", linep);
	not_reached();
    }
    if (kdirsize <= 0) {
	err(174, __func__, "ls k block value: %d <= 0", kdirsize);
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(175, __func__, "fprintf error while printing entry directory kibibyte sizes");
	    not_reached();
	}
	err(176, __func__, "The entry directory is too large: %s", entry_dir);
	not_reached();
    }
...
write_info(struct info *infop, char const *entry_dir, bool test_mode, char const *jinfochk, char const *fnamchk, int author_count)
{
    ...
    if (author_count <= 0) {
	err(179, __func__, "author_count %d <= 0", author_count);
	not_reached();
    } else if (author_count > MAX_AUTHORS) {
	err(180, __func__, "author count %d > max authors %d", author_count, MAX_AUTHORS);
	not_reached();
    }
	...
	errp(181, __func__, "strdup of %s failed", TIMESTAMP_EPOCH);
	not_reached();
    }
    ...
    ret = setenv("TZ", "UTC", 1);
    if (ret < 0) {
	errp(182, __func__, "cannot set TZ=UTC");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    timeptr = gmtime(&(infop->common.tstamp));
    if (timeptr == NULL) {
	errp(183, __func__, "localtime #1 returned NULL");
	not_reached();
    }
	...
	errp(184, __func__, "calloc of %lu bytes failed", (unsigned long)utctime_len + 1);
	not_reached();
    }
    ...
    if (strftime_ret == 0) {
	errp(185, __func__, "strftime returned 0");
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(194, __func__, "failed to cmdprintf: jinfochk %s", info_path);
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(195, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    clearerr(stderr);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ret = fflush(stderr);
    if (ret < 0) {
	errp(196, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    exit_code = system(cmd);
    if (exit_code < 0) {
	errp(197, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(198, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(199, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
...
write_author(struct info *infop, int author_count, struct author *authorp, char const *entry_dir, char const *jauthchk)
{
    ...
    if (author_count <= 0) {
	err(201, __func__, "author_count %d <= 0", author_count);
	not_reached();
    } else if (author_count > MAX_AUTHORS) {
	err(202, __func__, "author count %d > max authors %d", author_count, MAX_AUTHORS);
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(210, __func__, "failed to cmdprintf: jauthchk %s", author_path);
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(211, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    clearerr(stderr);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ret = fflush(stderr);
    if (ret < 0) {
	errp(212, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    exit_code = system(cmd);
    if (exit_code < 0) {
	errp(213, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(214, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(215, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
	     ...
	     char const *ls, char const *txzchk, char const *fnamchk)
{
    ...
    ret = chdir(work_dir);
    if (ret < 0) {
	errp(218, __func__, "cannot cd %s", work_dir);
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(219, __func__, "failed to cmdprintf: tar --format=v7 -cJf tarball_path entry_dir");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    if (ret != 0) {
	errp(225, __func__, "stat of the compressed tarball failed: %s", basename_tarball_path);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(226, __func__, "The compressed tarball: %s size: %lu > %ld",
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
	errp(227, __func__, "cannot fchdir to the previous current directory");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = close(cwd);
    if (ret < 0) {
	errp(228, __func__, "close of previous current directory failed");
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(229, __func__, "failed to cmdprintf: txzchk -q tarball_path");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(230, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    clearerr(stderr);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ret = fflush(stderr);
    if (ret < 0) {
	errp(231, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    exit_code = system(cmd);
    if (exit_code < 0) {
	errp(232, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(233, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(234, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
    para("",
...
remind_user(char const *work_dir, char const *entry_dir, char const *tar, char const *tarball_path, bool test_mode)
{
	...
	err(236, __func__, "failed to cmdprintf: entry_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(237, __func__, "printf #0 error");
	not_reached();
    }
    free(entry_dir_esc);
	...
	err(238, __func__, "failed to cmdprintf: work_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(239, __func__, "printf #2 error");
	not_reached();
    }
    free(work_dir_esc);
	...
	ret = printf("    %s\n\n", IOCCC_SUBMIT_URL);
	if (ret < 0) {
	    errp(241, __func__, "printf #4 error");
	    not_reached();
	}
    }
    return;
