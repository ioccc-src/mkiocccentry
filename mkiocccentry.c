...
static const char * const usage_msg3 =
    ...
    "\t-X path\t\tread ignore list from file path\n"
...
main(int argc, char *argv[])
{
    ...
    char const *workdir_arg = NULL;		/* workdir arg */
    char const *workdir_real = NULL;		/* realpath(3) evaluated workdir_arg */
    char const *workdir = NULL;			/* where the submission directory and tarball are formed */
    char *workdir_file = NULL;                  /* to test for workdir/{Makefile,prog.c,remarks.md} */
    char *prog_c = NULL;			/* path to prog.c */
    char *Makefile = NULL;			/* path to Makefile */
    char *remarks_md = NULL;			/* path to remarks.md */
    char const *topdir_arg = NULL;		/* topdir arg */
    char const *topdir_real = NULL;		/* realpath(3) evaluated topdir_arg */
    char *topdir = NULL;			/* directory from which files are to be copied to the workdir */
    char *topdir_file = NULL;                   /* to test for topdir/{Makefile,prog.c,remarks.md} */
    ...
    struct walk_stat wstat2;                    /* will be initialised in submission directory */
    ...
    bool cap_I = false;                         /* if -I specified we must recheck args */
    bool cap_X = false;                         /* if -X specified we must recheck args */
    bool found_workdir_prog_c = false;          /* if workdir/prog.c exists */
    bool found_topdir_prog_c = false;           /* if topdir/prog.c exists */
    bool found_workdir_Makefile = false;        /* if workdir/Makefile exists */
    bool found_topdir_Makefile = false;         /* if topdir/Makefile exists */
    bool found_workdir_remarks = false;         /* if workdir/remarks.md exists */
    bool found_topdir_remarks = false;          /* if topdir/remarks.md exists */
    ...
    program = argv[0];
    while ((i = getopt(argc, argv, ":hv:J:qVt:l:a:i:A:WT:ef:F:C:yYds:m:I:u:U:xr:M:X:")) != -1) {
	switch (i) {
	...
	case 'i':		/* -i input_recorded_answers */
	    ...
	    answers = optarg;
	    break;
	case 'W':		/* -W ignores all warnings (this does NOT mean the judges will! :) ) */
	...
	case 's':		/* set seed as seed & SEED_MASK */
        ...
        case 'M':
            errno = 0; /* pre-clear errno for errp() */
            manifest = fopen(optarg, "r");
            if (manifest == NULL) {
                errp(25, __func__, "manifest does not exist or could not be opened: %s", optarg);
                not_reached();
            }
            break;
        case 'X':
    ...
    /*
     * convert workdir command line arg into canonicalized absolute pathname
     *
     * Even though we late use canon_path(3) to process the path, we do so with
     * dotdot_err as true because we do NOT want paths to jump up over the top.
     * However, if the command line arg has .. (dot-dot)'s in the path, that
     * might trigger a dotdot_err error condition.  So to allow the user to
     * use such a path on the command line, we use realpath(3) to convert
     * it to an canonicalized absolute pathname.  This will allow canon_path(3)
     * to check for BOGUS sub-paths while allow the use to use  .. (dot-dot)'s.
     */
    workdir_arg = argv[optind];
    dbg(DBG_MED, "workdir_arg: %s", workdir_arg);
    errno = 0; /* pre-clear errno for errp() */
    workdirfd = open(workdir_arg, O_RDONLY|O_SEARCH|O_DIRECTORY|O_CLOEXEC);
    if (workdirfd < 0) {
        errp(3, __func__, "workdir arg is not a readable and searchable directory: %s", workdir_arg); /*ooo*/
        not_reached();
    }
    errno = 0;		/* pre-clear errno for errp() */
    workdir_real = realpath(workdir_arg, NULL); /* canonicalised path */
    if (workdir_real == NULL) {
	errp(26, program, "workdir arg: %s is an invalid path",
		workdir_arg); /*ooo*/
	not_reached();
    }

    dbg(DBG_MED, "workdir_real: %s", workdir_real);
    close(workdirfd);
    workdirfd = -1;
    ...
    workdir = canon_path(workdir_real,
			 0, 0, 0,
			 &sanity, NULL, NULL,
    ...
    if (workdir == NULL || sanity != PATH_OK) {
        err(3, __func__, "failed to canonicalize workdir: %s error: %s (%s)", /*ooo*/
			 workdir_real, path_sanity_name(sanity), path_sanity_error(sanity));
        not_reached();
    }
    ...
    /*
     * convert topdir command line arg into canonicalized absolute pathname
     *
     * Even though we later use canon_path(3) to process the path, we do so with
     * dotdot_err as true because we do NOT want paths to jump up over the top.
     * However, if the command line arg has .. (dot-dot)'s in the path, that
     * might trigger a dotdot_err error condition.  So to allow the user to
     * use such a path on the command line, we use realpath(3) to convert
     ...
     * to check for BOGUS sub-paths while allow the use to use  .. (dot-dot)'s.
     */
    topdir_arg = argv[optind+1];
    dbg(DBG_MED, "topdir_arg: %s", topdir_arg);
    errno = 0; /* pre-clear errno for errp() */
    topdirfd = open(topdir_arg, O_RDONLY|O_SEARCH|O_DIRECTORY|O_CLOEXEC);
    if (topdirfd < 0) {
        errp(3, __func__, "topdir arg is not a readable and searchable directory: %s", topdir_arg); /*ooo*/
        not_reached();
    }
    errno = 0;		/* pre-clear errno for errp() */
    topdir_real = realpath(topdir_arg, NULL);
    if (topdir_real == NULL) {
	errp(27, program, "topdir arg: %s is an invalid path",
		         topdir_arg); /*ooo*/
	not_reached();
    }
    dbg(DBG_MED, "topdir_real: %s", topdir_real);
    close(topdirfd);
    topdirfd = -1;
    ...
    if (topdir == NULL || sanity != PATH_OK) {
        err(3, __func__, "failed to canonicalize topdir: %s error: %s (%s)", /*ooo*/
			 topdir_real, path_sanity_name(sanity), path_sanity_error(sanity));
        not_reached();
    }
    ...
    /*
     * now we have to do some sanity checks to see if the user swapped the order
     * of workdir and topdir.
     *
     * To simplify the checks we will use calloc_path() (see soup/file_util.c)
     * so that we don't have to go chdir(3) to both and then back to the
     * starting point.
     *
     * Because we check for one file at a time we only need two pointers: one
     * for workdir and one for topdir. We will use exists() (see
     * soup/file_util.c) to test and then record which exists so we can check
     * for error conditions.
     *
     * NOTE: if topdir does not have a prog.c, remarks.md or Makefile we do NOT
     * report this here because the usual code will do that. This is strictly to
     * do some simple tests to see if the user swapped the order of workdir and
     * topdir at the command line.
     */

    /*
     * first prog.c
     */
    workdir_file = calloc_path(workdir_real, PROG_C_FILENAME);
    if (workdir_file == NULL) {
        err(28, __func__, "could not allocate path %s/%s", workdir_real, PROG_C_FILENAME);
        not_reached();
    }
    if (exists(workdir_file)) {
        found_workdir_prog_c = true;
    } else {
        found_workdir_prog_c = false;
    }
    if (workdir_file != NULL) {
        free(workdir_file);
        workdir_file = NULL;
     ...
     * now Makefile
     */
    workdir_file = calloc_path(workdir_real, MAKEFILE_FILENAME);
    if (workdir_file == NULL) {
        err(29, __func__, "could not allocate path %s/%s", workdir_real, MAKEFILE_FILENAME);
        not_reached();
    }
    if (exists(workdir_file)) {
        found_workdir_Makefile = true;
    } else {
        found_workdir_Makefile = false;
    }
    if (workdir_file != NULL) {
        free(workdir_file);
        workdir_file = NULL;
     ...
     * finally remarks.md
     */
    workdir_file = calloc_path(workdir_real, REMARKS_FILENAME);
    if (workdir_file == NULL) {
        err(30, __func__, "could not allocate path %s/%s", workdir_real, MAKEFILE_FILENAME);
        not_reached();
    }
    if (exists(workdir_file)) {
        found_workdir_remarks = true;
    } else {
        found_workdir_remarks = false;
    }
    if (workdir_file != NULL) {
        free(workdir_file);
        workdir_file = NULL;
    ...
    /*
     * now do the same for topdir
     */
    /*
     * first prog.c
     */
    topdir_file = calloc_path(topdir_real, PROG_C_FILENAME);
    if (topdir_file == NULL) {
        err(31, __func__, "could not allocate path %s/%s", topdir_real, PROG_C_FILENAME);
        not_reached();
    }
    if (exists(topdir_file)) {
        found_topdir_prog_c = true;
    } else {
        found_topdir_prog_c = false;
    }
    if (topdir_file != NULL) {
        free(topdir_file);
        topdir_file = NULL;
     ...
     * now Makefile
     */
    topdir_file = calloc_path(topdir_real, MAKEFILE_FILENAME);
    if (topdir_file == NULL) {
        err(32, __func__, "could not allocate path %s/%s", topdir_real, MAKEFILE_FILENAME);
        not_reached();
    }
    if (exists(topdir_file)) {
        found_topdir_Makefile = true;
    } else {
        found_topdir_Makefile = false;
    }
    if (topdir_file != NULL) {
        free(topdir_file);
        topdir_file = NULL;
     ...
     * finally remarks.md
     */
    topdir_file = calloc_path(topdir_real, REMARKS_FILENAME);
    if (topdir_file == NULL) {
        err(33, __func__, "could not allocate path %s/%s", topdir_real, MAKEFILE_FILENAME);
        not_reached();
    }
    if (exists(topdir_file)) {
        found_topdir_remarks = true;
    } else {
        found_topdir_remarks = false;
    }
    if (topdir_file != NULL) {
        free(topdir_file);
        topdir_file = NULL;
     ...
     * first prog.c
     */
    if (!found_topdir_prog_c && found_workdir_prog_c) {
        err(3, __func__, "found prog.c in workdir but not in prog.c; did you swap topdir and workdir?"); /*ooo*/
        not_reached();
    }
    /*
     * Makefile
     */
    if (!found_topdir_Makefile && found_workdir_Makefile) {
        err(3, __func__, "found Makefile in workdir but not in Makefile; did you swap topdir and workdir?"); /*ooo*/
        not_reached();
    }
    /*
     * last remarks.md
     */
    if (!found_topdir_remarks && found_workdir_remarks) {
        err(3, __func__, "found remarks.md in workdir but not in remarks.md; did you swap topdir and workdir?"); /*ooo*/
        not_reached();
    }
    ...
    memset(&wstat2, 0, sizeof(wstat2)); /* this will be initialised in a later function */
    wset_p = &walk_mkiocccentry;
    context = "mkiocccentry";
    init_walk_stat(&wstat,
		   topdir_real, wset_p, context,
		   MAX_PATH_LEN, MAX_FILENAME_LEN, MAX_PATH_DEPTH,
		   false);

    /*
     * if -I or -X. We have to do this because in order to initialise the walk_stat we
     * have to have the topdir but we can't have that until we have gone through
     * the args.
     */
    if (cap_I || cap_X) {
        optind = 1;
    ...
    if (ret < 0) {
	errp(34, __func__, "gettimeofday failed");
	not_reached();
    }
    ...
    dbg(DBG_HIGH, "info.usec: %d", info.usec);
     ...
     * warn about -Y option
     */
    if (force_yes && !read_answers_flag_used) {
        para("",
    ...
    } else if (answer_yes && !read_answers_flag_used) {
        /* warn about -y option */
        para("",
	...
	if (!overwrite_answers) {
	    ...
	    if (ret <= 0) {
		errp(35, __func__, "printf error telling the user how to use the answers file");
		not_reached();
	    }
	    err(36, __func__, "won't overwrite answers file");
	    not_reached();
	}
    }
	...
	if (!is_read(answers)) {
	    errp(37, __func__, "answers file not readable");
	    not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	answersp = fopen(answers, "r");
	if (answersp == NULL) {
	    errp(38, __func__, "cannot open answers file");
	    not_reached();
	}
    ...
    /*
     * note answers file FD if not NULL and not stdin
     */
    if (read_answers_flag_used && answersp != NULL && answersp != stdin)
    {
        errno = 0;			/* pre-clear errno for errp() */
        answersfd = open(answers, O_RDONLY|O_CLOEXEC);
        if (answersfd < 0) {
            errp(39, __func__, "cannot obtain answers FD");
            not_reached();
        }
         ...
         */
        errno = 0; /* pre-clear errno for errp() */
        if (fstat(answersfd, &answers_st) != 0) {
            errp(3, __func__, "failed to get fstat(2) info for answers file: %s", answers); /*ooo*/
            not_reached();
        }
    } else {
        /*
	...
	errp(40, __func__, "strdup() tarball path %s failed", tarball_path);
	not_reached();
    }
	...
	if (answersp == NULL) {
	    errp(41, __func__, "cannot create answers file: %s", answers);
	    not_reached();
	}
        errno = 0; /* pre-clear errno for errp() */
        ret = fprintf(answersp, "%s\n", MKIOCCCENTRY_ANSWERS_VERSION);
	if (ret <= 0) {
	    errp(42, __func__, "fprintf error printing header to the answers file");
            not_reached();
	}
    }
	...
	errno = 0;			/* pre-clear errno for errp() */
        ret = fprintf(answersp, "%s\n", info.ioccc_id);
	if (ret <= 0) {
	    errp(43, __func__, "fprintf error printing IOCCC contest id to the answers file");
            not_reached();
	}
	errno = 0;			/* pre-clear errno for errp() */
	ret = fprintf(answersp, "%u\n", info.submit_slot);
	if (ret <= 0) {
	    errp(44, __func__, "fprintf error printing submit slot number to the answers file");
            not_reached();
	}
    }
   ...
   /*
     ...
     */
    scan_topdir(&wstat, context, &info, make, submission_dir, &size, &wstat2);
	...
	ret = fprintf(answersp, "%s\n", info.title);
	if (ret <= 0) {
	    errp(45, __func__, "fprintf error printing title to the answers file");
            not_reached();
	}
    }
	...
	ret = fprintf(answersp, "%s\n", info.abstract);
	if (ret <= 0) {
	    errp(46, __func__, "fprintf error printing abstract to the answers file");
            not_reached();
	}
    }
    ...
    */
	...
	if (ret <= 0) {
	    errp(47, __func__, "fprintf error printing IOCCC author count to the answers file");
            not_reached();
	}
    ...
    write_json_files(&wstat2, &auth, &info, submission_dir, chksubmit);
    if (!quiet) {
	...
	if (read_answers_flag_used) {
	    ...
	    if (error) {
	        errp(48, __func__, "expected ANSWERS_EOF marker at the end of the answers file");
	        not_reached();
	    }
	    input_stream = stdin;
	    if (line != NULL) {
	    ...
	    if (ret <= 0) {
	        errp(49, __func__, "fprintf error writing ANSWERS_EOF marker to the answers file");
                not_reached();
	    }
	}
	if (answers != NULL) {
	    ...
	    ret = fclose(answersp);
	    if (ret != 0) {
	        errp(50, __func__, "error in fclose to the answers file");
                not_reached();
	    }
	}
	answersp = NULL;
    ...
    /*
     * close answers FD if -i answers used
     */
    if (answersfd > 0 && read_answers_flag_used) {
        errno = 0; /* pre-clear errno for errp() */
        if (close(answersfd) != 0) {
            errp(51, __func__, "failed to close(answers)");
            not_reached();
        }
    }
    ...
    if (topdir_real != NULL) {
        free((void *)topdir_real);
        topdir_real = NULL;
    }
    if (workdir_real != NULL) {
        free((void *)workdir_real);
        workdir_real = NULL;
    }
    ...
    if (manifest != NULL) {
        ...
        if (fclose(manifest) != 0) {
            warnp(__func__, "failed to close manifest file");
        }
    }
	    ...
	    RuleCount *size, struct walk_stat *wstat2)
{
	...
	size == NULL || wstat2 == NULL) {
        err(52, __func__, "passed NULL pointer(s)");
        not_reached();
    }
    topdir = wstat->topdir;
    ...
    if (chdir(submission_dir) != 0) {
        errp(53, __func__, "unable to change to submission directory");
        not_reached();
    }
    ...
    if (submit_path == NULL) {
        errp(54, __func__, "couldn't get absolute path of submission directory");
        not_reached();
    }
    ...
    if (fchdir(topdirfd) != 0) {
        err(55, __func__, "failed to switch back to original directory");
        not_reached();
    }
    ...
    /*
     * sort walk_stat arrays by canonicalised path in a case independent way
     */
	       ...
	       char const *submission_dir, char *submit_path, RuleCount *size, struct walk_stat *wstat2)
{
	...
	submit_path == NULL || size == NULL || wstat2 == NULL) {
        err(56, __func__, "passed NULL pointer(s)");
        not_reached();
    }
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
        errp(57, __func__, "unable to fchdir(cwd)");
        not_reached();
    }
     ...
     *
     * Here we MUST check that the wstat->skip_set is not NULL before trying to
     * get the length!
     */
    if (wstat->skip_set != NULL) {
        len = dyn_array_tell(wstat->skip_set);
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(58, __func__, "found NULL pointer in ignored path list, element: %jd", i);
                    not_reached();
                } else if (p->fts_path == NULL) {
                    err(59, __func__, "found NULL Path in ignored path list set, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p->fts_path);
            }
    ...
    if (wstat->prune != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(60, __func__, "found NULL pointer in ignored paths list, element: %jd", i);
                    not_reached();
                } else if (p->fts_path == NULL) {
                    err(61, __func__, "found NULL path in ignored paths list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p->fts_path);
            }
     ...
     * allowed), if any
     */
    if (wstat->symlink != NULL) {
        len = dyn_array_tell(wstat->symlink);
        if (len > 0) {
            if (need_hints) {
                para("",
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(62, __func__, "found NULL pointer in ignored symlinks list, element: %jd", i);
                    not_reached();
                } else if (p->fts_path == NULL) {
                    err(63, __func__, "found NULL path in ignored symlinks list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p->fts_path);
            }
    ...
    if (wstat->counted_dir != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(64, __func__, "found NULL pointer in directories list, element: %jd", i);
                    not_reached();
                } else if (p->fts_path == NULL) {
                    err(65, __func__, "found NULL path in directories list, element: %jd", i);
                    not_reached();
     ...
     */
    if (wstat->file == NULL) {
        err(4, __func__, "files list is NULL"); /*ooo*/
        not_reached();
    } else {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(66, __func__, "found NULL pointer in files list, element: %jd", i);
                    not_reached();
                } else if (p->fts_path == NULL) {
                    err(67, __func__, "found NULL path in files list, element: %jd", i);
                    not_reached();
                } else if (path_in_item_array(wstat->prune, p->fts_path) != NULL) {
                    continue;
                }
	    ...
	    char const *submission_dir, char *submit_path, RuleCount *size, struct walk_stat *wstat2)
{
    ...
    if (wstat->counted_dir != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                p = dyn_array_value(wstat->counted_dir, struct item *, i);
                if (p == NULL) {
                    err(68, __func__, "found NULL pointer in directories list, element: %jd", i);
                    not_reached();
                } else if (p->fts_path == NULL) {
                    err(69, __func__, "found NULL path in directories list, element: %jd", i);
                    not_reached();
                } else if (path_in_item_array(wstat->prune, p->fts_path) != NULL) {
                    continue;
                }
    ...
    errno = 0;
    if (fchdir(topdirfd) != 0) {
        errp(70, __func__, "cannot change to topdir");
        not_reached();
    }
     ...
     */
    if (wstat->file == NULL) {
        err(71, __func__, "file set is NULL");
        not_reached();
    }
    len = dyn_array_tell(wstat->file);
    if (len <= 0) {
        err(72, __func__, "list of files is empty");
        not_reached();
    }
    for (i = 0; i < len; ++i) {
        p = dyn_array_value(wstat->file, struct item *, i);
        if (p == NULL) {
            err(73, __func__, "found NULL pointer in files list, element: %jd", i);
            not_reached();
        } else if (p->fts_path == NULL) {
            err(74, __func__, "found NULL path in files list, element: %jd", i);
            not_reached();
        } else if (path_in_item_array(wstat->prune, p->fts_path) != NULL) {
            continue;
        }
        ...
        if (fname == NULL) {
            err(75, __func__, "couldn't allocate path to copy");
            not_reached();
        }
        if (target_path != NULL) {
            free(target_path);
        ...
        if (target_path == NULL) {
            errp(76, __func__, "failed to allocate target path for %s", p->fts_path);
            not_reached();
        }
        ...
        if (ret <= 0) {
            errp(77, __func__, "snprintf to form target path for %s failed", fname);
            not_reached();
        }
    ...
    if (close(workdirfd) != 0) {
        errp(78, __func__, "failed to close(workdirfd)");
        not_reached();
    }
    errno = 0; /* pre-clear errno for errp() */
    if (close(topdirfd) != 0) {
        errp(79, __func__, "failed to close(topdirfd)");
        not_reached();
    }
 ...
 * given:
 *
 *      wstat               - pointer to walk_stat struct
 *      context             - context from walk code
 ...
 *      wstat2              - pointer to walk_stat struct for submission directory (for main())
 *
 * NOTE: this function does not return on NULL pointers.
		     ...
		     char const *make, RuleCount *size, struct walk_stat *wstat2)
{
    ...
    intmax_t i = 0;                     /* index into arrays */
    bool yorn = false;                  /* when prompting yes or no */

    /*
     * firewall
     */
    if (wstat == NULL || context == NULL || infop == NULL || submit_path == NULL || wstat->topdir == NULL ||
            make == NULL || size == NULL || wstat2 == NULL) {
        err(80, __func__, "passed NULL arg(s)");
        not_reached();
    }
    /*
     * cwd must be >= 0.
     ...
     */
    if (cwd < 0) {
        err(81, __func__, "original directory file descriptor < 0");
        not_reached();
    }
    ...
    /*
     * initialise the walk_stat structure
     *
     * NOTE: we already zeroed it out in main()
     */
    wset_p2 = &walk_mkiocccentry;
    context2 = "mkiocccentry";
    init_walk_stat(wstat2,
    ...
    if (!walk_ok) {
        err(4, MKIOCCCENTRY_BASENAME, "failed to scan: %s", submit_path); /*ooo*/
        not_reached();
    }
     ...
     * sort walk_stat arrays by canonicalised path in a case independent way
     */
    sort_walk_istat(wstat2);

    /*
     * end walk and check if the walk was successful
     */
    walk_ok = chk_walk(wstat2, stderr, MAX_EXTRA_FILE_COUNT, MAX_EXTRA_DIR_COUNT, NO_COUNT, NO_COUNT, true);
    if (walk_ok) {
        dbg(DBG_LOW, "%s walk was successful for: %s", context2, submit_path);
    } else {
        err(4, MKIOCCCENTRY_BASENAME, "submission directory is invalid: %s", submit_path); /*ooo*/
        not_reached();
    }
     ...
     */
    if (wstat2->file == NULL) {
        err(82, __func__, "submission directory has empty file set");
        not_reached();
    } else {
        if (wstat->file == NULL) {
            err(83, __func__, "topdir was empty but submission directory not");
            not_reached();
        }
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p2 == NULL) {
                ...
                err(84, __func__, "NULL filename in file list, element: %jd", i);
                not_reached();
            }
            ...
            if (!strcasecmp(p2->fts_name, PROG_C_FILENAME) && p2->fts_level < 2) {
                ...
                *size = check_prog_c(infop, p2->fts_path);
                if (!quiet) {
                ...
                check_Makefile(infop, p2->fts_path);
                if (!quiet) {
                ...
                check_remarks_md(infop, p2->fts_path);
                if (!quiet) {
    ...
    if (wstat2->counted_dir != NULL) {
        ...
        if (len > 0) {
             ...
             */
            for (i = 0; i < len; ++i) {
                p = dyn_array_value(wstat2->counted_dir, struct item *, i);
                if (p == NULL) {
                    err(85, __func__, "found NULL pointer in directories list, element: %jd", i);
                    not_reached();
                } else if (p->fts_path == NULL) {
                    err(86, __func__, "found NULL directory name in directories list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p->fts_path);
            }
     ...
     */
    if (wstat2->file == NULL) {
        err(87, __func__, "files list in submission directory is NULL");
        not_reached();
    }
    len = dyn_array_tell(wstat2->file);
    if (len <= 0) {
        err(4, __func__, "list files in submission directory is empty"); /*ooo*/
        not_reached();
    }
    if (len > 0) {
        para("",
         ...
         */
        for (i = 0; i < len; ++i) {
            p = dyn_array_value(wstat2->file, struct item *, i);
            if (p == NULL) {
                err(88, __func__, "found NULL pointer in files list, element: %jd", i);
                not_reached();
            } else if (p->fts_path == NULL) {
                err(89, __func__, "found NULL filename in files list, element: %jd", i);
                not_reached();
            }
            print("%s\n", p->fts_path);
        }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(90, __func__, "unable to change to previous directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (close(cwd) != 0) {
        errp(91, __func__, "failed to close(cwd)");
        not_reached();
    }
			 ...
			 char *chksubmit, char *make, char *rm)
{
	...
	err(92, __func__, "called with NULL arg(s)");
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (tar != NULL) {
	    err(93, __func__, "tar does not exist: %s", tar);
        } else {
	    err(94, __func__, "tar does not exist");
        }
	not_reached();
    }
    if (!is_file(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(95, __func__, "tar is not a regular file: %s", tar);
	not_reached();
    }
    if (!is_exec(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(96, __func__, "tar is not an executable program: %s", tar);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (ls != NULL) {
            err(97, __func__, "ls does not exist: %s", ls);
        } else {
            err(98, __func__, "ls does not exist");
        }
	not_reached();
    }
    if (!is_file(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(99, __func__, "ls is not a regular file: %s", ls);
	not_reached();
    }
    if (!is_exec(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(100, __func__, "ls is not an executable program: %s", ls);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (txzchk != NULL) {
	    err(101, __func__, "txzchk does not exist: %s", txzchk);
        } else {
	    err(102, __func__, "txzchk does not exist");
        }
	not_reached();
    }
    if (!is_file(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(103, __func__, "txzchk is not a regular file: %s", txzchk);
	not_reached();
    }
    if (!is_exec(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(104, __func__, "txzchk is not an executable program: %s", txzchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (fnamchk != NULL) {
	    err(105, __func__, "fnamchk does not exist: %s", fnamchk);
        } else {
	    err(106, __func__, "fnamchk does not exist");
        }
	not_reached();
    }
    if (!is_file(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(107, __func__, "fnamchk is not a regular file: %s", fnamchk);
	not_reached();
    }
    if (!is_exec(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(108, __func__, "fnamchk is not an executable program: %s", fnamchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (chksubmit != NULL) {
	    err(109, __func__, "chksubmit does not exist: %s", chksubmit);
        } else {
	    err(110, __func__, "chksubmit does not exist");
        }
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(111, __func__, "chksubmit is not a regular file: %s", chksubmit);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(112, __func__, "chksubmit is not an executable program: %s", chksubmit);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (make != NULL) {
	    err(113, __func__, "make does not exist: %s", make);
        } else {
	    err(114, __func__, "make does not exist");
        }
	not_reached();
    }
    if (!is_file(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(115, __func__, "make is not a regular file: %s", make);
	not_reached();
    }
    if (!is_exec(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(116, __func__, "make is not an executable program: %s", make);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (rm != NULL) {
	    err(117, __func__, "rm does not exist: %s", rm);
        } else {
	    err(118, __func__, "rm does not exist");
        }
	not_reached();
    }
    if (!is_file(rm)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(119, __func__, "rm is not a regular file: %s", rm);
	not_reached();
    }
    if (!is_exec(rm)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(120, __func__, "rm is not an executable program: %s", rm);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(121, __func__, "workdir does not exist: %s", workdir);
	not_reached();
    }
    if (!is_dir(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(122, __func__, "workdir is not a directory: %s", workdir);
	not_reached();
    }
    if (!is_write(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(123, __func__, "workdir is not a writable directory: %s", workdir);
	not_reached();
    }
...
prompt(char const *str, size_t *lenp)
{
     ...
     */
    if (str == NULL) {
	err(124, __func__, "called with NULL str");
	not_reached();
    }

    /*
     * use stdin unless reading answers from an input stream
     */
    if (copying_topdir) {
        stream = stdin;
    } else {

	/* firewall - input_stream cannot be NULL */
	if (input_stream == NULL) {
	    err(125, __func__, "copying_topdir is false and input_stream is NULL");
	    not_reached();
	}

	/* use input_stream instead of stdin */
        stream = input_stream;
    }
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(126, __func__, "error printing prompt string");
		not_reached();
	    } else if (feof(stdout)) {
		err(128, __func__, "EOF while printing prompt string");
		not_reached();
	    } else {
		errp(129, __func__, "unexpected fputs error printing prompt string");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(130, __func__, "error printing :<space>");
		not_reached();
	    } else if (feof(stdout)) {
		err(131, __func__, "EOF while writing :<space>");
		not_reached();
	    } else {
		errp(132, __func__, "unexpected fputs error printing :<space>");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(133, __func__, "error flushing prompt to stdout");
		not_reached();
	    } else if (feof(stdout)) {
		err(134, __func__, "EOF while flushing prompt to stdout");
		not_reached();
	    } else {
		errp(135, __func__, "unexpected fflush error while flushing prompt to stdout");
		not_reached();
	    }
	}
    }
    ...
    if (buf == NULL) {
	err(136, __func__, "EOF while reading prompt input");
	not_reached();
    }
    dbg(DBG_VHIGH, "received a %zu byte response", len);
...
get_contest_id(bool *testp, char const *uuidfile, char *uuidstr)
{
     ...
     */
    if (testp == NULL) {
	err(137, __func__, "called with NULL testp");
	not_reached();
    }
	...
	if (read_answers_flag_used && !seen_answers_header) {
	    err(138, __func__, "didn't find the correct answers file header");
	    not_reached();
	}
...
get_submit_slot(struct info *infop)
{
    ...
    if (need_hints) {
        ...
        if (ret <= 0) {
            errp(140, __func__, "printf error printing number of submissions allowed");
            not_reached();
        }
        para("",
	...
	if (ret != 1 || submit_slot > MAX_SUBMIT_SLOT) {
	    ...
	    ret = fprintf(stderr, "\nThe submit slot number must be a number from 0 through %u; please re-enter.\n",
		    MAX_SUBMIT_SLOT);
	    if (ret <= 0) {
		errp(141, __func__, "fprintf error while informing about the valid submit slot number range");
                not_reached();
	    }
            /*
             * free storage
	    ...
	    submit_slot = MAX_SUBMIT_SLOT + 1;	/* invalidate input */
            ...
            if (need_confirm && !answer_yes) {
                ...
                ret = printf("\nThe slot number you entered is: %u\n",
                             submit_slot);
                if (ret <= 0) {
                    errp(142, __func__, "fprintf error writing slot number");
                    not_reached();
                }
		  ...
		  char const *rm)
{
    ...
    if (test == false) {
	err(144, __func__, "submit slot number: %u must >= 0 and <= %u", submit_slot, MAX_SUBMIT_SLOT);
	not_reached();
    }
    ...
    if (submission_dir == NULL) {
	errp(145, __func__, "calloc #0 of %zu bytes failed", (submission_dir_len + 1));
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(submission_dir, submission_dir_len + 1, "%s/%s-%u", workdir, ioccc_id, submit_slot);
    if (ret <= 0) {
	errp(146, __func__, "snprintf to form submission directory failed");
	not_reached();
    }
    ...
    if (exists(submission_dir)) {
        if (!force_remove) {
            ...
            if (ret <= 0) {
                errp(147, __func__, "fprintf error while informing that the submission directory already exists");
                not_reached();
            }
            fpara(stderr,
                  "",
                  ...
                  "",
                  NULL);
            err(148, __func__, "submission directory exists: %s", submission_dir);
            not_reached();
        } else {
            /*
            ...
            if (exit_code != 0) {
                err(149, __func__, "%s -rf %s failed with exit code: %d",
    ...
    if (ret < 0) {
	errp(150, __func__, "cannot mkdir %s", submission_dir);
	not_reached();
    }
    errno = 0; /* pre-clear errno for errp() */
    ret = chmod(submission_dir, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    if (ret < 0) {
        errp(151, __func__, "cannot chmod directory %s to mode 0755", submission_dir);
        not_reached();
    }
    ...
    *tarball_path = form_tar_filename(ioccc_id, submit_slot, test_mode, tstamp);
    if (*tarball_path == NULL) {
	errp(152, __func__, "failed to form compressed tarball path");
	not_reached();
    }
...
warn_empty_prog(void)
{
	...
	if (!yorn) {
	    err(153, __func__, "please fix your prog.c file");
	    not_reached();
	}
...
warn_rule_2a_size(struct info *infop, int mode, RuleCount size)
{
     ...
     */
    if (infop == NULL) {
	err(154, __func__, "called with NULL infop");
	not_reached();
    }
	...
	dbg(DBG_MED, "prog.c size: %zd > Rule 2a size: %d", infop->rule_2a_size, RULE_2A_SIZE);
	errno = 0;		/* pre-clear errno for errp() */
	ret = fprintf(stderr, "\nWARNING: The prog.c size: %zd > Rule 2a maximum: %d\n", infop->rule_2a_size, RULE_2A_SIZE);
	if (ret <= 0) {
	    errp(155, __func__, "fprintf error when printing prog.c Rule 2a warning");
            not_reached();
	}
	if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes) || (read_answers_flag_used && !force_yes)) {
	    ...
	    if (!yorn) {
		err(156, __func__, "please fix your prog.c file");
		not_reached();
	    }
	    dbg(DBG_MED, "user says that their prog.c size: %zu > Rule 2a max size: %d is OK", infop->rule_2a_size, RULE_2A_SIZE);
	}
	...
	if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes) || (read_answers_flag_used && !force_yes)) {
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "\nInteresting: prog.c file size: %zu != rule_count function size: %zu\n"
				  "In order to avoid a possible Rule 2a violation, BE SURE TO CLEARLY MENTION THIS IN\n"
				  "YOUR remarks.md FILE!\n\n",
				  infop->rule_2a_size, size.rule_2a_size);
	    if (ret <= 0) {
		errp(157, __func__, "fprintf error when printing prog.c file size and Rule 2a mismatch");
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	    ...
	    if (!yorn) {
		err(158, __func__, "please fix your prog.c file");
		not_reached();
	    }
	    dbg(DBG_MED, "user says that prog.c size: %zu != rule_count function size: %zu is OK",
		infop->rule_2a_size, size.rule_2a_size);
	}
	...
	err(159, __func__, "invalid mode passed to function: %d", mode);
	not_reached();
    }
    return;
}
...
warn_trigraph(void)
{
	...
	if (ret <= 0) {
	    errp(160, __func__, "fprintf error when printing prog.c trigraph_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	yorn = yes_or_no("Are you sure you want to proceed? [Ny]", false);
	if (!yorn) {
	    err(161, __func__, "please fix your prog.c file");
	    not_reached();
	}
...
warn_ungetc(void)
{
	...
	ret = fprintf(stderr, "\nprog.c triggered an ungetc error: @SirWumpus goofed\n"
			      ...
			      "YOUR remarks.md FILE!\n\n");
	if (ret <= 0) {
	    errp(162, __func__, "fprintf error when printing prog.c ungetc_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	yorn = yes_or_no("Are you sure you want to proceed? [Ny]", false);
	if (!yorn) {
	    err(163, __func__, "please fix your prog.c file");
	    not_reached();
	}
...
warn_rule_2b_size(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(164, __func__, "called with NULL infop");
	not_reached();
    }
    ...
    if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes) || (read_answers_flag_used && !force_yes)) {
	errno = 0;
	ret = fprintf(stderr, "\nWARNING: The prog.c size: %zu > Rule 2b maximum: %d\n", infop->rule_2b_size, RULE_2B_SIZE);
	if (ret <= 0) {
	    errp(165, __func__, "printf error printing prog.c size > Rule 2b maximum");
	    not_reached();
	}
...
check_prog_c(struct info *infop, char const *prog_c)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(168, __func__, "prog.c does not exist: %s", prog_c);
	not_reached();
    }
    if (!is_file(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(169, __func__, "prog.c is not a regular file: %s", prog_c);
	not_reached();
    }
    if (!is_read(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(170, __func__, "prog.c is not a readable file: %s", prog_c);
	not_reached();
    }
    ...
    if (prog_stream == NULL) {
	errp(171, __func__, "failed to fopen: %s", prog_c);
	not_reached();
    }
    size = rule_count(prog_stream);
    infop->rule_2b_size = size.rule_2b_size;
    dbg(DBG_MED, "prog.c: %s Rule 2b size: %zu", prog_c, infop->rule_2b_size);
    errno = 0;			/* pre-clear errno for errp() */
    ret = fclose(prog_stream);
    if (ret != 0) {
	errp(172, __func__, "failed to fclose: %s", prog_c);
	not_reached();
    }
    ...
    dbg(DBG_MED, "Rule 2a size: %zd", infop->rule_2a_size);
    if (infop->rule_2a_size == 0 || infop->rule_2b_size == 0) {
	warn_empty_prog();
...
inspect_Makefile(char const *Makefile, struct info *infop)
{
    ...
    if (stream == NULL) {
	errp(174, __func__, "cannot open Makefile: %s", Makefile);
	not_reached();
    }
    ...
    ret = fclose(stream);
    if (ret < 0) {
	errp(175, __func__, "fclose error");
	not_reached();
    }
...
warn_Makefile(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(176, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (!answer_yes && !force_yes) {
	    ...
	    if (!yorn) {
		err(177, __func__, "Use a different Makefile or modify your Makefile");
		not_reached();
	    }
	}
    }
...
check_Makefile(struct info *infop, char const *Makefile)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(179, __func__, "Makefile does not exist: %s", Makefile);
	not_reached();
    }
    if (!is_file(Makefile)) {
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(180, __func__, "Makefile is not a regular file: %s", Makefile);
	not_reached();
    }
    if (!is_read(Makefile)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(181, __func__, "Makefile is not readable file: %s", Makefile);
	not_reached();
    }
    filesize = file_size(Makefile);
    if (filesize < 0) {
	err(182, __func__, "file_size error: %lld on Makefile  %s", (long long)filesize, Makefile);
	not_reached();
    } else if (filesize == 0) {
	err(183, __func__, "Makefile cannot be empty: %s", Makefile);
	not_reached();
    }
...
check_remarks_md(struct info *infop, char const *remarks_md)
{
	...
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(185, __func__, "remarks.md does not exist: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr, "",
	      ...
	      "",
	      NULL);
	err(186, __func__, "remarks.md is not a regular file: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(187, __func__, "remarks.md is not readable file: %s", remarks_md);
	not_reached();
    }
    filesize = file_size(remarks_md);
    if (filesize < 0) {
	err(188, __func__, "file_size error: %lld on remarks.md %s", (long long)filesize, remarks_md);
	not_reached();
    } else if (filesize == 0) {
	err(189, __func__, "remarks.md cannot be empty: %s", remarks_md);
	not_reached();
    }
    return;
}
...
yes_or_no(char const *question, bool def_answer)
{
     ...
     */
    if (question == NULL) {
	err(190, __func__, "called with NULL question");
	not_reached();
    }
...
get_title(struct info *infop)
{
	...
	if (ret <= 0) {
	    errp(192, __func__, "fprintf #0 error: %d", ret);
            not_reached();
	}
    }
	 ...
	 */
	    ...
	    ret = fprintf(stderr, "Your title must be between 1 and %d ASCII characters long.\n\n", MAX_TITLE_LEN);
	    if (ret <= 0) {
		errp(193, __func__, "fprintf #1 error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	if (!isalnum((unsigned char)title[0]) || safe == false) {
        ...
        if (need_confirm && !answer_yes) {
            ...
            ret = printf("\nThe title you entered is: %s\n",
                         title);
            if (ret <= 0) {
                errp(194, __func__, "fprintf title");
                not_reached();
            }
...
get_abstract(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(195, __func__, "called with NULL infp");
	not_reached();
    }
	 ...
	 */
	    ...
	    if (ret <= 0) {
		errp(196, __func__, "fprintf error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
        ...
        if (need_confirm && !answer_yes) {
            ...
            ret = printf("\nThe abstract you entered is: %s\n",
                         abstract);
            if (ret <= 0) {
                errp(197, __func__, "fprintf abstract");
                not_reached();
            }
...
noprompt_yes_or_no(void)
{
    ...
    /*
     * use stdin unless reading answers from a file and not assuming yes
     */
    if (read_answers_flag_used && (!answer_yes && !force_yes)) {
        stream = stdin;
    } else {
	...
	/* firewall - input_stream cannot be NULL */
	if (input_stream == NULL) {
	    err(198, __func__, "read_answers_flag_used, answer_yes, and force_yes are false and input_stream is NULL");
	    not_reached();
	}
     ...
     */
    dbg(DBG_VVHIGH, "received a %zu byte response", len);
    if (len <= 0) {
...
get_author_info(struct author **author_set_p)
{
	...
	err(199, __func__, "called with NULL author_set_p");
	not_reached();
    }
	...
	if (ret != 1 || test_author_count(author_count) == false) {
	    ...
	    if (ret <= 0) {
		errp(200, __func__, "fprintf error #0 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "\nIf you happen to have more than %d authors, we ask that you pick\n", MAX_AUTHORS);
	    if (ret <= 0) {
		errp(201, __func__, "fprintf error #1 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "just %d authors and mention the remaining NUMBER of the authors in\nthe remarks file.\n",
                    MAX_AUTHORS);
	    if (ret <= 0) {
		errp(202, __func__, "fprintf error #2 while printing author number range");
                not_reached();
	    }
    ...
    if (author_set == NULL) {
	errp(203, __func__, "calloc a struct author array of length: %d failed", author_count);
	not_reached();
    }
	...
	if (ret < 0) {
	    errp(204, __func__, "puts error printing ISO 3166-1 URL0");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL1);
	if (ret < 0) {
	    errp(205, __func__, "puts error printing ISO 3166-1 URL1");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL2);
	if (ret < 0) {
	    errp(206, __func__, "puts error printing ISO 3166-1 URL2");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL3);
	if (ret < 0) {
	    errp(207, __func__, "puts error printing ISO 3166-1 URL3");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL4);
	if (ret < 0) {
	    errp(208, __func__, "puts error printing ISO 3166-1 URL4");
            not_reached();
	}
	para("",
	...
	if (ret <= 0) {
	    errp(209, __func__, "printf error printing author number");
            not_reached();
	}
	...
	do {
		...
		if (ret <= 0) {
		    errp(210, __func__, "fprintf error while reject name that is too long");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(211, __func__, "fprintf error while reject duplicate name");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(212, __func__, "fprintf while printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		...
		if (ret <= 0) {
		    errp(213, __func__, "fprintf while printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(214, __func__, "fprintf while printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(215, __func__, "fprintf while printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL0);
		if (ret <= 0) {
		    errp(216, __func__, "fprintf when printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		      "or from these Wikipedia / ISO web pages:",
		...
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL1);
		if (ret <= 0) {
		    errp(217, __func__, "fprintf when printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(218, __func__, "fprintf when printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(219, __func__, "fprintf when printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		if (ret <= 0) {
		    errp(220, __func__, "fprintf location/country code assignment");
                    not_reached();
		}
	...
	do {
		...
		if (ret <= 0) {
		    errp(221, __func__, "fprintf error while printing Email address length limit");
                    not_reached();
		}
		fpara(stderr,
	...
	do {
		...
		if (ret <= 0) {
		    errp(222, __func__, "fprintf error while printing URL length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret = fprintf(stderr, "\nSorry ( tm Canada :-) ), we limit URLs to %d characters\n\n", MAX_URL_LEN);
		if (ret <= 0) {
		    errp(223, __func__, "fprintf error while printing URL length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(224, __func__, "fprintf error while printing mastodon handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr,
			    ...
			    MAX_GITHUB_LEN);
		if (ret <= 0) {
		    errp(225, __func__, "fprintf error while printing GitHub user length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr, "\nSorry ( tm Canada :-) ), we limit affiliation names to %d characters\n\n",
			    MAX_AFFILIATION_LEN);
		if (ret <= 0) {
		    errp(226, __func__, "fprintf error while printing affiliation length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
	    ...
	    if (def_handle == NULL) {
		err(227, __func__, "default_handle() returned NULL!");
		not_reached();
	    }
		...
		if (ret <= 0) {
		    errp(228, __func__, "fprintf error while printing default IOCCC author handle");
                    not_reached();
		}
	    }
		...
		if (ret <= 0) {
		    errp(229, __func__, "fprintf error while printing IOCCC author handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(230, __func__, "fprintf error while printing duplicate author_handle error");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	if (printf("\nPlease verify the information about author #%d\n\n", i) <= 0 ||
	    ...
	    errp(231, __func__, "error while printing author #%d information\n", i);
	    not_reached();
	}
	if (need_confirm) {
...
verify_submission_dir(char const *submission_dir, char const *ls)
{
    ...
    ret = printf("    %s\n", submission_dir);
    if (ret <= 0) {
	errp(233, __func__, "printf error code: %d", ret);
        not_reached();
    }
    para("",
    ...
    if (exit_code != 0) {
	err(234, __func__, "cd -- %s && %s -lakR . failed with exit code: %d",
    ...
    if (ls_stream == NULL) {
	err(235, __func__, "popen filed for: cd -- %s && %s -lakR .", submission_dir, ls);
	not_reached();
    }
	...
	dbg(DBG_HIGH, "ls line read length: %zd buffer: %s", readline_len, linep);
        /*
	...
	err(236, __func__, "EOF while reading output of ls: %s", ls);
	not_reached();
    }
     ...
     */
    if (i == 0) {
        err(237, __func__, "found no k-block line in ls output");
        not_reached();
    }
    if (kdirsize <= 0) {
	err(238, __func__, "ls k-block value: %d <= 0", kdirsize);
	not_reached();
    }
...
form_info(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
        err(239, __func__, "passed NULL infop");
        not_reached();
    }
    ...
    ret = setenv("TZ", "UTC", 1);
    if (ret < 0) {
	errp(240, __func__, "cannot set TZ=UTC");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    timeptr = gmtime(&(infop->tstamp));
    if (timeptr == NULL) {
	errp(241, __func__, "gmtime returned NULL");
	not_reached();
    }
	...
	errp(242, __func__, "calloc of %zu bytes failed", utctime_len + 1);
	not_reached();
    }
    ...
    if (strftime_ret == 0) {
	errp(243, __func__, "strftime returned 0");
	not_reached();
    }
 ...
 *
 * given:
 *      wstat           -   pointer to struct walk_stat
		 ...
		 char const *chksubmit)
{
    ...
    struct item *p = NULL;      /* for each file in the dynamic array of the files list */
    int ret;			/* libc function return */
    int fd = -1;
    ...
    int exit_code;		/* exit code from shell_cmd() */
    size_t file_count = 0;	/* number of files */
    size_t extra_count = 0;	/* number of 'extra' files (see comments below) */
    int i;
    size_t j;
     ...
     * firewall
     */
    if (wstat == NULL || infop == NULL || authp == NULL || submission_dir == NULL || chksubmit == NULL) {
        err(244, __func__, "called with NULL arg(s)");
        not_reached();
    }

    if (wstat->file == NULL) {
        err(245, __func__, "called with NULL file list");
        not_reached();
    }
	...
	err(246, __func__, "author_count %d <= 0", authp->author_count);
	not_reached();
    } else if (authp->author_count > MAX_AUTHORS) {
	err(247, __func__, "author count %d > max authors %d", authp->author_count, MAX_AUTHORS);
	not_reached();
    }
    ...
    if (auth_path == NULL) {
	errp(248, __func__, "calloc of %zu bytes failed", auth_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(auth_path, auth_path_len, "%s/%s", submission_dir, AUTH_JSON_FILENAME);
    if (ret <= 0) {
	errp(249, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
    ...
    if (auth_stream == NULL) {
	errp(10, __func__, "failed to open for writing: %s", auth_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        err(11, __func__, "failed to obtain file descriptor for: %s", auth_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(12, __func__, "fprintf error writing leading part of authorship to %s", auth_path);
	not_reached();
    }
	...
	if (ret == false) {
	    errp(13, __func__, "fprintf error writing author %d info to %s", i, auth_path);
	    not_reached();
	}
    }
	...
	fprintf(auth_stream, "}\n") > 0;
    if (!ret) {
	errp(14, __func__, "fprintf error writing trailing part of authorship to %s", auth_path);
	not_reached();
    }
    ...
    if (ret != 0) {
        err(16, __func__, "chmod(2) failed to set user, group and other read-only on %s", auth_path);
        not_reached();
    }
    ...
    ret = close(fd);
    if (ret < 0) {
        errp(17, __func__, "close(fd) failed");
        not_reached();
    }
    ...
    if (info_path == NULL) {
	errp(18, __func__, "calloc of %zu bytes failed", info_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(info_path, info_path_len, "%s/%s", submission_dir, INFO_JSON_FILENAME);
    if (ret <= 0) {
	errp(19, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
    ...
    if (info_stream == NULL) {
	errp(20, __func__, "failed to open for writing: %s", info_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        errp(21, __func__, "failed to obtain file descriptor for: %s", info_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(22, __func__, "fprintf error writing leading part of info to %s", info_path);
	not_reached();
    }

    /*
     * note that we do want all the files even though we're only using it to
     * write extra files. This is because we have to skip the required files and
     * if we subtract by three it means that we'll skip some (and can result in
     * invalid JSON).
     */
    file_count = dyn_array_tell(wstat->file);

    /*
     * Set mark values
     *
     * NOTE: This has the side effect of validating that all items in wstat->file are non-NULL
     *	     and have non-NULL fts_name pointers.
     */
	 ...
	 */
	p = dyn_array_value(wstat->file, struct item *, j);

	/*
	 * firewall
	 */
	if (p == NULL) {
	    err(23, __func__, "found NULL pointer in files list, element: %zu", j);
	    not_reached();
	} else if (p->fts_name == NULL) {
	    err(24, __func__, "found NULL fts_name in files list, element: %zu", j);
	    not_reached();
	} else if (p->orig_name == NULL) {
	    err(25, __func__, "found NULL orig_name in files list, element: %zu", j);
	    not_reached();
	}
	 ...
	 */
	    ...
	    p->mark_ptr = "extra_file";
	    ++extra_count;
	}
    }

    /*
     * add .info.json and .auth.json
     *
     * These files were NOT part of the tree that was walked.  We are adding these files NOW
     * in this function so we add them into
     */
    ret =                    fprintf(info_stream, "        {\n") > 0 &&
	  ...
	  json_fprintf_value_string(info_stream,  "            ", "auth_JSON", " : ", AUTH_JSON_FILENAME, "\n") &&
			     fprintf(info_stream, "        },\n") > 0;
    if (!ret) {
	errp(26, __func__, "fprintf error writing mandatory filename to %s", info_path);
	not_reached();
    }
     ...
     */
    dbg(DBG_MED, "file count + 2: %zd", file_count+2);  /* account for .info.json and .auth.json added above */
    dbg(DBG_MED, "extra file count: %zd", extra_count);
    for (j = 0; j < file_count; ++j) {
	...
	ret =                   fprintf(info_stream, "        {\n") > 0 &&
	      json_fprintf_value_string(info_stream, "            ", (char *)p->mark_ptr, " : ", p->orig_path, "\n") &&
				fprintf(info_stream, "        }%s\n", ((j+1) < file_count) ?  "," : "") > 0;
	if (!ret) {
	    errp(27, __func__, "fprintf error writing extra filename[%zu] to %s", j, info_path);
	    not_reached();
	}
    }
	...
	fprintf(info_stream, "}\n") > 0;
    if (!ret) {
	errp(28, __func__, "fprintf error writing trailing part of info to %s", info_path);
	not_reached();
    }
    ...
    ret = fchmod(fd, S_IRUSR | S_IRGRP | S_IROTH);
    if (ret != 0) {
        err(30, __func__, "chmod(2) failed to set user, group and other read-only on %s", info_path);
        not_reached();
    }
    ...
    dbg(DBG_HIGH, "about to perform: %s -- %s", chksubmit, submission_dir);
    exit_code = shell_cmd(__func__, false, true, "% -- %", chksubmit, submission_dir);
    if (exit_code != 0) {
	err(4, __func__, "%s -- %s failed with exit code: %d", /*ooo*/
...
form_auth(struct auth *authp, struct info *infop, int author_count, struct author *authorp)
{
	...
	err(32, __func__, "infop->ioccc_id is NULL");
	not_reached();
    }
    if (infop->tarball == NULL) {
	err(33, __func__, "infop->tarball is NULL");
	not_reached();
    }
    if (infop->utctime == NULL) {
	err(34, __func__, "infop->utctime is NULL");
	not_reached();
    }
	...
	errp(35, __func__, "strdup() ioccc_id path %s failed", infop->ioccc_id);
	not_reached();
    }
	...
	errp(36, __func__, "strdup() tarball path %s failed", infop->tarball);
	not_reached();
    }
	...
	errp(37, __func__, "strdup() utctime path %s failed", infop->utctime);
	not_reached();
    }
    return;
}
	     ...
	     char const *ls, char const *txzchk, char const *fnamchk, bool test_mode)
{
    ...
    ret = chdir(workdir);
    if (ret < 0) {
	errp(40, __func__, "cannot cd %s", workdir);
	not_reached();
    }
    ...
    if (exit_code != 0) {
	err(41, __func__, "%s --format=v7 -cJf %s -- %s failed with exit code: %d",
    ...
    if (ret != 0) {
	errp(42, __func__, "stat of the compressed tarball failed: %s", basename_tarball_path);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(43, __func__, "The compressed tarball: %s size: %lld > %d",
		 basename_tarball_path, (long long)buf.st_size, MAX_TARBALL_LEN);
	not_reached();
    }
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
	errp(44, __func__, "cannot fchdir to the previous current directory");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = close(cwd);
    if (ret < 0) {
	errp(45, __func__, "close of previous current directory failed");
	not_reached();
    }
    ...
    if (entertain) {
        ...
        if (exit_code != 0) {
            if (test_mode) {
                err(46, __func__, "%s -x -e -f %ju -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                               txzchk, feathery, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            } else {
                err(47, __func__, "%s -e -f %ju -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                               txzchk, feathery, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            }
            not_reached();
        }
        ...
        if (exit_code != 0) {
            if (test_mode) {
                err(48, __func__, "%s -x -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                   txzchk, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            } else {
                err(49, __func__, "%s -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                   txzchk, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            }
            not_reached();
        }
	    ...
	    bool test_mode, unsigned int slot_number)
{
	...
	err(51, __func__, "failed to cmdprintf: submission_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(52, __func__, "printf #0 error");
	not_reached();
    }
    ...
    if (workdir_esc == NULL) {
	err(53, __func__, "failed to cmdprintf: workdir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(54, __func__, "printf #2 error");
	not_reached();
    }
    free(workdir_esc);
...
show_registration_url(void)
{
    ...
    ret = printf("    %s\n", IOCCC_REGISTER_URL);
    if (ret <= 0) {
	errp(55, __func__, "printf error printing IOCCC_REGISTER_URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(56, __func__, "printf error printing IOCCC register FAQ URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(57, __func__, "printf error printing IOCCC_REGISTER_INFO_URL, IOCCC_PW_CHANGE_INFO_URL and IOCCC_SUBMIT_INFO_URL");
	not_reached();
    }
    ...
    ret = printf("    %s\n", IOCCC_STATUS_URL);
    if (ret < 0) {
	errp(58, __func__, "printf error printing IOCCC status URL");
	not_reached();
    }
    return;
}
 ...
 * show_submit_url
 *
 * Tell the user where they may submit their submission if the contest is still open.
 *
 * given:
...
show_submit_url(char const *workdir, char const *tarball_path, unsigned int slot_number)
{
    ...
    if (ret <= 0) {
	errp(59, __func__, "printf error printing tarball path and slot number");
	not_reached();
    }
    para("",
    ...
    ret = printf("    %s\n", IOCCC_SUBMIT_URL);
    if (ret < 0) {
	errp(60, __func__, "printf error printing IOCCC submit URL");
	not_reached();
    }
    /**/
    para("",
     ...
     ret = printf("    %s\n", IOCCC_ENTER_FAQ_URL);
    if (ret < 0) {
	errp(61, __func__, "printf error printing IOCCC enter FAQ URL");
	not_reached();
    }
    return;
}
 ...
 *
 * NOTE: if a file does not exist (i.e. file_mode_size() returns false) then we
 * ignore it.
 */
static void
read_manifest(struct walk_stat *wstat)
{
    ssize_t readline_len = -1;
    char *line = NULL;
    bool process = false;
    off_t st_size = 0; /* size of each file */
    mode_t st_mode = 0; /* mode of each file */
     ...
     */
    if (manifest == NULL) {
        err(3, __func__, "manifest not open"); /*ooo*/
        not_reached();
    }
    if (wstat == NULL) {
        err(3, __func__, "wstat is NULL"); /*ooo*/
        not_reached();
    }
	 ...
	 */
        if (file_mode_size(cpath, &st_mode, &st_size)) {
            ...
            if (dup) {
                ...
                dbg(DBG_MED, "file %s has been successfully processed", cpath);
            }
        } else {
...
/*
 * read_ignore - read in a ignore file, adding to infop->ignore_paths
 *
 * given:
 *      ignore      - path to FILE * to read
 *      wstat       - pointer to walk_stat struct
 *
 * This function will not return on NULL pointers or errors.
 */
static void
read_ignore(char const *ignore, struct walk_stat *wstat)
{
    FILE *ignorefp = NULL;
    ssize_t readline_len = -1;
    char *line = NULL;
    ...
    if (ignore == NULL || !*ignore) {
        err(3, __func__, "ignore path NULL or empty"); /*ooo*/
        not_reached();
    }
    if (wstat == NULL) {
        err(62, __func__, "wstat is NULL");
        not_reached();
    }

    errno = 0; /* pre-clear errno for errp() */
    ignorefp = fopen(ignore, "r");
    if (ignorefp == NULL) {
        err(3, __func__, "ignore file not found or not readable"); /*ooo*/
        not_reached();
    }
	 ...
	 */
        ...
        if (skip_add_ret) {
            ...
            dbg(DBG_MED, "path: %s is already marked for skipping", line);
        }
