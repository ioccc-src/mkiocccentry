...
main(int argc, char *argv[])
{
    ...
    if (read_answers_flag_used && answersp != NULL && answersp != stdin)
    {
        ...
        if (answersfd < 0) {
            errp(11, __func__, "cannot obtain answers FD");
            not_reached();
        }
        /*
	...
	errp(12, __func__, "strdup() tarball path %s failed", tarball_path);
	not_reached();
    }
	...
	if (answersp == NULL) {
	    errp(13, __func__, "cannot create answers file: %s", answers);
	    not_reached();
	}
        errno = 0; /* pre-clear errno for errp() */
        ret = fprintf(answersp, "%s\n", MKIOCCCENTRY_ANSWERS_VERSION);
	if (ret <= 0) {
	    errp(14, __func__, "fprintf error printing header to the answers file");
            not_reached();
	}
    }
	...
	errno = 0;			/* pre-clear errno for errp() */
        ret = fprintf(answersp, "%s\n", info.ioccc_id);
	if (ret <= 0) {
	    errp(15, __func__, "fprintf error printing IOCCC contest id to the answers file");
            not_reached();
	}
	errno = 0;			/* pre-clear errno for errp() */
	ret = fprintf(answersp, "%d\n", info.submit_slot);
	if (ret <= 0) {
	    errp(16, __func__, "fprintf error printing submit slot number to the answers file");
            not_reached();
	}
    }
	...
	ret = fprintf(answersp, "%s\n", info.title);
	if (ret <= 0) {
	    errp(17, __func__, "fprintf error printing title to the answers file");
            not_reached();
	}
    }
	...
	ret = fprintf(answersp, "%s\n", info.abstract);
	if (ret <= 0) {
	    errp(18, __func__, "fprintf error printing abstract to the answers file");
            not_reached();
	}
    }
    ...
    */
	...
	errno = 0;			/* pre-clear errno for errp() */
        ret = fprintf(answersp, "%d\n", author_count);
	if (ret <= 0) {
	    errp(19, __func__, "fprintf error printing IOCCC author count to the answers file");
            not_reached();
	}
	...
	for (i = 0; i < author_count; i++) {
	    ...
	    if (ret <= 0) {
		errp(20, __func__, "fprintf error printing author info to the answers file");
                not_reached();
	    }
	}
    }
	...
	if (read_answers_flag_used) {
	    ...
	    if (error) {
	        errp(21, __func__, "expected ANSWERS_EOF marker at the end of the answers file");
	        not_reached();
	    }
	    input_stream = stdin;
	    if (line != NULL) {
	    ...
	    if (ret <= 0) {
	        errp(22, __func__, "fprintf error writing ANSWERS_EOF marker to the answers file");
                not_reached();
	    }
	}
	if (answers != NULL) {
	    ...
	    ret = fclose(answersp);
	    if (ret != 0) {
	        errp(23, __func__, "error in fclose to the answers file");
                not_reached();
	    }
	}
	answersp = NULL;
    ...
    if (answersfd > 0 && read_answers_flag_used) {
        ...
        if (close(answersfd) != 0) {
            errp(24, __func__, "failed to close(answers)");
            not_reached();
        }
    }
...
append_unique_filename(struct dyn_array *array, char *str, bool fn)
{
     ...
     */
    if (array == NULL) {
	err(25, __func__, "array is NULL");
	not_reached();
    }
    if (str == NULL) {
	err(26, __func__, "str is NULL");
	not_reached();
    }
    ...
    switch (ent->fts_info) {
        case FTS_DC: /* cycle in directory tree */
            ...
            if (fts_set(fts, ent, FTS_SKIP) != 0) {
                errp(28, __func__, "failed to set FTS_SKIP on a directory that causes a cycle in the tree: %s",
                        ent->fts_path);
                not_reached();
            } else {
        ...
        case FTS_DNR: /* directory not readable */
            errno = 0; /* pre-clear errno for errp() */
            if (fts_set(fts, ent, FTS_SKIP) != 0) {
                errp(29, __func__, "failed to set FTS_SKIP on an unreadable directory the tree: %s", ent->fts_path);
                not_reached();
            } else {
        ...
        case FTS_NS: /* no stat(2) info available but we requested it */
            err(30, __func__, "no stat(2) info available for %s in tree", ent->fts_path);
            not_reached();
            break;
        case FTS_NSOK: /* stat(2) was not requested */
            err(31, __func__, "stat(2) not requested: FTS_NOSTAT set");
            not_reached();
            break;
        case FTS_ERR: /* some error condition */
            ...
            errp(32, __func__, "encountered error from path %s in tree", ent->fts_path);
            not_reached();
            break;
        case FTS_DEFAULT: /* some other file type */
            err(33, __func__, "found invalid file type: %s", ent->fts_path);
            not_reached();
            break;
        case FTS_DOT: /* a filename '.' or '..' not requested in fts_open() */
             ...
             * just './'!
             */
            err(34, __func__, "found '.' or '..' not specified: %s", ent->fts_path);
            not_reached();
            break;
        default: /* okay */
            /*
...
scan_topdir(char *args, struct info *infop, char const *make, char const *submission_dir, RuleCount *size)
{
    ...
    if (args == NULL || infop == NULL || make == NULL || submission_dir == NULL || size == NULL) {
        err(35, __func__, "passed NULL pointer(s)");
        not_reached();
    }
    ...
    if (infop->required_files == NULL) {
        err(36, __func__, "failed to create required files list array");
        not_reached();
    }
    /*
    ...
    if (infop->extra_files == NULL) {
        err(37, __func__, "failed to create files list array");
        not_reached();
    }
    /*
    ...
    if (infop->directories == NULL) {
        err(38, __func__, "failed to create directories list array");
        not_reached();
    }
    /*
    ...
    if (infop->ignored_dirs == NULL) {
        err(39, __func__, "failed to create ignored directories list array");
        not_reached();
    }
    /*
    ...
    if (infop->forbidden_files == NULL) {
        err(40, __func__, "failed to create forbidden files list array");
        not_reached();
    }
    /*
    ...
    if (infop->unsafe_files == NULL) {
        err(41, __func__, "failed to create unsafe filenames list array");
        not_reached();
    }
    /*
    ...
    if (infop->unsafe_dirs == NULL) {
        err(42, __func__, "failed to create unsafe directories list array");
        not_reached();
    }
    /*
    ...
    if (infop->ignored_symlinks == NULL) {
        err(43, __func__, "failed to create ignored symlinks list array");
        not_reached();
    }
    /*
    ...
    if (chdir(submission_dir) != 0) {
        errp(45, __func__, "unable to change to submission directory");
        not_reached();
    }
    ...
    if (submit_path == NULL) {
        errp(46, __func__, "couldn't get absolute path of submission directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(47, __func__, "couldn't change back to original original directory");
        not_reached();
    }
    ...
    errno = 0;      /* pre-clear errno for errp() */
    if (chdir(args) != 0) {
        errp(48, __func__, "chdir(\"%s\") failed", args);
        not_reached();
    }
    /*
    ...
    if (topdir < 0) {
	errp(49, __func__, "cannot open topdir");
	not_reached();
    }
    ...
    if (topdir_path == NULL) {
        errp(50, __func__, "couldn't get absolute path of topdir");
        not_reached();
    }
    ...
    if (ent == NULL){
        err(51, __func__, "failed to find any files in \".\"");
        not_reached();
    } else {
        do {
            ...
            if (!strcmp(ent->fts_path, ".") || !strcmp(ent->fts_path, "..") || !strcmp(ent->fts_path, "./")) {
                ...
                if (ent->fts_info == FTS_D) {
                    ...
                    if (fts_set(fts.tree, ent, FTS_SKIP) != 0) {
                        errp(52, __func__, "fts_set() failed to set FTS_SKIP for %s", ent->fts_path + 2);
                        not_reached();
                    }
                }
                continue;
                ...
                case FTS_D: /* directory */
                    ...
                    */
                    if (ent->fts_statp->st_ino == workdir_st.st_ino && ent->fts_statp->st_dev == workdir_st.st_dev) {
                        ...
                        errno = 0; /* pre-clear errno for errp() */
                        if (fts_set(fts.tree, ent, FTS_SKIP) != 0) {
                            errp(60, __func__, "failed to set FTS_SKIP on workdir inside topdir");
                            not_reached();
                        }
        ...
        char *submit_path, int topdir, int cwd, RuleCount *size)
{
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
        errp(64, __func__, "unable to fchdir(cwd)");
        not_reached();
    }
    ...
    if (infop->ignore_paths != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(65, __func__, "found NULL pointer in ignored dirname list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
        ...
        if (len > 0) {
            if (need_hints) {
                ...
                for (i = 0; ignored_dirnames[i] != NULL; ++i) {
                    ...
                    if (ret <= 0) {
                        errp(66, __func__, "printf error printing an ignored dirname: %s", ignored_dirnames[i]);
                        not_reached();
                    }
                }
                para("",
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(67, __func__, "found NULL pointer in ignored dirname list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
    ...
    if (infop->unsafe_dirs != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(68, __func__, "found NULL pointer in unsafe directory names list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
        ...
        if (len > 0) {
            if (need_hints) {
                ...
                for (i = 0; forbidden_filenames[i] != NULL; ++i) {
                    ...
                    if (ret <= 0) {
                        errp(69, __func__, "printf error printing a forbidden filename: %s", forbidden_filenames[i]);
                        not_reached();
                    }
                }
                para("",
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(70, __func__, "found NULL pointer in forbidden files list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
    ...
    if (infop->unsafe_files) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(71, __func__, "found NULL pointer in unsafe filenames list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
    ...
    if (infop->ignored_symlinks != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(72, __func__, "found NULL pointer in ignored symlinks list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
    ...
    if (infop->directories != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(73, __func__, "found NULL pointer in directories list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
    ...
    if (infop->required_files == NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(74, __func__, "found NULL pointer in required files list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
            ...
            if (len > 0) {
                for (i = 0; i < len; ++i) {
                    ...
                    if (p == NULL) {
                        err(75, __func__, "found NULL pointer in extra files list, element: %jd", i);
                        not_reached();
                    }
                    print("%s\n", p);
                }
            ...
            if (len > 0) {
                ...
                for (i = 0; i < len; ++i) {
                    ...
                    p = dyn_array_value(infop->directories, char *, i);
                    if (p == NULL) {
                        err(76, __func__, "found NULL pointer in infop->directories list");
                        not_reached();
                    }
                    /*
            ...
            errno = 0;
            if (fchdir(topdir) != 0) {
                errp(77, __func__, "cannot change to topdir");
                not_reached();
            }
            ...
            len = dyn_array_tell(infop->required_files);
            if (len <= 0) {
                err(78, __func__, "list of required files is empty");
                not_reached();
            }
            for (i = 0; i < len; ++i) {
                p = dyn_array_value(infop->required_files, char *, i);
                if (p == NULL) {
                    err(79, __func__, "found NULL pointer in required files list, element: %jd", i);
                    not_reached();
                }
                /*
            ...
            for (i = 0; i < len; ++i) {
                p = dyn_array_value(infop->extra_files, char *, i);
                if (p == NULL) {
                    err(83, __func__, "found NULL pointer in non-required files list, element: %jd", i);
                    not_reached();
                }
                /*
                 * we have to allocate the full path of the file under topdir.
                ...
                fname = calloc_path(topdir_path, p);
                if (fname == NULL) {
                    err(84, __func__, "couldn't allocate path to copy");
                    not_reached();
                }
                if (target_path != NULL) {
                    free(target_path);
                ...
                target_path = calloc(1, strlen(submit_path) + LITLEN("/") + strlen(p) + 1);
                if (target_path == NULL) {
                    errp(85, __func__, "failed to allocate target path for %s", p);
                    not_reached();
                }
                /*
                 */
                ...
                ret = snprintf(target_path, strlen(submit_path) + 1 + strlen(p) + 1, "%s/%s", submit_path, p);
                if (ret <= 0) {
                    errp(86, __func__, "snprintf to form target path for %s failed", fname);
                    not_reached();
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (close(topdir) != 0) {
        errp(87, __func__, "failed to close(topdir)");
        not_reached();
    }
        ...
        char const *make, RuleCount *size, int cwd)
{
    ...
    if (infop == NULL || submit_path == NULL || make == NULL || size == NULL) {
        err(88, __func__, "passed NULL arg(s)");
        not_reached();
    }
    /*
     * cwd must be >= 0
     */
    if (cwd < 0) {
        err(89, __func__, "original directory file descriptor < 0");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(90, __func__, "failed to change to original directory");
        not_reached();
    }
    ...
    if (required_files == NULL) {
        err(91, __func__, "couldn't create required files list array");
        not_reached();
    }
    extra_files = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (extra_files == NULL) {
        err(92, __func__, "couldn't create extra files list array");
        not_reached();
    }
    missing_files = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (missing_files == NULL) {
        err(93, __func__, "couldn't create missing files list array");
        not_reached();
    }
    directories = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (directories == NULL) {
        err(94, __func__, "couldn't create directories list array");
        not_reached();
    }
    missing_dirs = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (missing_dirs == NULL) {
        err(95, __func__, "couldn't create missing directories list array");
        not_reached();
    }
    ...
    ent = read_fts(NULL, -1, NULL, &fts);
    if (ent == NULL){
        err(96, __func__, "failed to find any files in \".\"");
        not_reached();
    } else {
        do {
            bool ignored_dirname = false;
            ...
            if (!strcmp(ent->fts_path, ".") || !strcmp(ent->fts_path, "..") || !strcmp(ent->fts_path, "./")) {
                ...
                if (ent->fts_info == FTS_D) {
                    ...
                    errno = 0;  /* pre-clear errno for errp() */
                    if (fts_set(fts.tree, ent, FTS_SKIP) != 0) {
                        errp(97, __func__, "fts_set() failed to set FTS_SKIP for %s", ent->fts_path + 2);
                        not_reached();
                    }
                }
                continue;
	    ...
	    (void) canon_path(ent->fts_path, MAX_PATH_LEN, MAX_FILENAME_LEN, MAX_PATH_DEPTH,
            ...
            switch (ent->fts_info) {
                case FTS_D: /* directory */
                    ...
                    filename = strdup(ent->fts_path + 2);
                    if (filename == NULL) {
                        errp(98, __func__, "strdup(\"%s\") failed", ent->fts_path + 2);
                        not_reached();
                    }
                    /*
                ...
                case FTS_F:
                    ...
                    if (infop->ignore_paths != NULL && array_has_path(infop->ignore_paths, ent->fts_path + 2, false, true,
                                NULL)) {
                        err(99, __func__, "found file in submission directory that was ignored: %s",
                                ent->fts_path + 2); /*ooo*/
                        not_reached();
                    }
                    ...
                    if (infop->manifest_paths != NULL && !array_has_path(infop->manifest_paths, ent->fts_path + 2, false, true,
                                NULL)) {
                        err(100, __func__, "found file in submission directory that is not in the manifest: %s",
                                ent->fts_path + 2); /*ooo*/
                        not_reached();
                    }
    ...
    for (i = 0; i < len; ++i) {
        ...
        p = dyn_array_value(infop->required_files, char *, i);
        if (p == NULL) {
            err(102, __func__, "found NULL pointer in required files list, element: %jd", i);
            not_reached();
        }
        /*
        ...
        if (fname == NULL) {
            err(103, __func__, "found NULL pointer in required files list in submission directory, element: %jd", i);
            not_reached();
        }
        ...
        if (strcmp(p, fname) != 0 || !array_has_path(required_files, p, false, false, NULL)) {
            ...
            filename = strdup(p);
            if (filename == NULL) {
                errp(104, __func__, "strdup(\"%s\") failed", p);
                not_reached();
            }
        ...
        if (len > 0) {
            ...
            if (len > 0) {
                for (i = 0; i < len; ++i) {
                    ...
                    p = dyn_array_value(infop->extra_files, char *, i);
                    if (p == NULL) {
                        err(105, __func__, "found NULL pointer in extra files list, element: %jd", i);
                        not_reached();
                    }
                    /*
                    ...
                    if (p == NULL) {
                        err(106, __func__, "found NULL pointer in extra files list in submission directory, element: %jd", i);
                        not_reached();
                    }
    ...
    if (len > 0) {
        ...
        if (ret <= 0) {
            errp(108, __func__, "error writing missing files list title");
            not_reached();
        }
        for (i = 0; i < len; i++) {
            /*
            ...
            if (fname == NULL) {
                err(109, __func__, "found NULL pointer in missing files list in submission directory, element: %jd", i);
                not_reached();
            }
            print("%s\n", fname);
        }
    ...
    if (infop->directories != NULL) {
         ...
         */
        if (len != len2) {
            err(110, __func__, "size of directories list in submission directory != size in topdir: %zu != %zu",
		    len2, len);
            not_reached();
        }
        if (len > 0) {
            for (i = 0; i < len; ++i) {
                p = dyn_array_value(infop->directories, char *, i);
                if (p == NULL) {
                    err(111, __func__, "found NULL pointer in directories list, element: %jd", i);
                    not_reached();
                }
                fname = dyn_array_value(directories, char *, i);
                if (fname == NULL) {
                    err(112, __func__, "found NULL pointer in directories list in submission directory, element: %ju",
                            (uintmax_t)i);
                    not_reached();
                }
                ...
                if (strcmp(fname, p) != 0 || !array_has_path(directories, p, false, false, NULL)) {
                    ...
                    filename = strdup(p);
                    if (filename == NULL) {
                        errp(113, __func__, "strdup(\"%s\") failed", p);
                        not_reached();
                    }
    ...
    if (missing_dirs != NULL) {
        ...
        if (len > 0) {
            ...
            if (ret <= 0) {
                errp(114, __func__, "error writing missing directories list title");
                not_reached();
            }
            for (i = 0; i < len; i++) {
                /*
    ...
    if (directories != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(116, __func__, "found NULL pointer in directories list, element: %jd", i);
                    not_reached();
                }
                print("%s\n", p);
            }
     ...
     */
    if (required_files == NULL) {
        err(117, __func__, "required files in submission directory is NULL");
        not_reached();
    }
    ...
    if (len > 0) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(118, __func__, "found NULL pointer in required files list, element: %jd", i);
                not_reached();
            }
            print("%s\n", p);
        }
        ...
        if (extra_files != NULL) {
            ...
            if (len > 0) {
                for (i = 0; i < len; ++i) {
                    ...
                    if (p == NULL) {
                        err(119, __func__, "found NULL pointer in non-required files list, element: %jd", i);
                        not_reached();
                    }
                    print("%s\n", p);
                }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(120, __func__, "unable to change to previous directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (close(cwd) != 0) {
        errp(121, __func__, "failed to close(cwd)");
        not_reached();
    }
    /*
        ...
        char *chksubmit, char *make, char *rm)
{
	...
	fpara(stderr,
	      ...
	      NULL);
        if (tar != NULL) {
	    err(123, __func__, "tar does not exist: %s", tar);
        } else {
	    err(124, __func__, "tar does not exist");
        }
	not_reached();
    }
    if (!is_file(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(125, __func__, "tar is not a regular file: %s", tar);
	not_reached();
    }
    if (!is_exec(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(126, __func__, "tar is not an executable program: %s", tar);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (ls != NULL) {
            err(128, __func__, "ls does not exist: %s", ls);
        } else {
            err(129, __func__, "ls does not exist");
        }
	not_reached();
    }
    if (!is_file(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(130, __func__, "ls is not a regular file: %s", ls);
	not_reached();
    }
    if (!is_exec(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(131, __func__, "ls is not an executable program: %s", ls);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (txzchk != NULL) {
	    err(132, __func__, "txzchk does not exist: %s", txzchk);
        } else {
	    err(133, __func__, "txzchk does not exist");
        }
	not_reached();
    }
    if (!is_file(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(134, __func__, "txzchk is not a regular file: %s", txzchk);
	not_reached();
    }
    if (!is_exec(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(135, __func__, "txzchk is not an executable program: %s", txzchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (fnamchk != NULL) {
	    err(136, __func__, "fnamchk does not exist: %s", fnamchk);
        } else {
	    err(137, __func__, "fnamchk does not exist");
        }
	not_reached();
    }
    if (!is_file(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(138, __func__, "fnamchk is not a regular file: %s", fnamchk);
	not_reached();
    }
    if (!is_exec(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(139, __func__, "fnamchk is not an executable program: %s", fnamchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (make != NULL) {
	    err(144, __func__, "make does not exist: %s", make);
        } else {
	    err(145, __func__, "make does not exist");
        }
	not_reached();
    }
    if (!is_file(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(146, __func__, "make is not a regular file: %s", make);
	not_reached();
    }
    if (!is_exec(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(147, __func__, "make is not an executable program: %s", make);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
        if (rm != NULL) {
	    err(148, __func__, "rm does not exist: %s", rm);
        } else {
	    err(149, __func__, "rm does not exist");
        }
	not_reached();
    }
    if (!is_file(rm)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(150, __func__, "rm is not a regular file: %s", rm);
	not_reached();
    }
    if (!is_exec(rm)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(151, __func__, "rm is not an executable program: %s", rm);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(152, __func__, "workdir does not exist: %s", workdir);
	not_reached();
    }
    if (!is_dir(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(153, __func__, "workdir is not a directory: %s", workdir);
	not_reached();
    }
    if (!is_write(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(154, __func__, "workdir is not a writable directory: %s", workdir);
	not_reached();
    }
...
prompt(char const *str, size_t *lenp)
{
     ...
     */
    if (str == NULL) {
	err(155, __func__, "called with NULL str");
	not_reached();
    }
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(156, __func__, "error printing prompt string");
		not_reached();
	    } else if (feof(stdout)) {
		err(157, __func__, "EOF while printing prompt string");
		not_reached();
	    } else {
		errp(158, __func__, "unexpected fputs error printing prompt string");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(159, __func__, "error printing :<space>");
		not_reached();
	    } else if (feof(stdout)) {
		err(160, __func__, "EOF while writing :<space>");
		not_reached();
	    } else {
		errp(161, __func__, "unexpected fputs error printing :<space>");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(162, __func__, "error flushing prompt to stdout");
		not_reached();
	    } else if (feof(stdout)) {
		err(163, __func__, "EOF while flushing prompt to stdout");
		not_reached();
	    } else {
		errp(164, __func__, "unexpected fflush error while flushing prompt to stdout");
		not_reached();
	    }
	}
    }
    ...
    if (buf == NULL) {
	err(165, __func__, "EOF while reading prompt input");
	not_reached();
    }
...
get_contest_id(bool *testp, char const *uuidfile, char *uuidstr)
{
     ...
     */
    if (testp == NULL) {
	err(166, __func__, "called with NULL testp");
	not_reached();
    }
...
get_submit_slot(struct info *infop)
{
    ...
    if (need_hints) {
        ...
        if (ret <= 0) {
            errp(169, __func__, "printf error printing number of submissions allowed");
            not_reached();
        }
        para("",
	...
	if (ret != 1 || submit_slot < 0 || submit_slot > MAX_SUBMIT_SLOT) {
	    ...
	    ret = fprintf(stderr, "\nThe submit slot number must be a number from 0 through %d; please re-enter.\n",
		    MAX_SUBMIT_SLOT);
	    if (ret <= 0) {
		errp(170, __func__, "fprintf error while informing about the valid submit slot number range");
                not_reached();
	    }
            /*
             * free storage
	    ...
	    submit_slot = -1;	/* invalidate input */
            ...
            if (need_confirm && !answer_yes) {
                ...
                ret = printf("\nThe slot number you entered is: %d\n",
                             submit_slot);
                if (ret <= 0) {
                    errp(171, __func__, "fprintf error writing slot number");
                    not_reached();
                }
             ...
             char const *rm)
{
    ...
    if (test == false) {
	err(173, __func__, "submit slot number: %d must >= 0 and <= %d", submit_slot, MAX_SUBMIT_SLOT);
	not_reached();
    }
    ...
    if (submission_dir == NULL) {
	errp(174, __func__, "calloc #0 of %zu bytes failed", (submission_dir_len + 1));
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(submission_dir, submission_dir_len + 1, "%s/%s-%d", workdir, ioccc_id, submit_slot);
    if (ret <= 0) {
	errp(175, __func__, "snprintf to form submission directory failed");
	not_reached();
    }
    ...
    if (exists(submission_dir)) {
        if (!force_remove) {
            ...
            if (ret <= 0) {
                errp(176, __func__, "fprintf error while informing that the submission directory already exists");
                not_reached();
            }
            fpara(stderr,
                  "",
                  ...
                  "",
                  NULL);
            err(177, __func__, "submission directory exists: %s", submission_dir);
            not_reached();
        } else {
            /*
            ...
            if (exit_code != 0) {
                err(178, __func__, "%s -rf %s failed with exit code: %d",
    ...
    if (ret < 0) {
	errp(179, __func__, "cannot mkdir %s", submission_dir);
	not_reached();
    }
    errno = 0; /* pre-clear errno for errp() */
    ret = chmod(submission_dir, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    if (ret < 0) {
        errp(180, __func__, "cannot chmod directory %s to mode 0755", submission_dir);
        not_reached();
    }
    ...
    *tarball_path = form_tar_filename(ioccc_id, submit_slot, test_mode, tstamp);
    if (*tarball_path == NULL) {
	errp(181, __func__, "failed to form compressed tarball path");
	not_reached();
    }
...
warn_rule_2a_size(struct info *infop, int mode, RuleCount size)
{
     ...
     */
    if (infop == NULL) {
	err(183, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(184, __func__, "fprintf error when printing prog.c Rule 2a warning");
            not_reached();
	}
	...
	if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes) || (read_answers_flag_used && !force_yes)) {
	    ...
	    if (ret <= 0) {
		errp(186, __func__, "fprintf error when printing prog.c file size and Rule 2a mismatch");
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	    ...
	    if (!yorn) {
		err(187, __func__, "please fix your prog.c file");
		not_reached();
	    }
	...
	err(188, __func__, "invalid mode passed to function: %d", mode);
	not_reached();
    }
    return;
}
...
warn_trigraph(void)
{
	...
	if (ret <= 0) {
	    errp(189, __func__, "fprintf error when printing prog.c trigraph_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
warn_ungetc(void)
{
	...
	ret = fprintf(stderr, "\nprog.c triggered an ungetc error: @SirWumpus goofed\n"
			      ...
			      "YOUR remarks.md FILE!\n\n");
	if (ret <= 0) {
	    errp(191, __func__, "fprintf error when printing prog.c ungetc_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
warn_rule_2b_size(struct info *infop)
{
	...
	if (ret <= 0) {
	    errp(194, __func__, "printf error printing prog.c size > Rule 2b maximum");
	    not_reached();
	}
...
check_prog_c(struct info *infop, char const *prog_c)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(197, __func__, "prog.c does not exist: %s", prog_c);
	not_reached();
    }
    if (!is_file(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(198, __func__, "prog.c is not a regular file: %s", prog_c);
	not_reached();
    }
    if (!is_read(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(199, __func__, "prog.c is not a readable file: %s", prog_c);
	not_reached();
    }
    ...
    if (prog_stream == NULL) {
	errp(200, __func__, "failed to fopen: %s", prog_c);
	not_reached();
    }
    ...
    if (ret != 0) {
	errp(201, __func__, "failed to fclose: %s", prog_c);
	not_reached();
    }
...
inspect_Makefile(char const *Makefile, struct info *infop)
{
    ...
    if (stream == NULL) {
	errp(203, __func__, "cannot open Makefile: %s", Makefile);
	not_reached();
    }
...
warn_Makefile(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(205, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (!answer_yes && !force_yes) {
	    ...
	    if (!yorn) {
		err(206, __func__, "Use a different Makefile or modify your Makefile");
		not_reached();
	    }
	}
    }
...
check_Makefile(struct info *infop, char const *Makefile)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(208, __func__, "Makefile does not exist: %s", Makefile);
	not_reached();
    }
    if (!is_file(Makefile)) {
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(209, __func__, "Makefile is not a regular file: %s", Makefile);
	not_reached();
    }
    if (!is_read(Makefile)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(210, __func__, "Makefile is not readable file: %s", Makefile);
	not_reached();
    }
    filesize = file_size(Makefile);
    if (filesize < 0) {
	err(211, __func__, "file_size error: %lld on Makefile  %s", (long long)filesize, Makefile);
	not_reached();
    } else if (filesize == 0) {
	err(212, __func__, "Makefile cannot be empty: %s", Makefile);
	not_reached();
    }
...
check_remarks_md(struct info *infop, char const *remarks_md)
{
	...
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(214, __func__, "remarks.md does not exist: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr, "",
	      ...
	      "",
	      NULL);
	err(215, __func__, "remarks.md is not a regular file: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(216, __func__, "remarks.md is not readable file: %s", remarks_md);
	not_reached();
    }
    filesize = file_size(remarks_md);
    if (filesize < 0) {
	err(217, __func__, "file_size error: %lld on remarks.md %s", (long long)filesize, remarks_md);
	not_reached();
    } else if (filesize == 0) {
	err(218, __func__, "remarks.md cannot be empty: %s", remarks_md);
	not_reached();
    }
...
yes_or_no(char const *question, bool def_answer)
{
     ...
     */
    if (question == NULL) {
	err(219, __func__, "called with NULL question");
	not_reached();
    }
...
get_title(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(220, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(221, __func__, "fprintf #0 error: %d", ret);
            not_reached();
	}
    }
	 ...
	 */
	    ...
	    ret = fprintf(stderr, "Your title must be between 1 and %d ASCII characters long.\n\n", MAX_TITLE_LEN);
	    if (ret <= 0) {
		errp(222, __func__, "fprintf #1 error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	if (safe == false) {
        ...
        if (need_confirm && !answer_yes) {
            ...
            ret = printf("\nThe title you entered is: %s\n",
                         title);
            if (ret <= 0) {
                errp(223, __func__, "fprintf title");
                not_reached();
            }
...
get_abstract(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(224, __func__, "called with NULL infp");
	not_reached();
    }
	 ...
	 */
	    ...
	    if (ret <= 0) {
		errp(225, __func__, "fprintf error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
        ...
        if (need_confirm && !answer_yes) {
            ...
            ret = printf("\nThe abstract you entered is: %s\n",
                         abstract);
            if (ret <= 0) {
                errp(226, __func__, "fprintf abstract");
                not_reached();
            }
...
get_author_info(struct author **author_set_p)
{
	...
	err(227, __func__, "called with NULL author_set_p");
	not_reached();
    }
	...
	if (ret != 1 || test_author_count(author_count) == false) {
	    ...
	    if (ret <= 0) {
		errp(228, __func__, "fprintf error #0 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "\nIf you happen to have more than %d authors, we ask that you pick\n", MAX_AUTHORS);
	    if (ret <= 0) {
		errp(229, __func__, "fprintf error #1 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "just %d authors and mention the remaining NUMBER of the authors in\nthe remarks file.\n",
                    MAX_AUTHORS);
	    if (ret <= 0) {
		errp(230, __func__, "fprintf error #2 while printing author number range");
                not_reached();
	    }
    ...
    if (author_set == NULL) {
	errp(231, __func__, "calloc a struct author array of length: %d failed", author_count);
	not_reached();
    }
	...
	if (ret < 0) {
	    errp(232, __func__, "puts error printing ISO 3166-1 URL0");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL1);
	if (ret < 0) {
	    errp(233, __func__, "puts error printing ISO 3166-1 URL1");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL2);
	if (ret < 0) {
	    errp(234, __func__, "puts error printing ISO 3166-1 URL2");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL3);
	if (ret < 0) {
	    errp(235, __func__, "puts error printing ISO 3166-1 URL3");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL4);
	if (ret < 0) {
	    errp(236, __func__, "puts error printing ISO 3166-1 URL4");
            not_reached();
	}
	para("",
	...
	if (ret <= 0) {
	    errp(237, __func__, "printf error printing author number");
            not_reached();
	}
	...
	do {
		...
		if (ret <= 0) {
		    errp(238, __func__, "fprintf error while reject name that is too long");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(239, __func__, "fprintf error while reject duplicate name");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(240, __func__, "fprintf while printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		...
		if (ret <= 0) {
		    errp(241, __func__, "fprintf while printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(242, __func__, "fprintf while printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(243, __func__, "fprintf while printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL0);
		if (ret <= 0) {
		    errp(244, __func__, "fprintf when printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		      "or from these Wikipedia / ISO web pages:",
		...
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL1);
		if (ret <= 0) {
		    errp(245, __func__, "fprintf when printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(246, __func__, "fprintf when printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(247, __func__, "fprintf when printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		if (ret <= 0) {
		    errp(248, __func__, "fprintf location/country code assignment");
                    not_reached();
		}
	...
	do {
		...
		if (ret <= 0) {
		    errp(249, __func__, "fprintf error while printing Email address length limit");
                    not_reached();
		}
		fpara(stderr,
	...
	do {
		...
		ret = fprintf(stderr, "\nSorry ( tm Canada :-) ), we limit URLs to %d characters\n\n", MAX_URL_LEN);
		if (ret <= 0) {
		    errp(11, __func__, "fprintf error while printing URL length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(12, __func__, "fprintf error while printing mastodon handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr,
			    ...
			    MAX_GITHUB_LEN);
		if (ret <= 0) {
		    errp(13, __func__, "fprintf error while printing GitHub user length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr, "\nSorry ( tm Canada :-) ), we limit affiliation names to %d characters\n\n",
			    MAX_AFFILIATION_LEN);
		if (ret <= 0) {
		    errp(14, __func__, "fprintf error while printing affiliation length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
	    ...
	    if (def_handle == NULL) {
		err(15, __func__, "default_handle() returned NULL!");
		not_reached();
	    }
		...
		if (ret <= 0) {
		    errp(16, __func__, "fprintf error while printing default IOCCC author handle");
                    not_reached();
		}
	    }
		...
		if (ret <= 0) {
		    errp(17, __func__, "fprintf error while printing IOCCC author handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(18, __func__, "fprintf error while printing duplicate author_handle error");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	if (printf("\nPlease verify the information about author #%d\n\n", i) <= 0 ||
	    ...
	    errp(19, __func__, "error while printing author #%d information\n", i);
	    not_reached();
	}
	if (need_confirm) {
...
verify_submission_dir(char const *submission_dir, char const *ls)
{
    ...
    ret = printf("    %s\n", submission_dir);
    if (ret <= 0) {
	errp(21, __func__, "printf error code: %d", ret);
        not_reached();
    }
    para("",
    ...
    if (exit_code != 0) {
	err(22, __func__, "cd -- %s && %s -lakR . failed with exit code: %d",
    ...
    if (ls_stream == NULL) {
	err(23, __func__, "popen filed for: cd -- %s && %s -lakR .", submission_dir, ls);
	not_reached();
    }
	...
	err(24, __func__, "EOF while reading output of ls: %s", ls);
	not_reached();
    }
    /*
     ...
     */
    if (i == 0) {
        err(25, __func__, "found no k-block line in ls output");
        not_reached();
    }
    if (kdirsize <= 0) {
	err(26, __func__, "ls k-block value: %d <= 0", kdirsize);
	not_reached();
    }
...
form_info(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
        err(27, __func__, "passed NULL infop");
        not_reached();
    }
    /*
    ...
    ret = setenv("TZ", "UTC", 1);
    if (ret < 0) {
	errp(28, __func__, "cannot set TZ=UTC");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    timeptr = gmtime(&(infop->tstamp));
    if (timeptr == NULL) {
	errp(29, __func__, "gmtime returned NULL");
	not_reached();
    }
	...
	errp(30, __func__, "calloc of %zu bytes failed", utctime_len + 1);
	not_reached();
    }
    ...
    if (strftime_ret == 0) {
	errp(31, __func__, "strftime returned 0");
	not_reached();
    }
...
write_json_files(struct auth *authp, struct info *infop, char const *submission_dir, char const *chksubmit)
{
	...
	err(33, __func__, "author_count %d <= 0", authp->author_count);
	not_reached();
    } else if (authp->author_count > MAX_AUTHORS) {
	err(34, __func__, "author count %d > max authors %d", authp->author_count, MAX_AUTHORS);
	not_reached();
    }
    ...
    if (auth_path == NULL) {
	errp(35, __func__, "calloc of %zu bytes failed", auth_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ...
    if (auth_stream == NULL) {
	errp(37, __func__, "failed to open for writing: %s", auth_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        err(38, __func__, "failed to obtain file descriptor for: %s", auth_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(39, __func__, "fprintf error writing leading part of authorship to %s", auth_path);
	not_reached();
    }
	...
	if (ret == false) {
	    errp(40, __func__, "fprintf error writing author %d info to %s", i, auth_path);
	    not_reached();
	}
    }
	...
	fprintf(auth_stream, "}\n") > 0;
    if (!ret) {
	errp(41, __func__, "fprintf error writing trailing part of authorship to %s", auth_path);
	not_reached();
    }
    ...
    if (ret != 0) {
        err(43, __func__, "chmod(2) failed to set user, group and other read-only on %s", auth_path);
        not_reached();
    }
    ...
    ret = close(fd);
    if (ret < 0) {
        errp(44, __func__, "close(fd) failed");
        not_reached();
    }
     ...
     */
    if (infop->required_files == NULL) {
        err(45, __func__, "called with NULL files list");
        not_reached();
    }
    ...
    if (info_path == NULL) {
	errp(46, __func__, "calloc of %zu bytes failed", info_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(info_path, info_path_len, "%s/%s", submission_dir, INFO_JSON_FILENAME);
    if (ret <= 0) {
	errp(47, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
    ...
    if (info_stream == NULL) {
	errp(48, __func__, "failed to open for writing: %s", info_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        errp(49, __func__, "failed to obtain file descriptor for: %s", info_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(50, __func__, "fprintf error writing leading part of info to %s", info_path);
	not_reached();
    }
    ...
    if (!ret) {
	errp(51, __func__, "fprintf error writing mandatory filename to %s", info_path);
	not_reached();
    }
    ...
    if (infop->extra_count > 0) {
        ...
        for (j = 0; j < infop->extra_count; ++j) {
            ...
            if (p == NULL) {
                err(52, __func__, "found NULL pointer in files list, element: %zu", j);
                not_reached();
            }
            ret =                   fprintf(info_stream, "        {\n") > 0 &&
            ...
            if (!ret) {
                errp(53, __func__, "fprintf error writing extra filename[%zu] to %s", j, info_path);
                not_reached();
            }
        }
    }
	...
	fprintf(info_stream, "}\n") > 0;
    if (!ret) {
	errp(54, __func__, "fprintf error writing trailing part of info to %s", info_path);
	not_reached();
    }
    ...
    ret = fchmod(fd, S_IRUSR | S_IRGRP | S_IROTH);
    if (ret != 0) {
        err(56, __func__, "chmod(2) failed to set user, group and other read-only on %s", info_path);
        not_reached();
    }
...
form_auth(struct auth *authp, struct info *infop, int author_count, struct author *authorp)
{
	...
	err(58, __func__, "infop->ioccc_id is NULL");
	not_reached();
    }
    if (infop->tarball == NULL) {
	err(59, __func__, "infop->tarball is NULL");
	not_reached();
    }
    if (infop->utctime == NULL) {
	err(60, __func__, "infop->utctime is NULL");
	not_reached();
    }
	...
	errp(61, __func__, "strdup() ioccc_id path %s failed", infop->ioccc_id);
	not_reached();
    }
	...
	errp(62, __func__, "strdup() tarball path %s failed", infop->tarball);
	not_reached();
    }
	...
	errp(63, __func__, "strdup() utctime path %s failed", infop->utctime);
	not_reached();
    }
    return;
}
	     ...
	     char const *ls, char const *txzchk, char const *fnamchk, bool test_mode)
{
    ...
    ret = chdir(workdir);
    if (ret < 0) {
	errp(66, __func__, "cannot cd %s", workdir);
	not_reached();
    }
    ...
    if (exit_code != 0) {
	err(67, __func__, "%s --format=v7 -cJf %s -- %s failed with exit code: %d",
    ...
    if (ret != 0) {
	errp(68, __func__, "stat of the compressed tarball failed: %s", basename_tarball_path);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(69, __func__, "The compressed tarball: %s size: %lld > %d",
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
	errp(70, __func__, "cannot fchdir to the previous current directory");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = close(cwd);
    if (ret < 0) {
	errp(71, __func__, "close of previous current directory failed");
	not_reached();
    }
    ...
    if (entertain) {
        ...
        if (exit_code != 0) {
            if (test_mode) {
                err(72, __func__, "%s -x -e -f %ju -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                               txzchk, feathery, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            } else {
                err(73, __func__, "%s -e -f %ju -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                               txzchk, feathery, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            }
            not_reached();
        }
        ...
        if (exit_code != 0) {
            if (test_mode) {
                err(74, __func__, "%s -x -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                   txzchk, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            } else {
                err(75, __func__, "%s -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                   txzchk, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            }
            not_reached();
        }
        ...
        bool test_mode, int slot_number)
{
	...
	err(77, __func__, "failed to cmdprintf: submission_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(78, __func__, "printf #0 error");
	not_reached();
    }
    ...
    if (workdir_esc == NULL) {
	err(79, __func__, "failed to cmdprintf: workdir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(80, __func__, "printf #2 error");
	not_reached();
    }
    free(workdir_esc);
...
show_registration_url(void)
{
    ...
    ret = printf("    %s\n", IOCCC_REGISTER_URL);
    if (ret <= 0) {
	errp(81, __func__, "printf error printing IOCCC_REGISTER_URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(82, __func__, "printf error printing IOCCC register FAQ URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(83, __func__, "printf error printing IOCCC_REGISTER_INFO_URL, IOCCC_PW_CHANGE_INFO_URL and IOCCC_SUBMIT_INFO_URL");
	not_reached();
    }
    ...
    ret = printf("    %s\n", IOCCC_STATUS_URL);
    if (ret < 0) {
	errp(84, __func__, "printf error printing IOCCC status URL");
	not_reached();
    }
...
show_submit_url(char const *workdir, char const *tarball_path, int slot_number)
{
    ...
    if (ret <= 0) {
	errp(85, __func__, "printf error printing tarball path and slot number");
	not_reached();
    }
    para("",
    ...
    ret = printf("    %s\n", IOCCC_SUBMIT_URL);
    if (ret < 0) {
	errp(86, __func__, "printf error printing IOCCC submit URL");
	not_reached();
    }
    ...
    para("",
     ...
     ret = printf("    %s\n", IOCCC_ENTER_FAQ_URL);
    if (ret < 0) {
	errp(87, __func__, "printf error printing IOCCC enter FAQ URL");
	not_reached();
    }
}
