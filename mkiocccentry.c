...
check_extra_data_files(struct info *infop, char const *entry_dir, char const *cp, int count, char **args)
{
	...
	if (!isascii(base[0]) || !isalnum(base[0])) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(121, __func__, "basename of extra data file: %s start with in invalid character: %s", args[i], base);
	    not_reached();
	}
...
yes_or_no(char const *question)
{
    ...
    if (response == NULL) {
	errp(130, __func__, "retry prompt is disabled");
	not_reached();
    }
...
get_title(struct info *infop)
{
     ...
     */
    if (title == NULL) {
	errp(132, __func__, "retry prompt is disabled");
	not_reached();
    }
...
get_abstract(struct info *infop)
{
     ...
     */
    if (abstract == NULL) {
	errp(134, __func__, "retry prompt is disabled");
	not_reached();
    }
...
get_author_info(struct info *infop, char *ioccc_id, struct author **author_set_p)
{
	...
	errp(136, __func__, "retry prompt is disabled");
	not_reached();
    }
    ...
    if (author_set == NULL) {
	errp(137, __func__, "malloc a struct author array of length: %d failed", author_count);
	not_reached();
    }
