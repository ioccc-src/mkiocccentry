...
array_has_path(struct dyn_array *array, char *str)
{
	...
	if (u == NULL) {	/* paranoia */
	    err(25, __func__, "found NULL pointer in path name dynamic array element: %ju", (uintmax_t)i);
	    not_reached();
	}
...
append_unique_filename(struct dyn_array *array, char *str)
{
     ...
     */
    if (array == NULL) {
	err(26, __func__, "array is NULL");
	not_reached();
    }
    if (str == NULL) {
	err(27, __func__, "str is NULL");
	not_reached();
    }
    ...
    if (array_has_path(array, str)) {
        /* str found in array */
        err(28, __func__, "string %s already in array", str);
        not_reached();
    }
...
check_ftsent(FTSENT *ent)
{
    ...
    switch (ent->fts_info) {
        case FTS_DC: /* cycle in directory tree */
            err(30, __func__, "detected directory loop with %s", ent->fts_path + 2);
            not_reached();
            break;
        case FTS_DNR: /* directory not readable */
            err(31, __func__, "directory not readable: %s", ent->fts_path + 2);
            not_reached();
            break;
        case FTS_ERR: /* some error condition */
             ...
             */
            errno = ent->fts_errno;
            errp(32, __func__, "encountered error reading path: %s", ent->fts_path + 2);
            not_reached();
            break;
        case FTS_DEFAULT: /* some other file type */
            err(33, __func__, "found invalid file type: %s", ent->fts_path + 2);
            not_reached();
            break;
        ...
        case FTS_NS: /* couldn't stat file */
            err(35, __func__, "couldn't stat %s", ent->fts_path + 2);
            not_reached();
            break;
        default: /* okay */
            /*
...
scan_topdir(char *args, struct info *infop, char const *make, char const *submission_dir, RuleCount *size)
{
    ...
    if (args == NULL || infop == NULL || make == NULL || submission_dir == NULL || size == NULL) {
        err(36, __func__, "passed NULL pointer(s)");
        not_reached();
    }
    ...
    if (infop->ignored_symlinks == NULL) {
        err(37, __func__, "failed to create ignored symlinks list array");
        not_reached();
    }
    /*
    ...
    if (infop->extra_files == NULL) {
        err(38, __func__, "failed to create files list array");
        not_reached();
    }
    /*
    ...
    if (infop->required_files == NULL) {
        err(39, __func__, "failed to create required files list array");
        not_reached();
    }
    /*
    ...
    if (infop->directories == NULL) {
        err(40, __func__, "failed to create directories list array");
        not_reached();
    }
    /*
    ...
    if (infop->ignored_dirs == NULL) {
        err(41, __func__, "failed to create ignored directories list array");
        not_reached();
    }
    /*
    ...
    if (infop->forbidden_files == NULL) {
        err(42, __func__, "failed to create forbidden files list array");
        not_reached();
    }
    /*
    ...
    if (infop->unsafe_files == NULL) {
        err(43, __func__, "failed to create unsafe filenames list array");
        not_reached();
    }
    /*
    ...
    if (infop->unsafe_dirs == NULL) {
        err(44, __func__, "failed to create unsafe directories list array");
        not_reached();
    }
    ...
    if (chdir(submission_dir) != 0) {
        errp(46, __func__, "unable to change to submission directory");
        not_reached();
    }
    ...
    if (getcwd(submit_path, PATH_MAX) == NULL) {
        errp(47, __func__, "couldn't get absolute path of submission directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(48, __func__, "couldn't change back to original original directory");
        not_reached();
    }
    ...
    errno = 0;      /* pre-clear errno for errp() */
    if (chdir(args) != 0) {
        errp(49, __func__, "chdir(\"%s\") failed", args);
        not_reached();
    }
    /*
    ...
    if (topdir < 0) {
	errp(50, __func__, "cannot open topdir");
	not_reached();
    }
    ...
    if (getcwd(topdir_path, PATH_MAX) == NULL) {
        errp(51, __func__, "couldn't get absolute path of topdir");
        not_reached();
    }
    ...
    if (ent == NULL){
        err(52, __func__, "failed to open \".\"");
        not_reached();
    } else {
        do {
            ...
            switch (sanity) {
                ...
                case PATH_ERR_NOT_RELATIVE: /* path not relative: starts with '/' */
                    err(59, __func__, "%s: path not relative", ent->fts_path + 2);
                    not_reached();
                    break;
                ...
                case PATH_ERR_MAX_NAME_LEN_0: /* max name length <= 0 (should never happen) */
                    err(60, __func__, "%s: %s", ent->fts_path + 2, path_sanity_error(sanity));
                    not_reached();
                    break;
                ...
                default: /* should never be reached */
                     ...
                     */
                    err(61, __func__, "unknown status for %s", ent->fts_path + 2);
                    not_reached();
                    break;
            }
            if (ignored_dirname) {
            ...
            switch (ent->fts_info) {
                case FTS_D: /* directory */
                    ...
                    if (optional) {
                         ...
                         */
                        err(65, __func__, "directory name matches optional filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    if (mandatory_filename) {
                         ...
                         */
                        err(66, __func__, "directory name matches required filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
     ...
     */
    if (!found_prog_c) {
        err(72, __func__, "prog.c not found in topdir %s", args);
        not_reached();
    } else if (!found_Makefile) {
        err(73, __func__, "Makefile not found in topdir %s", args);
        not_reached();
    } else if (!found_remarks_md) {
        err(74, __func__, "remarks.md not found in topdir %s", args);
        not_reached();
    }
        ...
        char const *make, RuleCount *size, int cwd)
{
    ...
    ent = read_fts(NULL, -1, NULL, FTS_NOCHDIR | FTS_PHYSICAL, &fts, fts_cmp);
    if (ent == NULL){
        err(146, __func__, "failed to open \".\"");
        not_reached();
    } else {
        do {
            bool ignored_dirname = false;
            ...
            switch (ent->fts_info) {
                case FTS_D: /* directory */
                    ...
                    if (optional) {
                         ...
                         * actually optional filenames
                         */
                        err(156, __func__, "directory name matches optional filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    if (mandatory_filename) {
                        /*
                         ...
                         * actually required filenames either
                         */
                        err(157, __func__, "directory name matches required filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    /*
                    ...
                    if (!array_has_path(infop->directories, ent->fts_path + 2)) {
                        err(158, __func__, "directory in submission directory not found in topdir: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    /*
                     * extra sanity checks
                    ...
                    if (!is_dir(ent->fts_path + 2)) {
                        err(159, __func__, "fts_read() found %s as directory but is_dir() returned false", ent->fts_path + 2);
                        not_reached();
                    } else if (!is_mode(ent->fts_path + 2, 0755)) {
                        err(160, __func__, "directory %s: mode %o != 0755", ent->fts_path + 2, filemode(ent->fts_path + 2));
                        not_reached();
                    }
                    /*
                    ...
                    if (!is_mode(ent->fts_path + 2, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
                        err(161, __func__, "directory %s must be mode 0444: %o != 0444", ent->fts_path + 2,
                ...
                case FTS_F:
                    ...
                    if (!is_file(ent->fts_path + 2)) {
                        err(163, __func__, "fts_read() found %s as file but is_file() returned false", ent->fts_path + 2);
                        not_reached();
                    }
                    ...
                    if (strcmp(filename, PROG_C_FILENAME) && strcmp(filename, MAKEFILE_FILENAME) &&
                        strcmp(filename, REMARKS_FILENAME)) {
                            if (!array_has_path(infop->extra_files, filename)) {
                                err(165, __func__, "file in submission directory not in topdir: %s", filename);
                                not_reached();
                            }
                            if (!strcmp(filename, TRY_SH) || !strcmp(filename, TRY_ALT_SH)) {
                                if (!is_mode(filename, S_IRUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
                                     ...
                                     */
                                    err(166, __func__, "file %s must be mode 0555: %o != 0555",
                                ...
                                if (!is_mode(filename, S_IRUSR | S_IRGRP | S_IROTH)) {
                                    err(167, __func__, "file %s must be mode 0444: %o != 0444",
                                            filename, filemode(filename));
                                    not_reached();
                                }
                            }
                        ...
                        if (!array_has_path(infop->required_files, filename)) {
                            err(168, __func__, "file in submission directory not in topdir: %s", filename);
                            not_reached();
                        }
                        /*
                         ...
                         */
                        if (!is_mode(filename, S_IRUSR | S_IRGRP | S_IROTH)) {
                            err(169, __func__, "file %s must be mode 0444: %o != 0444", filename,
                                    filemode(filename));
                            not_reached();
                        }
                ...
                case FTS_SL: /* symlink */
                case FTS_SLNONE: /* symlink with non-existing target */
                    err(170, __func__, "found symlink in submission directory");
                    not_reached();
                    break;
                default:
                    break;
     ...
     */
    if (count > MAX_EXTRA_FILE_COUNT) {
        err(174, __func__, "too many files: %ju > %ju", (uintmax_t)count, (uintmax_t)MAX_FILE_COUNT);
        not_reached();
    }
    /*
     * verify that there are not too many directories if MAX_EXTRA_DIR_COUNT > 0.
    ...
    if (MAX_EXTRA_DIR_COUNT > 0 && len > MAX_EXTRA_DIR_COUNT) {
        err(175, __func__, "too many extra directories in submission directory: %ju > %ju", (uintmax_t)len,
                (uintmax_t)MAX_EXTRA_DIR_COUNT);
        not_reached();
    }
    ...
    len = dyn_array_tell(infop->required_files);
    if (len <= 0) {
        err(176, __func__, "list of required files list is empty");
        not_reached();
    }
    len2 = dyn_array_tell(required_files);
    if (len2 <= 0) {
        err(177, __func__, "list of required files in submission directory is empty");
        not_reached();
    }
    /*
     ...
     */
    if (len != len2) {
        err(178, __func__, "size of required files list in submission directory != size in topdir: %ju != %ju", (uintmax_t)len2,
                (uintmax_t)len);
        not_reached();
    }
    for (i = 0; i < len; ++i) {
        ...
        p = dyn_array_value(infop->required_files, char *, i);
        if (p == NULL) {
            err(179, __func__, "found NULL pointer in required files list, element: %ju", (uintmax_t)i);
            not_reached();
        }
        /*
        ...
        if (fname == NULL) {
            err(180, __func__, "found NULL pointer in required files list in submission directory, element: %ju", (uintmax_t)i);
            not_reached();
        }
        ...
        if (strcmp(p, fname) != 0 || !array_has_path(required_files, p)) {
            ...
            filename = strdup(p);
            if (filename == NULL) {
                errp(181, __func__, "strdup(\"%s\") failed", p);
                not_reached();
            }
        ...
        if (array_has_path(directories, p)) {
            err(182, __func__, "required file %s in directories list", p);
            not_reached();
        }
        if (array_has_path(extra_files, p)) {
            err(183, __func__, "required file %s in extra files list", p);
            not_reached();
        }
        ...
        if (!strcmp(fname, PROG_C_FILENAME)) {
            ...
            *size = check_prog_c(infop, fname);
             ...
             */
            err(184, __func__, "BUG: non-required file found in required files list: %s, please report", fname);
            not_reached();
        }
    }
    ...
    if (array_has_path(missing_files, PROG_C_FILENAME)) {
        err(185, __func__, "prog.c in missing files list!");
        not_reached();
    }
    if (array_has_path(missing_files, MAKEFILE_FILENAME)) {
        err(186, __func__, "Makefile in missing files list!");
        not_reached();
    }
    if (array_has_path(missing_files, REMARKS_FILENAME)) {
        err(187, __func__, "remarks.md in missing files list!");
        not_reached();
    }
     ...
     */
    if (len != len2) {
        err(188, __func__, "size of extra files list in submission directory != size in topdir: %ju != %ju", (uintmax_t)len2,
                (uintmax_t)len);
        not_reached();
    }
    if (len > 0) {
        for (i = 0; i < len; ++i) {
            ...
            p = dyn_array_value(infop->extra_files, char *, i);
            if (p == NULL) {
                err(189, __func__, "found NULL pointer in extra files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            /*
            ...
            if (p == NULL) {
                err(190, __func__, "found NULL pointer in extra files list in submission directory, element: %ju", (uintmax_t)i);
                not_reached();
            }
            ...
            if (strcmp(p, fname) != 0 || !array_has_path(extra_files, p)) {
                ...
                filename = strdup(p);
                if (filename == NULL) {
                    errp(191, __func__, "strdup(\"%s\") failed", p);
                    not_reached();
                }
                append_unique_filename(missing_files, filename);
            }
    ...
    if (len > 0) {
        ...
        if (ret <= 0) {
            errp(192, __func__, "error writing missing files list title");
            not_reached();
        }
        for (i = 0; i < len; i++) {
            /*
            ...
            if (fname == NULL) {
                err(193, __func__, "found NULL pointer in missing files list in submission directory, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", fname);
        }
        err(194, __func__, "aborting due to missing files");
        not_reached();
    }
     ...
     */
    if (len != len2) {
        err(195, __func__, "size of directories list in submission directory != size in topdir: %ju != %ju", (uintmax_t)len2,
                (uintmax_t)len);
        not_reached();
    }
    if (len > 0) {
        for (i = 0; i < len; ++i) {
            p = dyn_array_value(infop->directories, char *, i);
            if (p == NULL) {
                err(196, __func__, "found NULL pointer in directories list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            fname = dyn_array_value(directories, char *, i);
            if (fname == NULL) {
                err(197, __func__, "found NULL pointer in directories list in submission directory, element: %ju", (uintmax_t)i);
                not_reached();
            }
            ...
            if (strcmp(fname, p) != 0 || !array_has_path(directories, p)) {
                ...
                filename = strdup(p);
                if (filename == NULL) {
                    errp(198, __func__, "strdup(\"%s\") failed", p);
                    not_reached();
                }
    ...
    if (len > 0) {
        ...
        if (ret <= 0) {
            errp(199, __func__, "error writing missing directories list title");
            not_reached();
        }
        for (i = 0; i < len; i++) {
            /*
        ...
        err(201, __func__, "aborting due to missing directories");
        not_reached();
    }
    ...
    if (len > 0) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(202, __func__, "found NULL pointer in directories list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
    ...
    if (len <= 0) {
        err(204, __func__, "list of required files is empty");
        not_reached();
    }
    if (len > 0) {
        para("",
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(205, __func__, "found NULL pointer in required files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        ...
        if (len > 0) {
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(206, __func__, "found NULL pointer in non-required files list, element: %ju", (uintmax_t)i);
                    not_reached();
                }
                print("%s\n", p);
            }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(208, __func__, "unable to change to previous directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (close(cwd) != 0) {
        errp(209, __func__, "failed to close(cwd)");
        not_reached();
    }
}
           ...
           char const *make)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(211, __func__, "tar does not exist: %s", tar);
	not_reached();
    }
    if (!is_file(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(212, __func__, "tar is not a regular file: %s", tar);
	not_reached();
    }
    if (!is_exec(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(213, __func__, "tar is not an executable program: %s", tar);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(214, __func__, "ls does not exist: %s", ls);
	not_reached();
    }
    if (!is_file(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(215, __func__, "ls is not a regular file: %s", ls);
	not_reached();
    }
    if (!is_exec(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(216, __func__, "ls is not an executable program: %s", ls);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(217, __func__, "txzchk does not exist: %s", txzchk);
	not_reached();
    }
    if (!is_file(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(218, __func__, "txzchk is not a regular file: %s", txzchk);
	not_reached();
    }
    if (!is_exec(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(219, __func__, "txzchk is not an executable program: %s", txzchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(220, __func__, "fnamchk does not exist: %s", fnamchk);
	not_reached();
    }
    if (!is_file(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(221, __func__, "fnamchk is not a regular file: %s", fnamchk);
	not_reached();
    }
    if (!is_exec(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(222, __func__, "fnamchk is not an executable program: %s", fnamchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(223, __func__, "chkentry does not exist: %s", chkentry);
	not_reached();
    }
    if (!is_file(chkentry)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(224, __func__, "chkentry is not a regular file: %s", chkentry);
	not_reached();
    }
    if (!is_exec(chkentry)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(225, __func__, "chkentry is not an executable program: %s", chkentry);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(226, __func__, "make does not exist: %s", make);
	not_reached();
    }
    if (!is_file(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(227, __func__, "make is not a regular file: %s", make);
	not_reached();
    }
    if (!is_exec(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(228, __func__, "make is not an executable program: %s", make);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(229, __func__, "workdir does not exist: %s", workdir);
	not_reached();
    }
    if (!is_dir(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(230, __func__, "workdir is not a directory: %s", workdir);
	not_reached();
    }
    if (!is_write(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(231, __func__, "workdir is not a writable directory: %s", workdir);
	not_reached();
    }
...
prompt(char const *str, size_t *lenp)
{
     ...
     */
    if (str == NULL) {
	err(232, __func__, "called with NULL str");
	not_reached();
    }
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(233, __func__, "error printing prompt string");
		not_reached();
	    } else if (feof(stdout)) {
		err(234, __func__, "EOF while printing prompt string");
		not_reached();
	    } else {
		errp(235, __func__, "unexpected fputs error printing prompt string");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(236, __func__, "error printing :<space>");
		not_reached();
	    } else if (feof(stdout)) {
		err(237, __func__, "EOF while writing :<space>");
		not_reached();
	    } else {
		errp(238, __func__, "unexpected fputs error printing :<space>");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(239, __func__, "error flushing prompt to stdout");
		not_reached();
	    } else if (feof(stdout)) {
		err(240, __func__, "EOF while flushing prompt to stdout");
		not_reached();
	    } else {
		errp(241, __func__, "unexpected fflush error while flushing prompt to stdout");
		not_reached();
	    }
	}
    }
    ...
    if (buf == NULL) {
	err(242, __func__, "EOF while reading prompt input");
	not_reached();
    }
...
get_contest_id(bool *testp)
{
	...
	if (read_answers_flag_used && !seen_answers_header) {
	    err(244, __func__, "didn't find the correct answers file header");
	    not_reached();
	}
...
get_submit_slot(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(245, __func__, "called with NULL arg(s)");
	not_reached();
    }
    ...
    if (need_hints) {
        ...
        if (ret <= 0) {
            errp(246, __func__, "printf error printing number of submissions allowed");
            not_reached();
        }
        para("",
	...
	if (ret != 1 || submit_slot < 0 || submit_slot > MAX_SUBMIT_SLOT) {
	    ...
	    ret = fprintf(stderr, "\nThe submit slot number must be a number from 0 through %d; please re-enter.\n",
		    MAX_SUBMIT_SLOT);
	    if (ret <= 0) {
		errp(247, __func__, "fprintf error while informing about the valid submit slot number range");
                not_reached();
	    }
            /*
             * free storage
	    ...
	    submit_slot = -1;	/* invalidate input */
            ...
            if (need_confirm && !answer_yes) {
                ...
                ret = printf("\nThe slot number you entered is: %d\n",
                             submit_slot);
                if (ret <= 0) {
                    errp(248, __func__, "fprintf error writing slot number");
                    not_reached();
                }
	     ...
	     char **tarball_path, time_t tstamp, bool test_mode)
{
    ...
    if (test == false) {
	err(10, __func__, "submit slot number: %d must >= 0 and <= %d", submit_slot, MAX_SUBMIT_SLOT);
	not_reached();
    }
    ...
    if (submission_dir == NULL) {
	errp(11, __func__, "malloc #0 of %ju bytes failed", (uintmax_t)(submission_dir_len + 1));
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(submission_dir, submission_dir_len + 1, "%s/%s-%d", workdir, ioccc_id, submit_slot);
    if (ret <= 0) {
	errp(12, __func__, "snprintf to form submission directory failed");
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(13, __func__, "fprintf error while informing that the submission directory already exists");
            not_reached();
	}
	fpara(stderr,
	      "",
	      ...
	      "",
	      NULL);
	err(14, __func__, "submission directory exists: %s", submission_dir);
	not_reached();
    }
    dbg(DBG_HIGH, "submission directory path: %s", submission_dir);
    ...
    if (ret < 0) {
	errp(15, __func__, "cannot mkdir %s", submission_dir);
	not_reached();
    }
    errno = 0; /* pre-clear errno for errp() */
    ret = chmod(submission_dir, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    if (ret < 0) {
        errp(16, __func__, "cannot chmod directory %s to mode 0755", submission_dir);
        not_reached();
    }
    ...
    *tarball_path = form_tar_filename(ioccc_id, submit_slot, test_mode, tstamp);
    if (*tarball_path == NULL) {
	errp(17, __func__, "failed to form compressed tarball path");
	not_reached();
    }
...
warn_empty_prog(void)
{
	...
	if (!yorn) {
	    err(18, __func__, "please fix your prog.c file");
	    not_reached();
	}
...
warn_rule_2a_size(struct info *infop, int mode, RuleCount size)
{
     ...
     */
    if (infop == NULL) {
	err(19, __func__, "called with NULL infop");
	not_reached();
    }
	...
	ret = fprintf(stderr, "\nWARNING: The prog.c size: %jd > Rule 2a maximum: %jd\n",
		      (intmax_t)infop->rule_2a_size, (intmax_t)RULE_2A_SIZE);
	if (ret <= 0) {
	    errp(20, __func__, "fprintf error when printing prog.c Rule 2a warning");
            not_reached();
	}
	if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes)) {
	    fpara(stderr,
	...
	if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes)) {
	    ...
	    if (ret <= 0) {
		errp(22, __func__, "fprintf error when printing prog.c file size and Rule 2a mismatch");
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	    ...
	    if (!yorn) {
		err(23, __func__, "please fix your prog.c file");
		not_reached();
	    }
	...
	err(24, __func__, "invalid mode passed to function: %d", mode);
	not_reached();
    }
    return;
}
...
warn_nul_chars(void)
{
	...
	if (ret <= 0) {
	    errp(25, __func__, "fprintf error when printing prog.c nul_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
warn_trigraph(void)
{
	...
	if (ret <= 0) {
	    errp(27, __func__, "fprintf error when printing prog.c trigraph_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	yorn = yes_or_no("Are you sure you want to proceed? [Ny]", false);
	if (!yorn) {
	    err(28, __func__, "please fix your prog.c file");
	    not_reached();
	}
...
warn_wordbuf(void)
{
	...
	ret = fprintf(stderr, "\nprog.c triggered a word buffer overflow!\n"
			      ...
			      "YOUR remarks.md FILE!\n\n");
	if (ret <= 0) {
	    errp(29, __func__, "fprintf error when printing prog.c wordbuf_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	yorn = yes_or_no("Are you sure you want to proceed? [Ny]", false);
	if (!yorn) {
	    err(30, __func__, "please fix your prog.c file");
	    not_reached();
	}
...
warn_ungetc(void)
{
	...
	ret = fprintf(stderr, "\nprog.c triggered an ungetc error: @SirWumpus goofed\n"
			      ...
			      "YOUR remarks.md FILE!\n\n");
	if (ret <= 0) {
	    errp(31, __func__, "fprintf error when printing prog.c ungetc_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	yorn = yes_or_no("Are you sure you want to proceed? [Ny]", false);
	if (!yorn) {
	    err(32, __func__, "please fix your prog.c file");
	    not_reached();
	}
...
warn_rule_2b_size(struct info *infop)
{
	...
	if (ret <= 0) {
	    errp(34, __func__, "printf error printing prog.c size > Rule 2b maximum");
	    not_reached();
	}
...
check_prog_c(struct info *infop, char const *prog_c)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(37, __func__, "prog.c does not exist: %s", prog_c);
	not_reached();
    }
    if (!is_file(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(38, __func__, "prog.c is not a regular file: %s", prog_c);
	not_reached();
    }
    if (!is_read(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(39, __func__, "prog.c is not a readable file: %s", prog_c);
	not_reached();
    }
    ...
    if (prog_stream == NULL) {
	errp(40, __func__, "failed to fopen: %s", prog_c);
	not_reached();
    }
    ...
    if (ret != 0) {
	errp(41, __func__, "failed to fclose: %s", prog_c);
	not_reached();
    }
	...
	err(42, __func__, "file_size error: %jd on prog.c: %s", (intmax_t)infop->rule_2a_size, prog_c);
	not_reached();
...
inspect_Makefile(char const *Makefile, struct info *infop)
{
    ...
    if (stream == NULL) {
	errp(44, __func__, "cannot open Makefile: %s", Makefile);
	not_reached();
    }
...
warn_Makefile(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(46, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (!answer_yes) {
	    ...
	    if (!yorn) {
		err(47, __func__, "Use a different Makefile or modify your Makefile");
		not_reached();
	    }
	}
    }
...
check_Makefile(struct info *infop, char const *Makefile)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(49, __func__, "Makefile does not exist: %s", Makefile);
	not_reached();
    }
    if (!is_file(Makefile)) {
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(50, __func__, "Makefile is not a regular file: %s", Makefile);
	not_reached();
    }
    if (!is_read(Makefile)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(51, __func__, "Makefile is not readable file: %s", Makefile);
	not_reached();
    }
    filesize = file_size(Makefile);
    if (filesize < 0) {
	err(52, __func__, "file_size error: %jd on Makefile  %s", (intmax_t)filesize, Makefile);
	not_reached();
    } else if (filesize == 0) {
	err(53, __func__, "Makefile cannot be empty: %s", Makefile);
	not_reached();
    }
...
check_remarks_md(struct info *infop, char const *remarks_md)
{
	...
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(55, __func__, "remarks.md does not exist: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr, "",
	      ...
	      "",
	      NULL);
	err(56, __func__, "remarks.md is not a regular file: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(57, __func__, "remarks.md is not readable file: %s", remarks_md);
	not_reached();
    }
    filesize = file_size(remarks_md);
    if (filesize < 0) {
	err(58, __func__, "file_size error: %jd on remarks.md %s", (intmax_t)filesize, remarks_md);
	not_reached();
    } else if (filesize == 0) {
	err(59, __func__, "remarks.md cannot be empty: %s", remarks_md);
	not_reached();
    }
...
yes_or_no(char const *question, bool def_answer)
{
     ...
     */
    if (question == NULL) {
	err(60, __func__, "called with NULL question");
	not_reached();
    }
...
get_title(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(61, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(62, __func__, "fprintf #0 error: %d", ret);
            not_reached();
	}
    }
	 ...
	 */
	    ...
	    ret = fprintf(stderr, "Your title must be between 1 and %d ASCII characters long.\n\n", MAX_TITLE_LEN);
	    if (ret <= 0) {
		errp(63, __func__, "fprintf #1 error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	if (posix_plus_safe(title, true, false, true) == false) {
        ...
        if (need_confirm && !answer_yes) {
            ...
            ret = printf("\nThe title you entered is: %s\n",
                         title);
            if (ret <= 0) {
                errp(64, __func__, "fprintf title");
                not_reached();
            }
...
get_abstract(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(65, __func__, "called with NULL infp");
	not_reached();
    }
	 ...
	 */
	    ...
	    if (ret <= 0) {
		errp(66, __func__, "fprintf error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
        ...
        if (need_confirm && !answer_yes) {
            ...
            ret = printf("\nThe abstract you entered is: %s\n",
                         abstract);
            if (ret <= 0) {
                errp(67, __func__, "fprintf abstract");
                not_reached();
            }
...
get_author_info(struct author **author_set_p)
{
	...
	err(68, __func__, "called with NULL author_set_p");
	not_reached();
    }
	...
	if (ret != 1 || test_author_count(author_count) == false) {
	    ...
	    if (ret <= 0) {
		errp(69, __func__, "fprintf error #0 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "\nIf you happen to have more than %d authors, we ask that you pick\n", MAX_AUTHORS);
	    if (ret <= 0) {
		errp(70, __func__, "fprintf error #1 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "just %d authors and mention the remaining NUMBER of the authors in\nthe remarks file.\n",
                    MAX_AUTHORS);
	    if (ret <= 0) {
		errp(71, __func__, "fprintf error #2 while printing author number range");
                not_reached();
	    }
    ...
    if (author_set == NULL) {
	errp(72, __func__, "malloc a struct author array of length: %d failed", author_count);
	not_reached();
    }
	...
	if (ret < 0) {
	    errp(73, __func__, "puts error printing ISO 3166-1 URL0");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL1);
	if (ret < 0) {
	    errp(74, __func__, "puts error printing ISO 3166-1 URL1");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL2);
	if (ret < 0) {
	    errp(75, __func__, "puts error printing ISO 3166-1 URL2");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL3);
	if (ret < 0) {
	    errp(76, __func__, "puts error printing ISO 3166-1 URL3");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL4);
	if (ret < 0) {
	    errp(77, __func__, "puts error printing ISO 3166-1 URL4");
            not_reached();
	}
	para("",
	...
	if (ret <= 0) {
	    errp(78, __func__, "printf error printing author number");
            not_reached();
	}
	...
	do {
		...
		if (ret <= 0) {
		    errp(79, __func__, "fprintf error while reject name that is too long");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(80, __func__, "fprintf error while reject duplicate name");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(81, __func__, "fprintf while printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		...
		if (ret <= 0) {
		    errp(82, __func__, "fprintf while printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(83, __func__, "fprintf while printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(84, __func__, "fprintf while printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL0);
		if (ret <= 0) {
		    errp(85, __func__, "fprintf when printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		      "or from these Wikipedia / ISO web pages:",
		...
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL1);
		if (ret <= 0) {
		    errp(86, __func__, "fprintf when printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(87, __func__, "fprintf when printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(88, __func__, "fprintf when printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		if (ret <= 0) {
		    errp(89, __func__, "fprintf location/country code assignment");
                    not_reached();
		}
	...
	do {
		...
		if (ret <= 0) {
		    errp(90, __func__, "fprintf error while printing Email address length limit");
                    not_reached();
		}
		fpara(stderr,
	...
	do {
		...
		if (ret <= 0) {
		    errp(93, __func__, "fprintf error while printing mastodon handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr,
			    ...
			    MAX_GITHUB_LEN);
		if (ret <= 0) {
		    errp(94, __func__, "fprintf error while printing GitHub user length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr, "\nSorry ( tm Canada :-) ), we limit affiliation names to %d characters\n\n",
			    MAX_AFFILIATION_LEN);
		if (ret <= 0) {
		    errp(95, __func__, "fprintf error while printing affiliation length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
	    ...
	    if (def_handle == NULL) {
		err(96, __func__, "default_handle() returned NULL!");
		not_reached();
	    }
		...
		if (ret <= 0) {
		    errp(97, __func__, "fprintf error while printing default IOCCC author handle");
                    not_reached();
		}
	    }
		...
		if (ret <= 0) {
		    errp(98, __func__, "fprintf error while printing IOCCC author handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(99, __func__, "fprintf error while printing duplicate author_handle error");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	if (printf("\nPlease verify the information about author #%d\n\n", i) <= 0 ||
	    ...
	    errp(100, __func__, "error while printing author #%d information\n", i);
	    not_reached();
	}
	if (need_confirm) {
...
verify_submission_dir(char const *submission_dir, char const *ls)
{
    ...
    ret = printf("    %s\n", submission_dir);
    if (ret <= 0) {
	errp(102, __func__, "printf error code: %d", ret);
        not_reached();
    }
    para("",
    ...
    if (exit_code != 0) {
	err(103, __func__, "cd -- %s && %s -lakR . failed with exit code: %d",
    ...
    if (ls_stream == NULL) {
	err(104, __func__, "popen filed for: cd -- %s && %s -lakR .", submission_dir, ls);
	not_reached();
    }
	...
	err(105, __func__, "EOF while reading output of ls: %s", ls);
	not_reached();
    }
    /*
     ...
     */
    if (i == 0) {
        err(106, __func__, "found no k-block line in ls output");
        not_reached();
    }
    if (kdirsize <= 0) {
	err(107, __func__, "ls k-block value: %d <= 0", kdirsize);
	not_reached();
    }
	...
	if (!yorn) {
	    fpara(stderr,
		  ...
		  "We suggest you remove the existing submission directory (that we created,",
                  ...
                  "again.",
		  NULL);
	    err(108, __func__, "user rejected listing of submission directory: %s", submission_dir);
	    not_reached();
	}
    }
...
write_info(struct info *infop, char const *submission_dir, char const *chkentry, char const *fnamchk)
{
    ...
    if (infop->extra_files == NULL) {
        err(110, __func__, "called with NULL files list");
        not_reached();
    }
    ...
    ret = setenv("TZ", "UTC", 1);
    if (ret < 0) {
	errp(111, __func__, "cannot set TZ=UTC");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    timeptr = gmtime(&(infop->tstamp));
    if (timeptr == NULL) {
	errp(112, __func__, "gmtime returned NULL");
	not_reached();
    }
	...
	errp(113, __func__, "calloc of %ju bytes failed", (uintmax_t)utctime_len + 1);
	not_reached();
    }
    ...
    if (strftime_ret == 0) {
	errp(114, __func__, "strftime returned 0");
	not_reached();
    }
    ...
    if (info_path == NULL) {
	errp(115, __func__, "malloc of %ju bytes failed", (uintmax_t)info_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(info_path, info_path_len, "%s/%s", submission_dir, INFO_JSON_FILENAME);
    if (ret <= 0) {
	errp(116, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
    ...
    if (info_stream == NULL) {
	errp(117, __func__, "failed to open for writing: %s", info_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        errp(118, __func__, "failed to obtain file descriptor for: %s", info_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(119, __func__, "fprintf error writing leading part of info to %s", info_path);
	not_reached();
    }
    ...
    if (!ret) {
	errp(120, __func__, "fprintf error writing mandatory filename to %s", info_path);
	not_reached();
    }
    ...
    if (infop->extra_count > 0) {
        ...
        for (i = 0; i < infop->extra_count; ++i) {
            p = dyn_array_value(infop->extra_files, char *, i);
            if (p == NULL) {
                err(121, __func__, "found NULL pointer in files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            ret =                   fprintf(info_stream, "        {\n") > 0 &&
            ...
            if (!ret) {
                errp(122, __func__, "fprintf error writing extra filename[%ju] to %s", (uintmax_t)i, info_path);
                not_reached();
            }
        }
    }
	...
	fprintf(info_stream, "}\n") > 0;
    if (!ret) {
	errp(123, __func__, "fprintf error writing trailing part of info to %s", info_path);
	not_reached();
    }
    ...
    if (ret < 0) {
	errp(124, __func__, "fclose error");
	not_reached();
    }
    ...
    if (exit_code != 0) {
	err(125, __func__, "%s -q -- . %s failed with exit code: %d",
    ...
    if (ret != 0) {
        err(126, __func__, "chmod(2) failed to set user, group and other read-only on %s", info_path);
        not_reached();
    }
...
form_auth(struct auth *authp, struct info *infop, int author_count, struct author *authorp)
{
	...
	err(129, __func__, "infop->ioccc_id is NULL");
	not_reached();
    }
    if (infop->tarball == NULL) {
	err(130, __func__, "infop->tarball is NULL");
	not_reached();
    }
    if (infop->utctime == NULL) {
	err(131, __func__, "infop->utctime is NULL");
	not_reached();
    }
	...
	errp(132, __func__, "strdup() ioccc_id path %s failed", infop->ioccc_id);
	not_reached();
    }
	...
	errp(133, __func__, "strdup() tarball path %s failed", infop->tarball);
	not_reached();
    }
	...
	errp(134, __func__, "strdup() utctime path %s failed", infop->utctime);
	not_reached();
    }
    return;
}
...
write_auth(struct auth *authp, char const *submission_dir, char const *chkentry, char const *fnamchk)
{
	...
	err(136, __func__, "author_count %d <= 0", authp->author_count);
	not_reached();
    } else if (authp->author_count > MAX_AUTHORS) {
	err(137, __func__, "author count %d > max authors %d", authp->author_count, MAX_AUTHORS);
	not_reached();
    }
    ...
    if (auth_path == NULL) {
	errp(138, __func__, "malloc of %ju bytes failed", (uintmax_t)auth_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(auth_path, auth_path_len, "%s/%s", submission_dir, AUTH_JSON_FILENAME);
    if (ret <= 0) {
	errp(139, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
    ...
    if (auth_stream == NULL) {
	errp(140, __func__, "failed to open for writing: %s", auth_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        err(141, __func__, "failed to obtain file descriptor for: %s", auth_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(142, __func__, "fprintf error writing leading part of authorship to %s", auth_path);
	not_reached();
    }
	...
	if (ret == false) {
	    errp(143, __func__, "fprintf error writing author %d info to %s", i, auth_path);
	    not_reached();
	}
    }
	...
	fprintf(auth_stream, "}\n") > 0;
    if (!ret) {
	errp(144, __func__, "fprintf error writing trailing part of authorship to %s", auth_path);
	not_reached();
    }
    ...
    if (ret < 0) {
	errp(145, __func__, "fclose error");
	not_reached();
    }
    ...
    if (exit_code != 0) {
	err(146, __func__, "%s -q -- %s . failed with exit code: %d",
    ...
    ret = fchmod(fd, S_IRUSR | S_IRGRP | S_IROTH);
    if (ret != 0) {
        err(147, __func__, "chmod(2) failed to set user, group and other read-only on %s", auth_path);
        not_reached();
    }
    ...
    ret = close(fd);
    if (ret < 0) {
        errp(148, __func__, "close(fd) failed");
        not_reached();
    }
	     ...
	     char const *ls, char const *txzchk, char const *fnamchk)
{
    ...
    ret = chdir(workdir);
    if (ret < 0) {
	errp(151, __func__, "cannot cd %s", workdir);
	not_reached();
    }
    ...
    exit_code = shell_cmd(__func__, false, true, "% --format=v7 -cJf % -- %",
				    tar, basename_tarball_path, basename_submission_dir);
    if (exit_code != 0) {
	err(152, __func__, "%s --format=v7 -cJf %s -- %s failed with exit code: %d",
    ...
    if (ret != 0) {
	errp(153, __func__, "stat of the compressed tarball failed: %s", basename_tarball_path);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(154, __func__, "The compressed tarball: %s size: %ju > %jd",
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
	errp(155, __func__, "cannot fchdir to the previous current directory");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = close(cwd);
    if (ret < 0) {
	errp(156, __func__, "close of previous current directory failed");
	not_reached();
    }
    ...
    if (entertain) {
        ...
        exit_code = shell_cmd(__func__, false, true, "% -e -w -v 1 -F % -- %/../%",
                                              txzchk, fnamchk, submission_dir, basename_tarball_path);
        if (exit_code != 0) {
            err(157, __func__, "%s -e -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
        ...
        exit_code = shell_cmd(__func__, false, true, "% -w -v 1 -F % -- %/../%",
                                              txzchk, fnamchk, submission_dir, basename_tarball_path);
        if (exit_code != 0) {
            err(158, __func__, "%s -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                               txzchk, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            not_reached();
        }
    }
        ...
        bool test_mode, int slot_number)
{
	...
	err(160, __func__, "failed to cmdprintf: submission_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(161, __func__, "printf #0 error");
	not_reached();
    }
    ...
    if (workdir_esc == NULL) {
	err(162, __func__, "failed to cmdprintf: workdir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(163, __func__, "printf #2 error");
	not_reached();
    }
    free(workdir_esc);
...
show_registration_url(void)
{
    ...
    ret = printf("    %s\n", IOCCC_REGISTER_URL);
    if (ret <= 0) {
	errp(164, __func__, "printf error printing IOCCC_REGISTER_URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(165, __func__, "printf error printing IOCCC register FAQ URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(166, __func__, "printf error printing IOCCC_REGISTER_INFO_URL, IOCCC_PW_CHANGE_INFO_URL and IOCCC_SUBMIT_INFO_URL");
	not_reached();
    }
    ...
    ret = printf("    %s\n", IOCCC_STATUS_URL);
    if (ret < 0) {
	errp(167, __func__, "printf error printing IOCCC status URL");
	not_reached();
    }
...
show_submit_url(char const *workdir, char const *tarball_path, int slot_number)
{
    ...
    if (ret <= 0) {
	errp(168, __func__, "printf error printing tarball path and slot number");
	not_reached();
    }
    para("",
    ...
    ret = printf("    %s\n", IOCCC_SUBMIT_URL);
    if (ret < 0) {
	errp(169, __func__, "printf error printing IOCCC submit URL");
	not_reached();
    }
    ...
    para("",
     ...
     ret = printf("    %s\n", IOCCC_ENTER_FAQ_URL);
    if (ret < 0) {
	errp(170, __func__, "printf error printing IOCCC enter FAQ URL");
	not_reached();
    }
}
