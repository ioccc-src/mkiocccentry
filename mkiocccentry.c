...
static bool array_has_path(struct dyn_array *array, char *str);
...
main(int argc, char *argv[])
{
    ...
    program = argv[0];
    while ((i = getopt(argc, argv, ":hv:J:qVt:l:a:i:A:WT:eF:C:yds:m:I:")) != -1) {
	switch (i) {
...
/*
 * array_has_path   - check if array has the path in str
 *
 * given:
 *
 *  array   - array to search
 *  str     - path to look for
 *
 * This function is used in append_unique_filename() and also in check_submission()
 * to verify a number of things are in order.
 *
 * NOTE: this function returns false on NULL pointers. This makes it much easier
 * to use and besides that if the string is NULL or empty  or the array is NULL
 * then obviously the answer is false.
 *
 * This function returns true if the array has 'str' and false otherwise.
 */
static bool
array_has_path(struct dyn_array *array, char *str)
{
     ...
     * execution time to grow as O(n^2).  However given that the IOCCC restricts
     * the number of files there should be no need to worry about employing a
     * more optimised search.
     */
	...
	if (u == NULL) {	/* paranoia */
	    err(23, __func__, "found NULL pointer in path name dynamic array element: %ju", (uintmax_t)i);
	    not_reached();
	}
...
append_unique_filename(struct dyn_array *array, char *str)
{
     ...
     */
    if (array == NULL) {
	err(24, __func__, "array is NULL");
	not_reached();
    }
    if (str == NULL) {
	err(25, __func__, "str is NULL");
	not_reached();
    }
     ...
     */
    if (array_has_path(array, str)) {
        /* str found in array */
        err(26, __func__, "string %s already in array", str);
        not_reached();
    }
...
scan_topdir(char *args, struct info *infop, char const *make, char const *submission_dir, RuleCount *size)
{
    ...
    char *submit_path = NULL;       /* we need the absolute path to the submission directory for copying files */
    ...
    if (args == NULL || infop == NULL || make == NULL || submission_dir == NULL || size == NULL) {
        err(34, __func__, "passed NULL pointer(s)");
        not_reached();
    }
    ...
    if (infop->ignored_symlinks == NULL) {
        err(35, __func__, "failed to create ignored symlinks list array");
        not_reached();
    }
    /*
    ...
    if (infop->extra_files == NULL) {
        err(36, __func__, "failed to create files list array");
        not_reached();
    }
    /*
    ...
    if (infop->required_files == NULL) {
        err(37, __func__, "failed to create required files list array");
        not_reached();
    }
    /*
    ...
    if (chdir(submission_dir) != 0) {
        errp(44, __func__, "unable to change to submission directory");
        not_reached();
    }
    ...
    if (submit_path == NULL) {
        errp(45, __func__, "couldn't get absolute path of submission directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(46, __func__, "couldn't change back to original original directory");
        not_reached();
    }
    ...
    errno = 0;      /* pre-clear errno for errp() */
    if (chdir(args) != 0) {
        errp(47, __func__, "chdir(\"%s\") failed", args);
        not_reached();
    }
    /*
    ...
    if (topdir < 0) {
	errp(48, __func__, "cannot open topdir");
	not_reached();
    }
    ...
    if (topdir_path == NULL) {
        errp(49, __func__, "couldn't get absolute path of topdir");
        not_reached();
    }
    ...
    if (ent == NULL){
        ...
        do {
             ...
             */
            if (array_has_path(infop->ignore_paths, ent->fts_path + 2)) {
                /*
            ...
            switch (sanity) {
                ...
                case PATH_ERR_NOT_RELATIVE: /* path not relative: starts with '/' */
                    err(58, __func__, "%s: path not relative", ent->fts_path + 2);
                    not_reached();
                    break;
                ...
                case PATH_ERR_MAX_NAME_LEN_0: /* max name length <= 0 (should never happen) */
                    err(59, __func__, "%s: %s", ent->fts_path + 2, path_sanity_error(sanity));
                    not_reached();
                    break;
                ...
                default: /* should never be reached */
                     ...
                     */
                    err(60, __func__, "unknown status for %s", ent->fts_path + 2);
                    not_reached();
                    break;
            }
            if (ignored_dirname) {
            ...
            switch (ent->fts_info) {
                case FTS_D: /* directory */
                    ...
                    if (optional) {
                         ...
                         */
                        err(64, __func__, "directory name matches optional filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    if (mandatory_filename) {
                         ...
                         */
                        err(65, __func__, "directory name matches required filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
    ...
    if (count > MAX_EXTRA_FILE_COUNT) {
        err(69, __func__, "too many files: %ju > %ju", (uintmax_t)count, (uintmax_t)MAX_FILE_COUNT);
        not_reached();
    }
    ...
    if (MAX_EXTRA_DIR_COUNT > 0 && dirs + unsafe_dirs > MAX_EXTRA_DIR_COUNT) {
        err(70, __func__, "too many extra directories: %ju > %ju", (uintmax_t)(dirs + unsafe_dirs),
     ...
     */
    if (!found_prog_c) {
        err(71, __func__, "prog.c not found in topdir %s", args);
        not_reached();
    } else if (!found_Makefile) {
        err(72, __func__, "Makefile not found in topdir %s", args);
        not_reached();
    } else if (!found_remarks_md) {
        err(73, __func__, "remarks.md not found in topdir %s", args);
        not_reached();
    }
 ...
 * well the check_submission() function will be called.
 *
        ...
        char *submit_path, int topdir, int cwd, RuleCount *size)
{
    ...
    if (infop == NULL || make == NULL || submission_dir == NULL || topdir_path == NULL || submit_path == NULL || size == NULL) {
        err(74, __func__, "passed NULL pointer(s)");
        not_reached();
    }
    if (topdir < 0) {
        err(75, __func__, "passed invalid topdir file descriptors");
        not_reached();
    }
    if (cwd < 0) {
        err(76, __func__, "passed invalid cwd file descriptors");
        not_reached();
    }
     ...
     *
     * NOTE: the infop->ignore_paths MAY be NULL so we don't check for this here
     * (it is only created if the -I path option was used)
     */
    if (infop->ignored_symlinks == NULL) {
        err(77, __func__, "NULL ignored symlinks list array");
        not_reached();
    }
    if (infop->extra_files == NULL) {
        err(78, __func__, "NULL files list array");
        not_reached();
    }
    if (infop->required_files == NULL) {
        err(79, __func__, "NULL required files list array");
        not_reached();
    }
    if (infop->directories == NULL) {
        err(80, __func__, "NULL directories list array");
        not_reached();
    }
    if (infop->ignored_dirs == NULL) {
        err(81, __func__, "NULL ignored directories list array");
        not_reached();
    }
    if (infop->forbidden_files == NULL) {
        err(82, __func__, "NULL forbidden filenames list array");
        not_reached();
    }
    if (infop->unsafe_files == NULL) {
        err(83, __func__, "NULL unsafe filenames list array");
        not_reached();
    }
    if (infop->unsafe_dirs == NULL) {
        err(84, __func__, "NULL unsafe directory names list array");
        not_reached();
    }
    /*
    ...
    if (array_has_path(infop->extra_files, PROG_C_FILENAME)) {
        err(85, __func__, "extra files list has required file prog.c");
        not_reached();
    } else if (array_has_path(infop->directories, PROG_C_FILENAME)) {
        err(86, __func__, "directories list has required file prog.c");
        not_reached();
    } else if (array_has_path(infop->ignored_symlinks, PROG_C_FILENAME)) {
        err(87, __func__, "ignored symlinks list has required file prog.c");
        not_reached();
    }
    if (array_has_path(infop->extra_files, MAKEFILE_FILENAME)) {
        err(88, __func__, "extra files list has required file Makefile");
        not_reached();
    } else if (array_has_path(infop->directories, MAKEFILE_FILENAME)) {
        err(89, __func__, "directories list has required file Makefile");
        not_reached();
    } else if (array_has_path(infop->ignored_symlinks, MAKEFILE_FILENAME)) {
        err(90, __func__, "ignored symlinks list has required file Makefile");
        not_reached();
    }
    if (array_has_path(infop->extra_files, REMARKS_FILENAME)) {
        err(91, __func__, "extra files list has required file remarks.md");
        not_reached();
    } else if (array_has_path(infop->directories, REMARKS_FILENAME)) {
        err(92, __func__, "directories list has required file remarks.md");
        not_reached();
    } else if (array_has_path(infop->ignored_symlinks, REMARKS_FILENAME)) {
        err(93, __func__, "ignored symlinks list has required file remarks.md");
        not_reached();
    }
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
        errp(94, __func__, "unable to fchdir(cwd)");
        not_reached();
    }
    ...
    if (infop->ignore_paths != NULL) {
        ...
        if (len > 0) {
            ...
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(95, __func__, "found NULL pointer in ignored dirname list, element: %ju", (uintmax_t)i);
                    not_reached();
                }
                print("%s\n", p);
            }
            if (!answer_yes && !read_answers_flag_used) {
                ...
                if (!yorn) {
                    print("we suggest you delete %s and try again\nwith the correct options used\n",
                            submit_path);
                    err(96, __func__, "aborting because user said requested ignored paths list is not OK");
                    not_reached();
                }
            }
        }
    ...
    if (len > 0) {
        if (need_hints) {
            ...
            for (i = 0; ignored_dirnames[i] != NULL; ++i) {
                ...
                if (ret <= 0) {
                    errp(97, __func__, "printf error printing an ignored dirname: %s", ignored_dirnames[i]);
                    not_reached();
                }
            }
            para("",
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(98, __func__, "found NULL pointer in ignored dirname list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                ...
                err(99, __func__, "aborting because user said ignored directories list is not OK");
                not_reached();
            }
        }
    }
    ...
    if (len > 0) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(100, __func__, "found NULL pointer in unsafe directory names list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(101, __func__, "aborting because user said unsafe directory names list is not OK");
                not_reached();
            }
        }
    }
    ...
    if (len > 0) {
        if (need_hints) {
            ...
            for (i = 0; forbidden_filenames[i] != NULL; ++i) {
                ...
                if (ret <= 0) {
                    errp(102, __func__, "printf error printing a forbidden filename: %s", forbidden_filenames[i]);
                    not_reached();
                }
            }
            para("",
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(103, __func__, "found NULL pointer in forbidden files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(104, __func__, "aborting because user said forbidden files list is not OK");
                not_reached();
            }
        }
    }
    ...
    if (len > 0) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(105, __func__, "found NULL pointer in unsafe filenames list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(106, __func__, "aborting because user said unsafe filenames list is not OK");
                not_reached();
            }
        }
    }
    ...
    if (len > 0) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(107, __func__, "found NULL pointer in ignored symlinks list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(108, __func__, "aborting because user said ignored symlinks list is not OK");
                not_reached();
            }
        }
    }
    ...
    if (len > 0) {
        ...
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(110, __func__, "aborting because user said directories list is not OK");
                not_reached();
            }
        }
    }
    ...
    if (len <= 0) {
        err(111, __func__, "list of required files is empty");
        not_reached();
    }
    if (len > 0) {
        para("",
        ...
        if (len > 0) {
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(113, __func__, "found NULL pointer in extra files list, element: %ju", (uintmax_t)i);
                    not_reached();
                }
                print("%s\n", p);
            }
        ...
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(114, __func__, "aborting because user said files list is not OK");
                not_reached();
            }
        }
        ...
        if (len > 0) {
            ...
            errno = 0;      /* pre-clear errno for errp() */
            if (chdir(submission_dir) != 0) {
                errp(115, __func__, "chdir(\"%s\") failed", submission_dir);
                not_reached();
            }
            ...
            for (i = 0; i < len; ++i) {
                ...
                p = dyn_array_value(infop->directories, char *, i);
                if (p == NULL) {
                    err(116, __func__, "found NULL pointer in infop->directories list");
                    not_reached();
                }
                /*
        ...
        errno = 0;
        if (fchdir(topdir) != 0) {
            errp(117, __func__, "cannot change to topdir");
            not_reached();
        }
        ...
        len = dyn_array_tell(infop->required_files);
        if (len <= 0) {
            err(118, __func__, "list of required files is empty");
            not_reached();
        }
        for (i = 0; i < len; ++i) {
            p = dyn_array_value(infop->required_files, char *, i);
            if (p == NULL) {
                err(119, __func__, "found NULL pointer in required files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            /*
            ...
            if (fname == NULL) {
                err(120, __func__, "couldn't allocate path to copy");
                not_reached();
            }
            if (target_path != NULL) {
                free(target_path);
            ...
            if (target_path == NULL) {
                errp(121, __func__, "failed to allocate target path for %s", p);
                not_reached();
            }
            /*
             * create target path
            ...
            if (ret <= 0) {
                errp(122, __func__, "snprintf to form target path for %s failed", fname);
                not_reached();
            }
        ...
        for (i = 0; i < len; ++i) {
            p = dyn_array_value(infop->extra_files, char *, i);
            if (p == NULL) {
                err(123, __func__, "found NULL pointer in non-required files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            /*
             * we have to allocate the full path of the file under topdir.
            ...
            fname = calloc_path(topdir_path, p);
            if (fname == NULL) {
                err(124, __func__, "couldn't allocate path to copy");
                not_reached();
            }
            if (target_path != NULL) {
                free(target_path);
            ...
            target_path = calloc(1, strlen(submit_path) + LITLEN("/") + strlen(p) + 1);
            if (target_path == NULL) {
                errp(125, __func__, "failed to allocate target path for %s", p);
                not_reached();
            }
            /*
             */
            ...
            ret = snprintf(target_path, strlen(submit_path) + 1 + strlen(p) + 1, "%s/%s", submit_path, p);
            if (ret <= 0) {
                errp(126, __func__, "snprintf to form target path for %s failed", fname);
                not_reached();
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (close(topdir) != 0) {
        errp(128, __func__, "failed to close(topdir)");
        not_reached();
    }
 ...
 */
static void
check_submission(struct info *infop, char *submit_path, char *topdir_path,
        char const *make, RuleCount *size, int cwd)
{
    ...
    if (infop == NULL || submit_path == NULL || make == NULL || size == NULL) {
        err(129, __func__, "passed NULL arg(s)");
        not_reached();
    }
    /*
     * cwd must be >= 0
     */
    if (cwd < 0) {
        err(130, __func__, "original directory file descriptor < 0");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(131, __func__, "failed to change to original directory");
        not_reached();
    }
     ...
     * function makes these and uses them and so does copy_topdir() we still
     * insist they exist at this point).
     */
    if (infop->ignored_symlinks == NULL) {
        err(132, __func__, "NULL ignored symlinks list array");
        not_reached();
    }
    if (infop->extra_files == NULL) {
        err(133, __func__, "NULL files list array");
        not_reached();
    }
    if (infop->required_files == NULL) {
        err(134, __func__, "NULL required files list array");
        not_reached();
    }
    if (infop->directories == NULL) {
        err(135, __func__, "NULL directories list array");
        not_reached();
    }
    if (infop->ignored_dirs == NULL) {
        err(136, __func__, "NULL ignored directories list array");
        not_reached();
    }
    if (infop->forbidden_files == NULL) {
        err(137, __func__, "NULL forbidden filenames list array");
        not_reached();
    }
    if (infop->unsafe_files == NULL) {
        err(138, __func__, "NULL unsafe filenames list array");
        not_reached();
    }
    if (infop->unsafe_dirs == NULL) {
        err(139, __func__, "NULL unsafe directory names list array");
        not_reached();
    }
    ...
    if (required_files == NULL) {
        err(140, __func__, "couldn't create required files list array");
        not_reached();
    }
    extra_files = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (extra_files == NULL) {
        err(141, __func__, "couldn't create extra files list array");
        not_reached();
    }
    missing_files = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (missing_files == NULL) {
        err(142, __func__, "couldn't create missing files list array");
        not_reached();
    }
    directories = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (directories == NULL) {
        err(143, __func__, "couldn't create directories list array");
        not_reached();
    }
    missing_dirs = dyn_array_create(sizeof(char *), CHUNK, CHUNK, true);
    if (missing_dirs == NULL) {
        err(144, __func__, "couldn't create missing directories list array");
        not_reached();
    }
    ...
    if (chdir(submit_path) != 0) {
        errp(145, __func__, "unable to change to submission directory: %s", submit_path);
        not_reached();
    }
     ...
     */
    if (!is_read("Makefile")) {
        err(146, __func__, "Makefile not a regular readable file in submission directory %s", submit_path);
        not_reached();
    }
    /*
    ...
    if (ent == NULL){
        ...
        do {
            ...
            if (ignored_dirname) {
                err(148, __func__, "found ignored directory in submission directory");
                not_reached();
            } else if (forbidden_filename) {
                err(149, __func__, "found forbidden file in submission directory");
                not_reached();
            }
            /*
             * NOTE: when traversing the directory "." the filenames found under
            ...
            switch (sanity) {
                case PATH_ERR_NAME_TOO_LONG: /* last component too long */
                    err(150, __func__, "%s: name too long: strlen(\"%s\"): %ju > %ju", ent->fts_name, ent->fts_name,
                            (uintmax_t)strlen(ent->fts_name), (uintmax_t)MAX_FILENAME_LEN);
                    not_reached();
                    break;
                case PATH_ERR_PATH_TOO_LONG: /* entire path too long */
                    err(151, __func__, "%s: path too long: strlen(\"%s\"): %ju > %ju", ent->fts_path + 2, ent->fts_path,
                            (uintmax_t)strlen(ent->fts_path), (uintmax_t)MAX_FILENAME_LEN);
                    not_reached();
                    break;
                case PATH_ERR_PATH_TOO_DEEP: /* too many subdirectories */
                    err(152, __func__, "%s: path too deep: depth %ju > %ju", ent->fts_path + 2,
                            (uintmax_t)count_dirs(ent->fts_path), (uintmax_t)MAX_PATH_DEPTH);
                    not_reached();
                    break;
                case PATH_ERR_NOT_POSIX_SAFE: /* not sane relative path */
                    err(153, __func__, "%s: path not POSIX plus + safe", ent->fts_path + 2);
                    not_reached();
                    break;
                case PATH_ERR_NOT_RELATIVE: /* path not relative: starts with '/' */
                    err(154, __func__, "%s: path not relative", ent->fts_path + 2);
                    not_reached();
                    break;
                case PATH_ERR_UNKNOWN: /* unknown error */
                case PATH_ERR_PATH_IS_NULL: /* path is NULL (should never happen) */
                ...
                case PATH_ERR_MAX_DEPTH_0: /* max depth <= 0 (should never happen) */
                case PATH_ERR_MAX_NAME_LEN_0: /* max name length <= 0 (should never happen) */
                    err(155, __func__, "%s: %s", ent->fts_path + 2, path_sanity_error(sanity));
                    not_reached();
                    break;
                case PATH_OK: /* sane relative path */
                    break;
                default: /* should never be reached */
                     ...
                     * something is quite wrong here
                     */
                    err(156, __func__, "unknown status for %s", ent->fts_path);
                    not_reached();
                    break;
            }
            ...
            switch (ent->fts_info) {
                case FTS_D: /* directory */
                    ...
                    if (optional) {
                         ...
                         * actually optional filenames
                         */
                        err(157, __func__, "directory name matches optional filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    if (mandatory_filename) {
                        /*
                         ...
                         * actually required filenames either
                         */
                        err(158, __func__, "directory name matches required filename: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    /*
                    ...
                    if (!array_has_path(infop->directories, ent->fts_path + 2)) {
                        err(159, __func__, "directory in submission directory not found in topdir: %s", ent->fts_path + 2);
                        not_reached();
                    }
                    /*
                     * extra sanity checks
                    ...
                    if (!is_dir(ent->fts_path + 2)) {
                        err(160, __func__, "fts_read() found %s as directory but is_dir() returned false", ent->fts_path + 2);
                        not_reached();
                    } else if (!is_mode(ent->fts_path + 2, 0755)) {
                        err(161, __func__, "directory %s: mode %o != 0755", ent->fts_path + 2, filemode(ent->fts_path + 2));
                        not_reached();
                    }
                    /*
                    ...
                    if (!is_mode(ent->fts_path + 2, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
                        err(162, __func__, "directory %s must be mode 0444: %o != 0444", ent->fts_path + 2,
                    ...
                    filename = strdup(ent->fts_path + 2);
                    if (filename == NULL) {
                        errp(163, __func__, "strdup(\"%s\") failed", ent->fts_path + 2);
                        not_reached();
                    }
                    /*
                ...
                case FTS_F:
                    ...
                    if (!is_file(ent->fts_path + 2)) {
                        err(164, __func__, "fts_read() found %s as file but is_file() returned false", ent->fts_path + 2);
                        not_reached();
                    }
                    ...
                    filename = strdup(ent->fts_path + 2);
                    if (filename == NULL) {
                        errp(165, __func__, "strdup(\"%s\") failed", ent->fts_path + 2);
                        not_reached();
                    }
                    ...
                    if (strcmp(filename, PROG_C_FILENAME) && strcmp(filename, MAKEFILE_FILENAME) &&
                        strcmp(filename, REMARKS_FILENAME)) {
                            ...
                            if (!strcmp(filename, TRY_SH) || !strcmp(filename, TRY_ALT_SH)) {
                                if (!is_mode(filename, S_IRUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
                                     ...
                                     */
                                    err(167, __func__, "file %s must be mode 0555: %o != 0555",
                                ...
                                if (!is_mode(filename, S_IRUSR | S_IRGRP | S_IROTH)) {
                                    err(168, __func__, "file %s must be mode 0444: %o != 0444",
                                            filename, filemode(filename));
                                    not_reached();
                                }
                            }
                        ...
                        if (!array_has_path(infop->required_files, filename)) {
                            err(169, __func__, "file in submission directory not in topdir: %s", filename);
                            not_reached();
                        }
                        /*
                         ...
                         */
                        if (!is_mode(filename, S_IRUSR | S_IRGRP | S_IROTH)) {
                            err(170, __func__, "file %s must be mode 0444: %o != 0444", filename,
                                    filemode(filename));
                            not_reached();
                        }
                ...
                case FTS_SL: /* symlink */
                case FTS_SLNONE: /* symlink with non-existing target */
                    err(171, __func__, "found symlink in submission directory");
                    not_reached();
                    break;
                default:
                    break;
     ...
     */
    if (!found_prog_c) {
        err(172, __func__, "prog.c not found in submission directory %s", submit_path);
        not_reached();
    } else if (!found_Makefile) {
        err(173, __func__, "Makefile not found in submission directory %s", submit_path);
        not_reached();
    } else if (!found_remarks_md) {
        err(174, __func__, "remarks.md not found in submission directory %s", submit_path);
        not_reached();
    }
     ...
     */
    if (count > MAX_EXTRA_FILE_COUNT) {
        err(175, __func__, "too many files: %ju > %ju", (uintmax_t)count, (uintmax_t)MAX_FILE_COUNT);
        not_reached();
    }
    /*
     * verify that there are not too many directories if MAX_EXTRA_DIR_COUNT > 0.
    ...
    if (MAX_EXTRA_DIR_COUNT > 0 && len > MAX_EXTRA_DIR_COUNT) {
        err(176, __func__, "too many extra directories in submission directory: %ju > %ju", (uintmax_t)len,
                (uintmax_t)MAX_EXTRA_DIR_COUNT);
        not_reached();
    }
    ...
    len = dyn_array_tell(infop->required_files);
    if (len <= 0) {
        err(177, __func__, "list of required files list is empty");
        not_reached();
    }
    len2 = dyn_array_tell(required_files);
    if (len2 <= 0) {
        err(178, __func__, "list of required files in submission directory is empty");
        not_reached();
    }
    /*
     ...
     */
    if (len != len2) {
        err(179, __func__, "size of required files list in submission directory != size in topdir: %ju != %ju", (uintmax_t)len2,
                (uintmax_t)len);
        not_reached();
    }
    for (i = 0; i < len; ++i) {
        ...
        p = dyn_array_value(infop->required_files, char *, i);
        if (p == NULL) {
            err(180, __func__, "found NULL pointer in required files list, element: %ju", (uintmax_t)i);
            not_reached();
        }
        /*
        ...
        if (fname == NULL) {
            err(181, __func__, "found NULL pointer in required files list in submission directory, element: %ju", (uintmax_t)i);
            not_reached();
        }
         ...
         */
        if (strcmp(p, fname) != 0 || !array_has_path(required_files, p)) {
            /*
            ...
            filename = strdup(p);
            if (filename == NULL) {
                errp(182, __func__, "strdup(\"%s\") failed", p);
                not_reached();
            }
        ...
        if (array_has_path(directories, p)) {
            err(183, __func__, "required file %s in directories list", p);
            not_reached();
        }
        if (array_has_path(extra_files, p)) {
            err(184, __func__, "required file %s in extra files list", p);
            not_reached();
        }
        ...
        if (!strcmp(fname, PROG_C_FILENAME)) {
            ...
            *size = check_prog_c(infop, fname);
             ...
             */
            err(185, __func__, "BUG: non-required file found in required files list: %s, please report", fname);
            not_reached();
        }
    }
    ...
    if (array_has_path(missing_files, PROG_C_FILENAME)) {
        err(186, __func__, "prog.c in missing files list!");
        not_reached();
    }
    if (array_has_path(missing_files, MAKEFILE_FILENAME)) {
        err(187, __func__, "Makefile in missing files list!");
        not_reached();
    }
    if (array_has_path(missing_files, REMARKS_FILENAME)) {
        err(188, __func__, "remarks.md in missing files list!");
        not_reached();
    }
     ...
     */
    if (len != len2) {
        err(189, __func__, "size of extra files list in submission directory != size in topdir: %ju != %ju", (uintmax_t)len2,
                (uintmax_t)len);
        not_reached();
    }
    if (len > 0) {
        for (i = 0; i < len; ++i) {
            ...
            p = dyn_array_value(infop->extra_files, char *, i);
            if (p == NULL) {
                err(190, __func__, "found NULL pointer in extra files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            /*
            ...
            if (p == NULL) {
                err(191, __func__, "found NULL pointer in extra files list in submission directory, element: %ju", (uintmax_t)i);
                not_reached();
            }
             ...
             * it helps to document our purpose (a tiny bit?).
             */
            if (strcmp(p, fname) != 0 || !array_has_path(extra_files, p)) {
                /*
                 * we need to add this file to the missing files list
                 */
                errno = 0; /* pre-clear errno for errp() */
                filename = strdup(p);
                if (filename == NULL) {
                    errp(192, __func__, "strdup(\"%s\") failed", p);
                    not_reached();
                }
                append_unique_filename(missing_files, filename);
            }
    ...
    if (len > 0) {
        ...
        if (ret <= 0) {
            errp(193, __func__, "error writing missing files list title");
            not_reached();
        }
        for (i = 0; i < len; i++) {
            /*
            ...
            if (fname == NULL) {
                err(194, __func__, "found NULL pointer in missing files list in submission directory, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", fname);
        }
        err(195, __func__, "aborting due to missing files");
        not_reached();
    }
     ...
     */
    if (len != len2) {
        err(196, __func__, "size of directories list in submission directory != size in topdir: %ju != %ju", (uintmax_t)len2,
                (uintmax_t)len);
        not_reached();
    }
    if (len > 0) {
        for (i = 0; i < len; ++i) {
            p = dyn_array_value(infop->directories, char *, i);
            if (p == NULL) {
                err(197, __func__, "found NULL pointer in directories list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            fname = dyn_array_value(directories, char *, i);
            if (fname == NULL) {
                err(198, __func__, "found NULL pointer in directories list in submission directory, element: %ju", (uintmax_t)i);
                not_reached();
            }
             ...
             * it helps to document our purpose (a tiny bit?).
             */
            if (strcmp(fname, p) != 0 || !array_has_path(directories, p)) {
                /*
                ...
                filename = strdup(p);
                if (filename == NULL) {
                    errp(199, __func__, "strdup(\"%s\") failed", p);
                    not_reached();
                }
    ...
    if (len > 0) {
        ...
        if (ret <= 0) {
            errp(200, __func__, "error writing missing directories list title");
            not_reached();
        }
        for (i = 0; i < len; i++) {
            /*
        ...
        err(202, __func__, "aborting due to missing directories");
        not_reached();
    }
    ...
    if (len > 0) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(203, __func__, "found NULL pointer in directories list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        ...
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(204, __func__, "aborting because user said directories list in submission directory is not OK");
                not_reached();
            }
        }
    }
    ...
    if (len <= 0) {
        err(205, __func__, "list of required files is empty");
        not_reached();
    }
    if (len > 0) {
        para("",
        ...
        for (i = 0; i < len; ++i) {
            ...
            if (p == NULL) {
                err(206, __func__, "found NULL pointer in required files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            print("%s\n", p);
        }
        ...
        if (len > 0) {
            for (i = 0; i < len; ++i) {
                ...
                if (p == NULL) {
                    err(207, __func__, "found NULL pointer in non-required files list, element: %ju", (uintmax_t)i);
                    not_reached();
                }
                print("%s\n", p);
            }
        ...
        if (!answer_yes && !read_answers_flag_used) {
            ...
            if (!yorn) {
                print("we suggest you fix your %s directory,\ndelete %s and try again\n",
                        topdir_path, submit_path);
                err(208, __func__, "aborting because user said files list in submission directory is not OK");
                not_reached();
            }
        }
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(209, __func__, "unable to change to previous directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (close(cwd) != 0) {
        errp(210, __func__, "failed to close(cwd)");
        not_reached();
    }
}
           ...
           char const *make)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(212, __func__, "tar does not exist: %s", tar);
	not_reached();
    }
    if (!is_file(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(213, __func__, "tar is not a regular file: %s", tar);
	not_reached();
    }
    if (!is_exec(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(214, __func__, "tar is not an executable program: %s", tar);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(215, __func__, "ls does not exist: %s", ls);
	not_reached();
    }
    if (!is_file(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(216, __func__, "ls is not a regular file: %s", ls);
	not_reached();
    }
    if (!is_exec(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(217, __func__, "ls is not an executable program: %s", ls);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(218, __func__, "txzchk does not exist: %s", txzchk);
	not_reached();
    }
    if (!is_file(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(219, __func__, "txzchk is not a regular file: %s", txzchk);
	not_reached();
    }
    if (!is_exec(txzchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(220, __func__, "txzchk is not an executable program: %s", txzchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(221, __func__, "fnamchk does not exist: %s", fnamchk);
	not_reached();
    }
    if (!is_file(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(222, __func__, "fnamchk is not a regular file: %s", fnamchk);
	not_reached();
    }
    if (!is_exec(fnamchk)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(223, __func__, "fnamchk is not an executable program: %s", fnamchk);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(224, __func__, "chkentry does not exist: %s", chkentry);
	not_reached();
    }
    if (!is_file(chkentry)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(225, __func__, "chkentry is not a regular file: %s", chkentry);
	not_reached();
    }
    if (!is_exec(chkentry)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(226, __func__, "chkentry is not an executable program: %s", chkentry);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(227, __func__, "make does not exist: %s", make);
	not_reached();
    }
    if (!is_file(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(228, __func__, "make is not a regular file: %s", make);
	not_reached();
    }
    if (!is_exec(make)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(229, __func__, "make is not an executable program: %s", make);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(230, __func__, "workdir does not exist: %s", workdir);
	not_reached();
    }
    if (!is_dir(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(231, __func__, "workdir is not a directory: %s", workdir);
	not_reached();
    }
    if (!is_write(workdir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(232, __func__, "workdir is not a writable directory: %s", workdir);
	not_reached();
    }
...
prompt(char const *str, size_t *lenp)
{
     ...
     */
    if (str == NULL) {
	err(233, __func__, "called with NULL str");
	not_reached();
    }
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(234, __func__, "error printing prompt string");
		not_reached();
	    } else if (feof(stdout)) {
		err(235, __func__, "EOF while printing prompt string");
		not_reached();
	    } else {
		errp(236, __func__, "unexpected fputs error printing prompt string");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(237, __func__, "error printing :<space>");
		not_reached();
	    } else if (feof(stdout)) {
		err(238, __func__, "EOF while writing :<space>");
		not_reached();
	    } else {
		errp(239, __func__, "unexpected fputs error printing :<space>");
		not_reached();
	    }
	}
	clearerr(stdout);		/* pre-clear ferror() status */
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(240, __func__, "error flushing prompt to stdout");
		not_reached();
	    } else if (feof(stdout)) {
		err(241, __func__, "EOF while flushing prompt to stdout");
		not_reached();
	    } else {
		errp(242, __func__, "unexpected fflush error while flushing prompt to stdout");
		not_reached();
	    }
	}
    }
    ...
    if (buf == NULL) {
	err(243, __func__, "EOF while reading prompt input");
	not_reached();
    }
...
get_contest_id(bool *testp)
{
     ...
     */
    if (testp == NULL) {
	err(244, __func__, "called with NULL testp");
	not_reached();
    }
...
get_submit_slot(struct info *infop)
{
    ...
    if (need_hints) {
        ...
        if (ret <= 0) {
            errp(247, __func__, "printf error printing number of submissions allowed");
            not_reached();
        }
        para("",
	...
	if (ret != 1 || submit_slot < 0 || submit_slot > MAX_SUBMIT_SLOT) {
	    ...
	    ret = fprintf(stderr, "\nThe submit slot number must be a number from 0 through %d; please re-enter.\n",
		    MAX_SUBMIT_SLOT);
	    if (ret <= 0) {
		errp(248, __func__, "fprintf error while informing about the valid submit slot number range");
                not_reached();
	    }
            /*
             * free storage
	     ...
	     char **tarball_path, time_t tstamp, bool test_mode)
{
    ...
    if (test == false) {
	err(11, __func__, "submit slot number: %d must >= 0 and <= %d", submit_slot, MAX_SUBMIT_SLOT);
	not_reached();
    }
    ...
    if (submission_dir == NULL) {
	errp(12, __func__, "malloc #0 of %ju bytes failed", (uintmax_t)(submission_dir_len + 1));
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(submission_dir, submission_dir_len + 1, "%s/%s-%d", workdir, ioccc_id, submit_slot);
    if (ret <= 0) {
	errp(13, __func__, "snprintf to form submission directory failed");
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(14, __func__, "fprintf error while informing that the submission directory already exists");
            not_reached();
	}
	fpara(stderr,
	      "",
	      ...
	      "",
	      NULL);
	err(15, __func__, "submission directory exists: %s", submission_dir);
	not_reached();
    }
    dbg(DBG_HIGH, "submission directory path: %s", submission_dir);
    ...
    if (ret < 0) {
	errp(16, __func__, "cannot mkdir %s", submission_dir);
	not_reached();
    }
    errno = 0; /* pre-clear errno for errp() */
    ret = chmod(submission_dir, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
    if (ret < 0) {
        errp(17, __func__, "cannot chmod directory %s to mode 0755", submission_dir);
        not_reached();
    }
    ...
    *tarball_path = form_tar_filename(ioccc_id, submit_slot, test_mode, tstamp);
    if (*tarball_path == NULL) {
	errp(18, __func__, "failed to form compressed tarball path");
	not_reached();
    }
...
warn_rule_2a_size(struct info *infop, int mode, RuleCount size)
{
     ...
     */
    if (infop == NULL) {
	err(20, __func__, "called with NULL infop");
	not_reached();
    }
	...
	ret = fprintf(stderr, "\nWARNING: The prog.c size: %jd > Rule 2a maximum: %jd\n",
		      (intmax_t)infop->rule_2a_size, (intmax_t)RULE_2A_SIZE);
	if (ret <= 0) {
	    errp(21, __func__, "fprintf error when printing prog.c Rule 2a warning");
            not_reached();
	}
	if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes)) {
	    fpara(stderr,
	...
	if (abort_on_warning || (need_confirm && !ignore_warnings && !answer_yes)) {
	    ...
	    if (ret <= 0) {
		errp(23, __func__, "fprintf error when printing prog.c file size and Rule 2a mismatch");
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	err(25, __func__, "invalid mode passed to function: %d", mode);
	not_reached();
    }
    return;
}
...
warn_nul_chars(void)
{
	...
	if (ret <= 0) {
	    errp(26, __func__, "fprintf error when printing prog.c nul_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
warn_trigraph(void)
{
	...
	if (ret <= 0) {
	    errp(28, __func__, "fprintf error when printing prog.c trigraph_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
warn_wordbuf(void)
{
	...
	ret = fprintf(stderr, "\nprog.c triggered a word buffer overflow!\n"
			      ...
			      "YOUR remarks.md FILE!\n\n");
	if (ret <= 0) {
	    errp(30, __func__, "fprintf error when printing prog.c wordbuf_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
warn_ungetc(void)
{
	...
	ret = fprintf(stderr, "\nprog.c triggered an ungetc error: @SirWumpus goofed\n"
			      ...
			      "YOUR remarks.md FILE!\n\n");
	if (ret <= 0) {
	    errp(32, __func__, "fprintf error when printing prog.c ungetc_warning");
            not_reached();
	}
	if (abort_on_warning) {
	    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
warn_rule_2b_size(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(34, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(35, __func__, "printf error printing prog.c size > Rule 2b maximum");
	    not_reached();
	}
...
check_prog_c(struct info *infop, char const *prog_c)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(38, __func__, "prog.c does not exist: %s", prog_c);
	not_reached();
    }
    if (!is_file(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(39, __func__, "prog.c is not a regular file: %s", prog_c);
	not_reached();
    }
    if (!is_read(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(40, __func__, "prog.c is not a readable file: %s", prog_c);
	not_reached();
    }
    ...
    if (prog_stream == NULL) {
	errp(41, __func__, "failed to fopen: %s", prog_c);
	not_reached();
    }
    ...
    if (ret != 0) {
	errp(42, __func__, "failed to fclose: %s", prog_c);
	not_reached();
    }
	...
	err(43, __func__, "file_size error: %jd on prog.c: %s", (intmax_t)infop->rule_2a_size, prog_c);
	not_reached();
...
inspect_Makefile(char const *Makefile, struct info *infop)
{
    ...
    if (stream == NULL) {
	errp(45, __func__, "cannot open Makefile: %s", Makefile);
	not_reached();
    }
...
warn_Makefile(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(47, __func__, "called with NULL infop");
	not_reached();
    }
...
check_Makefile(struct info *infop, char const *Makefile)
{
	...
	err(49, __func__, "called with NULL arg(s)");
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(50, __func__, "Makefile does not exist: %s", Makefile);
	not_reached();
    }
    if (!is_file(Makefile)) {
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(51, __func__, "Makefile is not a regular file: %s", Makefile);
	not_reached();
    }
    if (!is_read(Makefile)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(52, __func__, "Makefile is not readable file: %s", Makefile);
	not_reached();
    }
    filesize = file_size(Makefile);
    if (filesize < 0) {
	err(53, __func__, "file_size error: %jd on Makefile  %s", (intmax_t)filesize, Makefile);
	not_reached();
    } else if (filesize == 0) {
	err(54, __func__, "Makefile cannot be empty: %s", Makefile);
	not_reached();
    }
...
check_remarks_md(struct info *infop, char const *remarks_md)
{
	...
	err(55, __func__, "called with NULL arg(s)");
	not_reached();
    }
	...
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(56, __func__, "remarks.md does not exist: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr, "",
	      ...
	      "",
	      NULL);
	err(57, __func__, "remarks.md is not a regular file: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(58, __func__, "remarks.md is not readable file: %s", remarks_md);
	not_reached();
    }
    filesize = file_size(remarks_md);
    if (filesize < 0) {
	err(59, __func__, "file_size error: %jd on remarks.md %s", (intmax_t)filesize, remarks_md);
	not_reached();
    } else if (filesize == 0) {
	err(60, __func__, "remarks.md cannot be empty: %s", remarks_md);
	not_reached();
    }
...
yes_or_no(char const *question, bool def_answer)
{
     ...
     */
    if (question == NULL) {
	err(61, __func__, "called with NULL question");
	not_reached();
    }
...
get_title(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(62, __func__, "called with NULL infop");
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(63, __func__, "fprintf #0 error: %d", ret);
            not_reached();
	}
    }
	 ...
	 */
	    ...
	    ret = fprintf(stderr, "Your title must be between 1 and %d ASCII characters long.\n\n", MAX_TITLE_LEN);
	    if (ret <= 0) {
		errp(64, __func__, "fprintf #1 error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
get_abstract(struct info *infop)
{
     ...
     */
    if (infop == NULL) {
	err(66, __func__, "called with NULL infp");
	not_reached();
    }
	 ...
	 */
	    ...
	    if (ret <= 0) {
		errp(67, __func__, "fprintf error: %d", ret);
                not_reached();
	    }
	    if (abort_on_warning) {
		err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
...
get_author_info(struct author **author_set_p)
{
	...
	err(69, __func__, "called with NULL author_set_p");
	not_reached();
    }
	...
	if (ret != 1 || test_author_count(author_count) == false) {
	    ...
	    if (ret <= 0) {
		errp(70, __func__, "fprintf error #0 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "\nIf you happen to have more than %d authors, we ask that you pick\n", MAX_AUTHORS);
	    if (ret <= 0) {
		errp(71, __func__, "fprintf error #1 while printing author number range");
                not_reached();
	    }
	    errno = 0;		/* pre-clear errno for errp() */
	    ret = fprintf(stderr, "just %d authors and mention the remaining NUMBER of the authors in\nthe remarks file.\n",
                    MAX_AUTHORS);
	    if (ret <= 0) {
		errp(72, __func__, "fprintf error #2 while printing author number range");
                not_reached();
	    }
    ...
    if (author_set == NULL) {
	errp(73, __func__, "malloc a struct author array of length: %d failed", author_count);
	not_reached();
    }
	...
	if (ret < 0) {
	    errp(74, __func__, "puts error printing ISO 3166-1 URL0");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL1);
	if (ret < 0) {
	    errp(75, __func__, "puts error printing ISO 3166-1 URL1");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL2);
	if (ret < 0) {
	    errp(76, __func__, "puts error printing ISO 3166-1 URL2");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL3);
	if (ret < 0) {
	    errp(77, __func__, "puts error printing ISO 3166-1 URL3");
            not_reached();
	}
	errno = 0;		/* pre-clear errno for errp() */
	ret = puts(ISO_3166_1_CODE_URL4);
	if (ret < 0) {
	    errp(78, __func__, "puts error printing ISO 3166-1 URL4");
            not_reached();
	}
	para("",
	...
	if (ret <= 0) {
	    errp(79, __func__, "printf error printing author number");
            not_reached();
	}
	...
	do {
		...
		if (ret <= 0) {
		    errp(80, __func__, "fprintf error while reject name that is too long");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(81, __func__, "fprintf error while reject duplicate name");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(82, __func__, "fprintf while printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		...
		if (ret <= 0) {
		    errp(83, __func__, "fprintf while printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(84, __func__, "fprintf while printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(85, __func__, "fprintf while printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL0);
		if (ret <= 0) {
		    errp(86, __func__, "fprintf when printing ISO 3166-1 CODE URL #0");
                    not_reached();
		}
		fpara(stderr,
		      "or from these Wikipedia / ISO web pages:",
		...
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL1);
		if (ret <= 0) {
		    errp(87, __func__, "fprintf when printing ISO 3166-1 CODE URL #1");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n", ISO_3166_1_CODE_URL2);
		if (ret <= 0) {
		    errp(88, __func__, "fprintf when printing ISO 3166-1 CODE URL #2");
                    not_reached();
		}
		errno = 0;		/* pre-clear errno for errp() */
		ret = fprintf(stderr, "    %s\n\n", ISO_3166_1_CODE_URL3);
		if (ret <= 0) {
		    errp(89, __func__, "fprintf when printing ISO 3166-1 CODE URL #3");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(91, __func__, "fprintf error while printing Email address length limit");
                    not_reached();
		}
		fpara(stderr,
	...
	do {
		...
		if (ret <= 0) {
		    errp(92, __func__, "fprintf error while printing URL length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret = fprintf(stderr, "\nSorry ( tm Canada :-) ), we limit URLs to %d characters\n\n", MAX_URL_LEN);
		if (ret <= 0) {
		    errp(93, __func__, "fprintf error while printing URL length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		if (ret <= 0) {
		    errp(94, __func__, "fprintf error while printing mastodon handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr,
			    ...
			    MAX_GITHUB_LEN);
		if (ret <= 0) {
		    errp(95, __func__, "fprintf error while printing GitHub user length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
		...
		ret =
		    fprintf(stderr, "\nSorry ( tm Canada :-) ), we limit affiliation names to %d characters\n\n",
			    MAX_AFFILIATION_LEN);
		if (ret <= 0) {
		    errp(96, __func__, "fprintf error while printing affiliation length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	do {
	    ...
	    if (def_handle == NULL) {
		err(97, __func__, "default_handle() returned NULL!");
		not_reached();
	    }
		...
		if (ret <= 0) {
		    errp(98, __func__, "fprintf error while printing default IOCCC author handle");
                    not_reached();
		}
	    }
		...
		if (ret <= 0) {
		    errp(99, __func__, "fprintf error while printing IOCCC author handle length limit");
                    not_reached();
		}
		if (abort_on_warning) {
		    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
		...
		for (j=0; j < i; ++j) {
			...
			if (ret <= 0) {
			    errp(100, __func__, "fprintf error while printing duplicate author_handle error");
                            not_reached();
			}
			if (abort_on_warning) {
			    err(1, __func__, "-E forcing exit on 1st warning"); /*ooo*/
	...
	if (printf("\nPlease verify the information about author #%d\n\n", i) <= 0 ||
	    ...
	    errp(101, __func__, "error while printing author #%d information\n", i);
	    not_reached();
	}
	if (need_confirm) {
...
verify_submission_dir(char const *submission_dir, char const *ls)
{
    ...
    ret = printf("    %s\n", submission_dir);
    if (ret <= 0) {
	errp(103, __func__, "printf error code: %d", ret);
        not_reached();
    }
    para("",
    ...
    if (exit_code != 0) {
	err(104, __func__, "cd -- %s && %s -lakR . failed with exit code: %d",
    ...
    if (ls_stream == NULL) {
	err(105, __func__, "popen filed for: cd -- %s && %s -lakR .", submission_dir, ls);
	not_reached();
    }
	...
	err(106, __func__, "EOF while reading output of ls: %s", ls);
	not_reached();
    }
    /*
     ...
     */
    if (i == 0) {
        err(107, __func__, "found no k-block line in ls output");
        not_reached();
    }
    if (kdirsize <= 0) {
	err(108, __func__, "ls k-block value: %d <= 0", kdirsize);
	not_reached();
    }
	...
	if (!yorn) {
	    fpara(stderr,
		  ...
		  "We suggest you remove the existing submission directory (that we created,",
                  ...
                  "again.",
		  NULL);
	    err(109, __func__, "user rejected listing of submission directory: %s", submission_dir);
	    not_reached();
	}
    }
...
write_info(struct info *infop, char const *submission_dir, char const *chkentry, char const *fnamchk)
{
	...
	err(110, __func__, "called with NULL arg(s)");
	not_reached();
    }
    if (infop->extra_files == NULL) {
        err(111, __func__, "called with NULL files list");
        not_reached();
    }
    ...
    if (info_path == NULL) {
	errp(116, __func__, "malloc of %ju bytes failed", (uintmax_t)info_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ...
    if (info_stream == NULL) {
	errp(118, __func__, "failed to open for writing: %s", info_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        errp(119, __func__, "failed to obtain file descriptor for: %s", info_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(120, __func__, "fprintf error writing leading part of info to %s", info_path);
	not_reached();
    }
    ...
    if (!ret) {
	errp(121, __func__, "fprintf error writing mandatory filename to %s", info_path);
	not_reached();
    }
    ...
    if (infop->extra_count > 0) {
        ...
        for (i = 0; i < infop->extra_count; ++i) {
            p = dyn_array_value(infop->extra_files, char *, i);
            if (p == NULL) {
                err(122, __func__, "found NULL pointer in files list, element: %ju", (uintmax_t)i);
                not_reached();
            }
            ret =                   fprintf(info_stream, "        {\n") > 0 &&
            ...
            if (!ret) {
                errp(123, __func__, "fprintf error writing extra filename[%ju] to %s", (uintmax_t)i, info_path);
                not_reached();
            }
        }
    }
	...
	fprintf(info_stream, "}\n") > 0;
    if (!ret) {
	errp(124, __func__, "fprintf error writing trailing part of info to %s", info_path);
	not_reached();
    }
    ...
    if (ret < 0) {
	errp(125, __func__, "fclose error");
	not_reached();
    }
    ...
    if (exit_code != 0) {
	err(126, __func__, "%s -q -- . %s failed with exit code: %d",
    ...
    if (ret != 0) {
        err(128, __func__, "chmod(2) failed to set user, group and other read-only on %s", info_path);
        not_reached();
    }
...
form_auth(struct auth *authp, struct info *infop, int author_count, struct author *authorp)
{
	...
	err(130, __func__, "infop->ioccc_id is NULL");
	not_reached();
    }
    if (infop->tarball == NULL) {
	err(131, __func__, "infop->tarball is NULL");
	not_reached();
    }
    if (infop->utctime == NULL) {
	err(132, __func__, "infop->utctime is NULL");
	not_reached();
    }
	...
	errp(133, __func__, "strdup() ioccc_id path %s failed", infop->ioccc_id);
	not_reached();
    }
	...
	errp(134, __func__, "strdup() tarball path %s failed", infop->tarball);
	not_reached();
    }
	...
	errp(135, __func__, "strdup() utctime path %s failed", infop->utctime);
	not_reached();
    }
    return;
}
...
write_auth(struct auth *authp, char const *submission_dir, char const *chkentry, char const *fnamchk)
{
	...
	err(137, __func__, "author_count %d <= 0", authp->author_count);
	not_reached();
    } else if (authp->author_count > MAX_AUTHORS) {
	err(138, __func__, "author count %d > max authors %d", authp->author_count, MAX_AUTHORS);
	not_reached();
    }
    ...
    if (auth_path == NULL) {
	errp(139, __func__, "malloc of %ju bytes failed", (uintmax_t)auth_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ...
    if (auth_stream == NULL) {
	errp(141, __func__, "failed to open for writing: %s", auth_path);
	not_reached();
    }
    ...
    if (fd < 0) {
        err(142, __func__, "failed to obtain file descriptor for: %s", auth_path);
        not_reached();
    }
    ...
    if (!ret) {
	errp(143, __func__, "fprintf error writing leading part of authorship to %s", auth_path);
	not_reached();
    }
	...
	if (ret == false) {
	    errp(144, __func__, "fprintf error writing author %d info to %s", i, auth_path);
	    not_reached();
	}
    }
	...
	fprintf(auth_stream, "}\n") > 0;
    if (!ret) {
	errp(145, __func__, "fprintf error writing trailing part of authorship to %s", auth_path);
	not_reached();
    }
    ...
    if (ret < 0) {
	errp(146, __func__, "fclose error");
	not_reached();
    }
    ...
    if (exit_code != 0) {
	err(147, __func__, "%s -q -- %s . failed with exit code: %d",
    ...
    ret = fchmod(fd, S_IRUSR | S_IRGRP | S_IROTH);
    if (ret != 0) {
        err(148, __func__, "chmod(2) failed to set user, group and other read-only on %s", auth_path);
        not_reached();
    }
    ...
    ret = close(fd);
    if (ret < 0) {
        errp(149, __func__, "close(fd) failed");
        not_reached();
    }
	     ...
	     char const *ls, char const *txzchk, char const *fnamchk)
{
    ...
    if (entertain) {
        ...
        exit_code = shell_cmd(__func__, false, true, "% -e -w -v 1 -F % -- %/../%",
                                              txzchk, fnamchk, submission_dir, basename_tarball_path);
        if (exit_code != 0) {
            err(158, __func__, "%s -e -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
        ...
        exit_code = shell_cmd(__func__, false, true, "% -w -v 1 -F % -- %/../%",
                                              txzchk, fnamchk, submission_dir, basename_tarball_path);
        if (exit_code != 0) {
            err(159, __func__, "%s -w -v 1 -F %s -- %s/../%s failed with exit code: %d",
                               txzchk, fnamchk, submission_dir, basename_tarball_path, WEXITSTATUS(exit_code));
            not_reached();
        }
    }
        ...
        bool test_mode, int slot_number)
{
	...
	err(161, __func__, "failed to cmdprintf: submission_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(162, __func__, "printf #0 error");
	not_reached();
    }
    ...
    if (workdir_esc == NULL) {
	err(163, __func__, "failed to cmdprintf: workdir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(164, __func__, "printf #2 error");
	not_reached();
    }
    free(workdir_esc);
...
show_registration_url(void)
{
    ...
    ret = printf("    %s\n", IOCCC_REGISTER_URL);
    if (ret <= 0) {
	errp(165, __func__, "printf error printing IOCCC_REGISTER_URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(166, __func__, "printf error printing IOCCC register FAQ URL");
	not_reached();
    }
    para("",
    ...
    if (ret <= 0) {
	errp(167, __func__, "printf error printing IOCCC_REGISTER_INFO_URL, IOCCC_PW_CHANGE_INFO_URL and IOCCC_SUBMIT_INFO_URL");
	not_reached();
    }
    ...
    ret = printf("    %s\n", IOCCC_STATUS_URL);
    if (ret < 0) {
	errp(168, __func__, "printf error printing IOCCC status URL");
	not_reached();
    }
...
show_submit_url(char const *workdir, char const *tarball_path, int slot_number)
{
    ...
    if (ret <= 0) {
	errp(169, __func__, "printf error printing tarball path and slot number");
	not_reached();
    }
    para("",
    ...
    ret = printf("    %s\n", IOCCC_SUBMIT_URL);
    if (ret < 0) {
	errp(170, __func__, "printf error printing IOCCC submit URL");
	not_reached();
    }
    ...
    para("",
     ...
     ret = printf("    %s\n", IOCCC_ENTER_FAQ_URL);
    if (ret < 0) {
	errp(171, __func__, "printf error printing IOCCC enter FAQ URL");
	not_reached();
    }
}
