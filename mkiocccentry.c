...
main(int argc, char *argv[])
{
	...
	case 'v':		/* -v verbosity */
	    ...
	    if (errno != 0) {
		errp(20, __func__, "cannot parse -v arg: %s error: %s", optarg, strerror(errno));
		not_reached();
	    }
	    break;
    ...
    if (ret < 0) {
	errp(21, __func__, "gettimeofday failed");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(22, __func__, "printf error printing the welcome string");
	not_reached();
    }
	...
	errp(23, __func__, "cannot strdup version: %s", MKIOCCCENTRY_VERSION);
	not_reached();
    }
	...
	if (overwrite_answers == false) {
	    ...
	    if (ret <= 0) {
		errp(24, __func__, "printf error telling the user how to use the answers file");
		not_reached();
	    }
	    err(25, __func__, "won't overwrite answers file");
	    not_reached();
	}
    }
	...
	if (is_read(answers) == false) {
	    errp(26, __func__, "cannot read answers file");
	    not_reached();
	}
	errno = 0;
	answerp = fopen(answers, "r");
	if (answerp == NULL) {
	    errp(27, __func__, "cannot open answers file");
	    not_reached();
	}
	...
	if (answerp == NULL) {
	    errp(28, __func__, "cannot create answers file: %s", answers);
	    not_reached();
	}
...
sanity_chk(struct info *infop, char const *work_dir, char const *tar, char const *cp, char const *ls, char const *txzchk)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(34, __func__, "tar does not exist: %s", tar);
	not_reached();
    }
    if (!is_file(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(35, __func__, "tar is not a file: %s", tar);
	not_reached();
    }
    if (!is_exec(tar)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(36, __func__, "tar is not an executable program: %s", tar);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(37, __func__, "cp does not exist: %s", cp);
	not_reached();
    }
    if (!is_file(cp)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(38, __func__, "cp is not a file: %s", cp);
	not_reached();
    }
    if (!is_exec(cp)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(39, __func__, "cp is not an executable program: %s", cp);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(40, __func__, "ls does not exist: %s", ls);
	not_reached();
    }
    if (!is_file(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(41, __func__, "ls is not a file: %s", ls);
	not_reached();
    }
    if (!is_exec(ls)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(42, __func__, "ls is not an executable program: %s", ls);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(43, __func__, "work_dir does not exist: %s", work_dir);
	not_reached();
    }
    if (!is_dir(work_dir)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(44, __func__, "work_dir is not a directory: %s", work_dir);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(45, __func__, "work_dir is not a writable directory: %s", work_dir);
	not_reached();
    }
...
prompt(char const *str, size_t *lenp)
{
    ...
    if (ret == EOF) {
	if (ferror(stdout)) {
	    errp(47, __func__, "error printing prompt string");
	    not_reached();
	} else if (feof(stdout)) {
	    err(48, __func__, "EOF while printing prompt string");
	    not_reached();
	} else {
	    errp(49, __func__, "unexpected fputs error printing prompt string");
	    not_reached();
	}
    }
    clearerr(stdout);		/* pre-clear ferror() status */
    ...
    if (ret == EOF) {
	if (ferror(stdout)) {
	    errp(50, __func__, "error printing :<space>");
	    not_reached();
	} else if (feof(stdout)) {
	    err(51, __func__, "EOF while writing :<space>");
	    not_reached();
	} else {
	    errp(52, __func__, "unexpected fputs error printing :<space>");
	    not_reached();
	}
    }
    clearerr(stdout);		/* pre-clear ferror() status */
    ...
    if (ret == EOF) {
	if (ferror(stdout)) {
	    errp(53, __func__, "error flushing prompt to stdout");
	    not_reached();
	} else if (feof(stdout)) {
	    err(54, __func__, "EOF while flushing prompt to stdout");
	    not_reached();
	} else {
	    errp(55, __func__, "unexpected fflush error while flushing prompt to stdout");
	    not_reached();
	}
    }
    ...
    if (buf == NULL) {
	err(56, __func__, "EOF while reading prompt input");
	not_reached();
    }
...
get_contest_id(struct info *infop, bool *testp, bool *i_flag_used)
{
	...
	if (*i_flag_used == true && seen_answers_header == false) {
	    ...
	    err(58, __func__, "didn't find the correct answers file header");
	    not_reached();
	}
     ...
     */
    if (ret != 8) {
	errp(59, __func__, "retry prompt is disabled");
	not_reached();
    }
...
get_entry_num(struct info *infop)
{
	...
	if (need_hints == true) {
	    ...
	    if (ret <= 0) {
		errp(61, __func__, "printf error printing number of entries allowed");
		not_reached();
	    }
	    para("",
	...
	err(62, __func__, "retry prompt is disabled");
	not_reached();
    }
...
mk_entry_dir(char const *work_dir, char const *ioccc_id, int entry_num, char **tarball_path, time_t tstamp)
{
    ...
    if (entry_num < 0 || entry_num > MAX_ENTRY_NUM) {
	err(64, __func__, "entry number: %d must >= 0 and <= %d", entry_num, MAX_ENTRY_NUM);
	not_reached();
    }
    ...
    if (entry_dir == NULL) {
	errp(65, __func__, "malloc #0 of %lu bytes failed", (unsigned long)(entry_dir_len + 1));
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(entry_dir, entry_dir_len + 1, "%s/%s-%d", work_dir, ioccc_id, entry_num);
    if (ret <= 0) {
	errp(66, __func__, "snprintf to form entry directory failed");
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(67, __func__, "entry directory exists: %s", entry_dir);
	not_reached();
    }
    dbg(DBG_HIGH, "entry directory path: %s", entry_dir);
    ...
    if (ret < 0) {
	errp(68, __func__, "cannot mkdir %s with mode 0755", entry_dir);
	not_reached();
    }
    ...
    *tarball_path = (char *)malloc(tarball_len + 1);
    if (*tarball_path == NULL) {
	errp(69, __func__, "malloc #1 of %lu bytes failed", (unsigned long)(tarball_len + 1));
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(*tarball_path, tarball_len + 1, "entry.%s.%d.%ld.txz", ioccc_id, entry_num, (long)tstamp);
    if (ret <= 0) {
	errp(70, __func__, "snprintf to form compressed tarball path failed");
	not_reached();
    }
...
warn_rule2a_size(struct info *infop, char const *prog_c, int mode)
{
	...
	err(76, __func__, "invalid mode used: %d", mode);
	not_reached();
    }
    return;
}
...
warn_rule2b_size(struct info *infop, char const *prog_c)
{
	...
	if (ret <= 0) {
	    errp(88, __func__, "printf error printing prog.c size > Rule 2b maximum");
	    not_reached();
	}
...
check_prog_c(struct info *infop, char const *entry_dir, char const *cp, char const *prog_c)
{
    ...
    if (entry_dir_len <= 0) {
	err(91, __func__, "entry_dir arg is an empty string");
	not_reached();
    }
    prog_c_len = strlen(prog_c);
    if (prog_c_len <= 0) {
	err(92, __func__, "prog_c arg is an empty string");
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(93, __func__, "prog.c does not exist: %s", prog_c);
	not_reached();
    }
    if (!is_file(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(94, __func__, "prog.c is not a file: %s", prog_c);
	not_reached();
    }
    if (!is_read(prog_c)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(95, __func__, "prog.c is not a readable file: %s", prog_c);
	not_reached();
    }
	...
	err(96, __func__, "file_size error: %ld on prog_c: %s", (long)infop->rule_2a_size, prog_c);
	not_reached();
    ...
    if (prog_stream == NULL) {
	errp(97, __func__, "failed to fopen: %s", prog_c);
	not_reached();
    }
    ...
    if (ret != 0) {
	errp(98, __func__, "failed to fclose: %s", prog_c);
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(99, __func__, "failed to cmdprintf: cp prog_c entry_dir/prog.c");
	not_reached();
    }
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(100, __func__, "fflush(stdout) #2: error code: %d", ret);
	not_reached();
    }
    ...
    ret = fflush(stderr);
    if (ret < 0) {
	errp(101, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    if (exit_code < 0) {
	errp(102, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(103, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(104, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
    ...
    if (infop->prog_c == NULL) {
	errp(105, __func__, "malloc #2 of %lu bytes failed", (unsigned long)(LITLEN("prog.c") + 1));
	not_reached();
    }
...
inspect_Makefile(char const *Makefile, struct info *infop)
{
    ...
    if (stream == NULL) {
	errp(107, __func__, "cannot open Makefile: %s", Makefile);
	not_reached();
    }
    ...
    ret = fclose(stream);
    if (ret < 0) {
	errp(108, __func__, "fclose error");
	not_reached();
    }
...
warn_Makefile(char const *Makefile, struct info *infop)
{
	...
	if (yorn == false) {
	    err(110, __func__, "Use a different Makefile or modify this file: %s", Makefile);
	    not_reached();
	}
    }
}
...
check_Makefile(struct info *infop, char const *entry_dir, char const *cp, char const *Makefile)
{
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(112, __func__, "Makefile does not exist: %s", Makefile);
	not_reached();
    }
    if (!is_file(Makefile)) {
	fpara(stderr,
	       ...
	       "",
	       NULL);
	err(113, __func__, "Makefile is not a file: %s", Makefile);
	not_reached();
    }
    if (!is_read(Makefile)) {
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(114, __func__, "Makefile is not readable file: %s", Makefile);
	not_reached();
    }
    filesize = file_size(Makefile);
    if (filesize < 0) {
	err(115, __func__, "file_size error: %ld on Makefile  %s", (long)filesize, Makefile);
	not_reached();
    } else if (filesize == 0) {
	err(116, __func__, "Makefile cannot be empty: %s", Makefile);
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(117, __func__, "failed to cmdprintf: cp Makefile entry_dir/Makefile");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(118, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    clearerr(stderr);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ret = fflush(stderr);
    if (ret < 0) {
	errp(119, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    exit_code = system(cmd);
    if (exit_code < 0) {
	errp(120, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(121, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(122, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
    ...
    if (infop->Makefile == NULL) {
	errp(123, __func__, "malloc #1 of %lu bytes failed", (unsigned long)(LITLEN("Makefile") + 1));
	not_reached();
    }
...
check_remarks_md(struct info *infop, char const *entry_dir, char const *cp, char const *remarks_md)
{
	...
	err(124, __func__, "called with NULL arg(s)");
	not_reached();
    }
	...
	fpara(stderr, "",
	      ...
	      "",
	      NULL);
	err(126, __func__, "remarks_md is not a file: %s", remarks_md);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(128, __func__, "remarks_md is not readable file: %s", remarks_md);
	not_reached();
    }
    filesize = file_size(remarks_md);
    if (filesize < 0) {
	err(129, __func__, "file_size error: %ld on remarks_md %s", (long)filesize, remarks_md);
	not_reached();
    } else if (filesize == 0) {
    ...
    if (cmd == NULL) {
	err(131, __func__, "failed to cmdprintf: cp remarks.md entry_dir/remarks.md");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stderr);
    if (ret < 0) {
	errp(133, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    exit_code = system(cmd);
    if (exit_code < 0) {
	errp(134, __func__, "error calling system(%s)", cmd);
	not_reached();
    } else if (exit_code == 127) {
	errp(135, __func__, "execution of the shell failed for system(%s)", cmd);
	not_reached();
    } else if (exit_code != 0) {
	err(136, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
    ...
    if (infop->remarks_md == NULL) {
	errp(137, __func__, "malloc #1 of %lu bytes failed", (unsigned long)(LITLEN("remarks.md") + 1));
	not_reached();
    }
...
check_extra_data_files(struct info *infop, char const *entry_dir, char const *cp, int count, char **args)
{
    ...
    if (count < 0) {
	err(139, __func__, "count :%d < 0", count);
	not_reached();
    }
    ...
    if (infop->extra_file == NULL) {
	errp(140, __func__, "calloc #0 of %d char * pointers failed", count + 1);
	not_reached();
    }
	...
	if (!exists(args[i])) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(141, __func__, "extra[%i] does not exist: %s", i, args[i]);
	    not_reached();
	}
	if (!is_file(args[i])) {
	    fpara(stderr,
		   ...
		   "",
		   NULL);
	    err(142, __func__, "extra[%i] is not a file: %s", i, args[i]);
	    not_reached();
	}
	if (!is_read(args[i])) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(143, __func__, "extra[%i] is not readable file: %s", i, args[i]);
	    not_reached();
	}
	...
	if (base_len == 0) {
	    err(144, __func__, "basename of extra data file: %s has a length of 0", args[i]);
	    not_reached();
	    ...
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(145, __func__, "basename of extra data file: %s is %lu characters an is > the limit: %lu",
	...
	if (base[0] == '.' || base[0] == '-' || base[0] == '+') {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(146, __func__, "basename of extra data file: %s start with in invalid character: %s", args[i], base);
	    not_reached();
	}
	...
	for (j=0; j < base_len; ++j) {
		...
		fpara(stderr,
		      ...
		      "",
		      NULL);
		err(147, __func__, "basename of %s character %lu is NOT a POSIX Fully portable character nor +",
	...
	if (dest == NULL) {
	    errp(148, __func__, "malloc #0 of %lu bytes failed", (unsigned long)(dest_len + 1));
	    not_reached();
	}
	...
	if (exists(dest)) {
	    fpara(stderr,
		  ...
		  "",
		  NULL);
	    err(150, __func__, "for extra file: %s destination already exists: %s", args[i], dest);
	    not_reached();
	}
	...
	if (cmd == NULL) {
	    err(151, __func__, "failed to cmdprintf: cp extra_file[%d]: %s dest: %s", i, args[i], dest);
	    not_reached();
	}
	dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
	...
	ret = fflush(stderr);
	if (ret < 0) {
	    errp(153, __func__, "fflush(stderr) #1: error code: %d", ret);
	    not_reached();
	}
	...
	exit_code = system(cmd);
	if (exit_code < 0) {
	    errp(154, __func__, "error calling system(%s)", cmd);
	    not_reached();
	} else if (exit_code == 127) {
	    errp(155, __func__, "execution of the shell failed for system(%s)", cmd);
	    not_reached();
	} else if (exit_code != 0) {
	    err(156, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	    not_reached();
	}
...
get_author_info(struct info *infop, char *ioccc_id, struct author **author_set_p)
{
    ...
    if (author_set == NULL) {
	errp(166, __func__, "malloc a struct author array of length: %d failed", author_count);
	not_reached();
    }
	...
	if (author_set[i].twitter == NULL) {
	    errp(171, __func__, "retry prompt is disabled");
	    not_reached();
	}
	...
	if (author_set[i].github == NULL) {
	    errp(172, __func__, "retry prompt is disabled");
	    not_reached();
	}
	...
	if (author_set[i].affiliation == NULL) {
	    errp(173, __func__, "retry prompt is disabled");
	    not_reached();
	}
	...
	if (printf("\nPlease verify the information about author #%d\n\n", i) <= 0 ||
	    ...
	    errp(174, __func__, "error while printing author #%d information\n", i);
	    not_reached();
	}
	if (need_confirm == true) {
...
verify_entry_dir(char const *entry_dir, char const *ls)
{
    ...
    if (cmd == NULL) {
	err(176, __func__, "failed to cmdprintf: cd entry_dir && ls -lak .");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(177, __func__, "fflush(stdout) error code: %d", ret);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    clearerr(stderr);		/* pre-clear ferror() status */
	...
	err(181, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
    ...
    ret = fflush(stdout);
    if (ret < 0) {
	errp(182, __func__, "fflush(stdout) #0: error code: %d", ret);
	not_reached();
    }
    clearerr(stderr);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ret = fflush(stderr);
    if (ret < 0) {
	errp(183, __func__, "fflush(stderr) #1: error code: %d", ret);
	not_reached();
    }
    ...
    if (ls_stream == NULL) {
	errp(184, __func__, "popen for reading failed for: %s", cmd);
	not_reached();
    }
    ...
    if (readline_len < 0) {
	err(185, __func__, "EOF while reading 1st line from ls: %s", ls);
	not_reached();
    } else {
    ...
    if (ret != 1) {
	err(186, __func__, "failed to parse block line from ls: %s", linep);
	not_reached();
    }
    if (kdirsize <= 0) {
	err(187, __func__, "ls k block value: %d <= 0", kdirsize);
	not_reached();
    }
	...
	if (ret <= 0) {
	    errp(188, __func__, "fprintf error while printing entry directory kibibyte sizes");
	    not_reached();
	}
	err(189, __func__, "The entry directory is too large: %s", entry_dir);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      NULL);
	err(190, __func__, "%s failed with exit code: %d", cmd, WEXITSTATUS(exit_code));
	not_reached();
    }
...
write_info(struct info *infop, char const *entry_dir, bool test_mode)
{
    ...
    if (infop->epoch == NULL) {
	errp(192, __func__, "strdup of %s failed", TIMESTAMP_EPOCH);
	not_reached();
    }
    ...
    if (timeptr == NULL) {
	errp(194, __func__, "localtime #1 returned NULL");
	not_reached();
    }
    if (timeptr->tm_zone == NULL) {
	err(195, __func__, "timeptr->tm_zone #1 is NULL");
	not_reached();
    }
    ...
    p = asctime(timeptr);
    if (p == NULL) {
	errp(196, __func__, "asctime #1 returned NULL");
	not_reached();
    }
    ...
    if (infop->gmtime == NULL) {
	errp(197, __func__, "calloc of %lu bytes failed", (unsigned long)gmtime_len + 1);
	not_reached();
    }
    ...
    if (info_path == NULL) {
	errp(198, __func__, "malloc of %lu bytes failed", (unsigned long)info_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(info_path, info_path_len, "%s/.info.json", entry_dir);
    if (ret <= 0) {
	errp(199, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
    ...
    if (info_stream == NULL) {
	errp(200, __func__, "failed to open for writing: %s", info_path);
	not_reached();
    }
    ...
    if (ret == false) {
	errp(201, __func__, "fprintf error writing leading part of info to %s", info_path);
	not_reached();
    }
    ...
    ret = json_fprintf_value_string(info_stream, "\t\t{", "info_JSON", " : ", ".info.json", "},\n") &&
	  ...
	  json_fprintf_value_string(info_stream, "\t\t{", "remarks", " : ", infop->remarks_md,
				    ((infop->extra_count > 0) ?  "},\n" : "}\n"));
    if (ret == false) {
	errp(202, __func__, "fprintf error writing mandatory filename to %s", info_path);
	not_reached();
    }
    ...
    for (i=0, q=infop->extra_file; i < infop->extra_count && *q != NULL; ++i, ++q) {
	    ...
	    errp(203, __func__, "fprintf error writing extra filename[%d] to %s", i, info_path);
	    not_reached();
	}
    }
	...
	fprintf(info_stream, "}\n") > 0;
    if (ret == false) {
	errp(204, __func__, "fprintf error writing trailing part of info to %s", info_path);
	not_reached();
    }
...
write_author(struct info *infop, int author_count, struct author *authorp, char const *entry_dir)
{
    ...
    if (author_path == NULL) {
	errp(207, __func__, "malloc of %lu bytes failed", (unsigned long)author_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(author_path, author_path_len, "%s/.author.json", entry_dir);
    if (ret <= 0) {
	errp(208, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
	...
	errp(209, __func__, "failed to open for writing: %s", author_path);
	not_reached();
    }
    ...
    if (ret == false) {
	errp(210, __func__, "fprintf error writing leading part of authorship to %s", author_path);
	not_reached();
    }
	...
	if (ret < 0) {
	    errp(211, __func__, "fprintf error writing author info to %s", author_path);
	    not_reached();
	}
    }
    ...
    if (ret == false) {
	errp(212, __func__, "fprintf error writing trailing part of authorship to %s", author_path);
	not_reached();
    }
    ...
    if (ret < 0) {
	errp(213, __func__, "fclose error");
	not_reached();
    }
    return;
}
...
form_tarball(char const *work_dir, char const *entry_dir, char const *tarball_path, char const *tar, char const *ls, char const *txzchk)
{
    ...
    ret = chdir(work_dir);
    if (ret < 0) {
	errp(215, __func__, "cannot cd %s", work_dir);
	not_reached();
    }
    ...
    if (cmd == NULL) {
	err(216, __func__, "failed to cmdprintf: tar --format=v7 -cJf tarball_path entry_dir");
	not_reached();
    }
    dbg(DBG_HIGH, "about to perform: system(%s)", cmd);
    ...
    if (ret != 0) {
	errp(222, __func__, "stat of the compressed tarball failed: %s", basename_tarball_path);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(223, __func__, "The compressed tarball: %s size: %lu > %ld",
...
remind_user(char const *work_dir, char const *entry_dir, char const *tar, char const *tarball_path, bool test_mode)
{
	...
	err(237, __func__, "failed to cmdprintf: entry_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(238, __func__, "printf #0 error");
	not_reached();
    }
    free(entry_dir_esc);
	...
	err(239, __func__, "failed to cmdprintf: work_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(240, __func__, "printf #2 error");
	not_reached();
    }
    free(work_dir_esc);
	...
	if (ret <= 0) {
	    errp(241, __func__, "printf #2 error");
	    not_reached();
	}
    }
	...
	ret = printf("    %s\n\n", IOCCC_SUBMIT_URL);
	if (ret < 0) {
	    errp(242, __func__, "printf #4 error");
	    not_reached();
	}
    }
    return;
