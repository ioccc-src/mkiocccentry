...
verify_entry_dir(char const *entry_dir, char const *ls)
{
	...
	fpara(stderr,
	      ...
	      NULL);
	err(133, __func__, "user rejected listing of entry_dir: %s", entry_dir);
	not_reached();
    }
...
write_info(struct info *infop, char const *entry_dir, char const *chkentry, char const *fnamchk)
{
    ...
    ret = setenv("TZ", "UTC", 1);
    if (ret < 0) {
	errp(135, __func__, "cannot set TZ=UTC");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    timeptr = gmtime(&(infop->tstamp));
    if (timeptr == NULL) {
	errp(136, __func__, "localtime #1 returned NULL");
	not_reached();
    }
	...
	errp(137, __func__, "calloc of %ju bytes failed", (uintmax_t)utctime_len + 1);
	not_reached();
    }
    ...
    if (strftime_ret == 0) {
	errp(138, __func__, "strftime returned 0");
	not_reached();
    }
    ...
    if (info_path == NULL) {
	errp(139, __func__, "malloc of %ju bytes failed", (uintmax_t)info_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ...
    if (info_stream == NULL) {
	errp(141, __func__, "failed to open for writing: %s", info_path);
	not_reached();
    }
    ...
    if (!ret) {
	errp(142, __func__, "fprintf error writing leading part of info to %s", info_path);
	not_reached();
    }
    ...
    ret = json_fprintf_value_string(info_stream, "\t\t{", "info_JSON", " : ", INFO_JSON_FILENAME, "},\n") &&
	  ...
	  json_fprintf_value_string(info_stream, "\t\t{", "remarks", " : ", infop->remarks_md,
				    ((infop->extra_count > 0) ?  "},\n" : "}\n"));
    if (!ret) {
	errp(143, __func__, "fprintf error writing mandatory filename to %s", info_path);
	not_reached();
    }
    ...
    for (i=0, q=infop->extra_file; i < infop->extra_count && *q != NULL; ++i, ++q) {
	    ...
	    errp(144, __func__, "fprintf error writing extra filename[%d] to %s", i, info_path);
	    not_reached();
	}
    }
	...
	fprintf(info_stream, "}\n") > 0;
    if (!ret) {
	errp(145, __func__, "fprintf error writing trailing part of info to %s", info_path);
	not_reached();
    }
    ...
    if (exit_code != 0) {
	err(147, __func__, "%s -q -F %s -- %s . failed with exit code: %d",
...
form_auth(struct auth *authp, struct info *infop, int author_count, struct author *authorp)
{
	...
	err(149, __func__, "infop->ioccc_id is NULL");
	not_reached();
    }
    if (infop->tarball == NULL) {
	err(150, __func__, "infop->tarball is NULL");
	not_reached();
    }
    if (infop->utctime == NULL) {
	err(151, __func__, "infop->utctime is NULL");
	not_reached();
    }
	...
	errp(152, __func__, "strdup() ioccc_id path %s failed", infop->ioccc_id);
	not_reached();
    }
	...
	errp(153, __func__, "strdup() tarball path %s failed", infop->tarball);
	not_reached();
    }
	...
	errp(154, __func__, "strdup() utctime path %s failed", infop->utctime);
	not_reached();
    }
    return;
}
...
write_author(struct auth *authp, char const *entry_dir, char const *chkentry, char const *fnamchk)
{
	...
	err(156, __func__, "author_count %d <= 0", authp->author_count);
	not_reached();
    } else if (authp->author_count > MAX_AUTHORS) {
	err(157, __func__, "author count %d > max authors %d", authp->author_count, MAX_AUTHORS);
	not_reached();
    }
    ...
    if (author_path == NULL) {
	errp(158, __func__, "malloc of %ju bytes failed", (uintmax_t)author_path_len + 1);
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = snprintf(author_path, author_path_len, "%s/%s", entry_dir, AUTHOR_JSON_FILENAME);
    if (ret <= 0) {
	errp(159, __func__, "snprintf #0 error: %d", ret);
	not_reached();
    }
	...
	errp(160, __func__, "failed to open for writing: %s", author_path);
	not_reached();
    }
    ...
    if (!ret) {
	errp(161, __func__, "fprintf error writing leading part of authorship to %s", author_path);
	not_reached();
    }
	...
	if (ret == false) {
	    errp(162, __func__, "fprintf error writing author %d info to %s", i, author_path);
	    not_reached();
	}
    }
    ...
    if (!ret) {
	errp(163, __func__, "fprintf error writing trailing part of authorship to %s", author_path);
	not_reached();
    }
	     ...
	     char const *ls, char const *txzchk, char const *fnamchk)
{
    ...
    ret = chdir(work_dir);
    if (ret < 0) {
	errp(168, __func__, "cannot cd %s", work_dir);
	not_reached();
    }
    ...
    exit_code = shell_cmd(__func__, true, "% --format=v7 -cJf % -- %",
				    tar, basename_tarball_path, basename_entry_dir);
    if (exit_code != 0) {
	err(169, __func__, "%s --format=v7 -cJf %s -- %s failed with exit code: %d",
    ...
    if (ret != 0) {
	errp(170, __func__, "stat of the compressed tarball failed: %s", basename_tarball_path);
	not_reached();
    }
	...
	fpara(stderr,
	      ...
	      "",
	      NULL);
	err(171, __func__, "The compressed tarball: %s size: %ju > %jd",
    ...
    ret = fchdir(cwd);
    if (ret < 0) {
	errp(172, __func__, "cannot fchdir to the previous current directory");
	not_reached();
    }
    errno = 0;			/* pre-clear errno for errp() */
    ret = close(cwd);
    if (ret < 0) {
	errp(173, __func__, "close of previous current directory failed");
	not_reached();
    }
    ...
    exit_code = shell_cmd(__func__, true, "% -q -F % -- %/../%",
					  txzchk, fnamchk, entry_dir, basename_tarball_path);
    if (exit_code != 0) {
	err(174, __func__, "%s -q -F %s -- %s/../%s failed with exit code: %d",
...
remind_user(char const *work_dir, char const *entry_dir, char const *tar, char const *tarball_path, bool test_mode)
{
	...
	err(176, __func__, "failed to cmdprintf: entry_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(177, __func__, "printf #0 error");
	not_reached();
    }
    free(entry_dir_esc);
	...
	err(178, __func__, "failed to cmdprintf: work_dir");
	not_reached();
    }
    ...
    if (ret <= 0) {
	errp(179, __func__, "printf #2 error");
	not_reached();
    }
    free(work_dir_esc);
	...
	ret = printf("    %s\n\n", IOCCC_SUBMIT_URL);
	if (ret < 0) {
	    errp(181, __func__, "printf #4 error");
	    not_reached();
	}
    }
    return;
