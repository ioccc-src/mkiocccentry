 ...
 */
static void
chkauth_sanity_chks(char const *file, char const *fnamchk)
{
    /*
     * firewall
     */
...
check_author_json(char const *file, char const *fnamchk)
{
    ...
    *p = '\0';
    ...
    /* if issues != 0 there will be a non-zero return status of chkauth */
    return issues;
}
...
check_found_author_json_fields(char const *json_filename, bool test)
{
	...
	for (value = field->values; value != NULL; value = value->next) {
	    ...
	    } else if (!strcmp(field->name, "chkauth_version")) {
		if (!test && strcmp(val, CHKAUTH_VERSION)) {
		    warn(__func__, "chkauth_version != CHKAUTH_VERSION \"%s\" in file %s: \"%s\"",
				   CHKAUTH_VERSION, json_filename, val);
		    ++issues;
		}
...
usage(int exitcode, char const *str, char const *prog)
{
    ...
    fprintf_usage(exitcode, stderr, usage_msg, prog, DBG_DEFAULT, FNAMCHK_PATH_0, CHKAUTH_VERSION);
    exit(exitcode); /*ooo*/
    not_reached();
}
...
main(int argc, char **argv)
{
	...
	case 'V':		/* -V - print version and exit */
	    ...
	    ret = printf("%s\n", CHKAUTH_VERSION);
	    if (ret <= 0) {
		warnp(__func__, "printf error printing version string: %s", CHKAUTH_VERSION);
	    }
	    exit(0); /*ooo*/
	    not_reached();
	    break;
	...
	ret = printf("Welcome to chkauth version: %s\n", CHKAUTH_VERSION);
	if (ret <= 0) {
    ...
    chkauth_sanity_chks(file, fnamchk);
    if (!quiet) {
