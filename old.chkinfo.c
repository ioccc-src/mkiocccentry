 ...
 */
static void
chkinfo_sanity_chks(char const *file, char const *fnamchk)
{
    /*
     * firewall
     */
...
check_info_json(char const *file, char const *fnamchk)
{
     ...
     * (though for this tool, chkauth, it's not complete) and we think it's
    ...
    *p = '\0';
	...
	if (info_field == NULL && common_field == NULL) {
	    chk_warn(JSON_CODE_RESERVED(1), __func__, file, NULL, line_num, "invalid field '%s'", p);
	    ++issues;
	}
	...
	if (!strcmp(p, "manifest")) {
	    if (!info_field) {
		err(20, __func__, "manifest field not found in info_json_fields table");
		not_reached();
	    }
		...
		err(21, __func__, "couldn't add manifest to found_info_json_fields");
		not_reached();
	    }
		...
		if (array_val_esc == NULL) {
		    err(28, __func__, "json_decode(): invalidly formed field '%s' value '%s' or malloc failure in file %s",
		...
		*end-- = '\0';
	    ...
	    if (val_esc == NULL) {
		err(30, __func__, "json_decode_str(): invalidly formed field '%s' value '%s' or "
    ...
    /* if issues != 0 there will be a non-zero return status of chkinfo */
    return issues;
}
 ...
 * supposed to add it to the table info_json_fields but it does not. This
 * function is called in order to check if the field is a .info.json field and
 * if so add it to the found_info_json_fields table. This function actually uses
 * add_found_info_json_field() but perhaps that function should do all of the
...
add_found_info_json_field(char const *json_filename, char const *name, char const *val, int line_num)
{
	...
	err(32, __func__, "called add_found_info_json_field() on field '%s' not specific "
...
add_manifest_file(char const *filename)
{
	...
	err(33, __func__, "calloc() error allocating struct manifest_file * for filename %s", filename);
	not_reached();
    }
	...
	err(34, __func__, "strdup() error on filename %s", filename);
	not_reached();
    }
...
free_manifest_file(struct manifest_file *file)
{
     ...
     */
    if (file == NULL) {
	err(35, __func__, "passed NULL file");
	not_reached();
    }
...
check_found_info_json_fields(char const *json_filename, bool test)
{
	...
	err(36, __func__, "passed NULL json_filename");
	not_reached();
    }
	...
	for (value = field->values; value != NULL; value = value->next) {
		 ...
		 */
		err(37, __func__, "empty value found for field '%s' in file %s", field->name, json_filename);
		not_reached();
	    }
	    ...
	    } else if (!strcmp(field->name, "chkinfo_version")) {
		if (!test && strcmp(val, CHKINFO_VERSION)) {
		    warn(__func__, "chkinfo_version != CHKINFO_VERSION \"%s\" in file %s: \"%s\"",
				   CHKINFO_VERSION, json_filename, val);
		    ++issues;
		}
		...
		if (manifest_file == NULL) {
		    err(38, __func__, "couldn't add info_JSON file '%s'", val);
		    not_reached();
		}
		...
		manifest_file = add_manifest_file(val);
		if (manifest_file == NULL) {
		    err(39, __func__, "couldn't add author_JSON file '%s'", val);
		    not_reached();
		}
		...
		manifest_file = add_manifest_file(val);
		if (manifest_file == NULL) {
		    err(40, __func__, "couldn't add c_src file '%s'", val);
		    not_reached();
		}
		...
		manifest_file = add_manifest_file(val);
		if (manifest_file == NULL) {
		    err(41, __func__, "couldn't add Makefile file '%s'", val);
		    not_reached();
		}
		...
		manifest_file = add_manifest_file(val);
		if (manifest_file == NULL) {
		    err(42, __func__, "couldn't add remarks file '%s'", val);
		    not_reached();
		}
		...
		manifest_file = add_manifest_file(val);
		if (manifest_file == NULL) {
		    err(43, __func__, "couldn't add extra_file file '%s' in file %s", val, json_filename);
		    not_reached();
		}
...
usage(int exitcode, char const *str, char const *prog)
{
    ...
    fprintf_usage(exitcode, stderr, usage_msg, prog, DBG_DEFAULT, FNAMCHK_PATH_0, CHKINFO_VERSION);
    exit(exitcode); /*ooo*/
    not_reached();
}
...
main(int argc, char **argv)
{
	...
	case 'V':		/* -V - print version and exit */
	    ...
	    ret = printf("%s\n", CHKINFO_VERSION);
	    if (ret <= 0) {
		warnp(__func__, "printf error printing version string: %s", CHKINFO_VERSION);
	    }
	    exit(0); /*ooo*/
	    not_reached();
	    break;
	...
	ret = printf("Welcome to chkinfo version: %s\n", CHKINFO_VERSION);
	if (ret <= 0) {
	    errp(44, __func__, "printf error printing the welcome string");
	    not_reached();
	}
    ...
    chkinfo_sanity_chks(file, fnamchk);
    if (!quiet) {
