...
main(int argc, char **argv)
{
	...
	case 'h':	/* -h - write help, to stderr and exit 0 */
	    fprintf_usage(24, stderr, usage, program, FILE_UTIL_TEST_VERSION, JPARSE_UTILS_VERSION, JPARSE_UTF8_VERSION,
	...
	case ':':
	    ...
	    fprintf_usage(25, stderr, usage, program, FILE_UTIL_TEST_VERSION, JPARSE_UTILS_VERSION, JPARSE_UTF8_VERSION,
                    JPARSE_LIBRARY_VERSION); /*ooo*/
	    not_reached();
	    break;
	case '?':
	    (void) fprintf(stderr, "%s: illegal option -- %c\n\n", program, optopt);
	    fprintf_usage(26, stderr, usage, program, FILE_UTIL_TEST_VERSION, JPARSE_UTILS_VERSION, JPARSE_UTF8_VERSION,
                    JPARSE_LIBRARY_VERSION); /*ooo*/
	    not_reached();
	    break;
	default:
	    fprintf_usage(DO_NOT_EXIT, stderr, "invalid -flag");
	    fprintf_usage(27, stderr, usage, program, FILE_UTIL_TEST_VERSION, JPARSE_UTILS_VERSION,
    ...
    if (relpath != NULL) {
        ...
        if (stat("test_file_util.copy.c", &out_st) != 0) {
            errp(128, __func__, "couldn't stat file test_file_util.copy.c");
            not_reached();
        }
        ...
        if (unlink("test_file_util.copy.c") != 0) {
            errp(130, __func__, "unable to delete file test_file_util.copy.c");
            not_reached();
        } else {
        ...
        if (!is_empty("test_file_util.copy.c")) {
            errp(131, __func__, "just deleted file test_file_util.copy.c is not empty");
            not_reached();
        }
    ...
    if (relpath != NULL) {
        ...
        errno = 0;  /* pre-clear errno for errp() */
        if (stat("test_file_util.copy.c", &out_st) != 0) {
            errp(132, __func__, "failed to stat test_file_util.copy.c");
            not_reached();
        }
        if ((out_st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO)) != (S_IRUSR|S_IWUSR)) {
            err(133, __func__, "copyfile() failed to set S_IRUSR|S_IWUSR on test_file_util.copy.c");
            not_reached();
        } else {
        ...
        errno = 0;      /* pre-clear errno for errp() */
        if (unlink("test_file_util.copy.c") != 0) {
            errp(134, __func__, "unable to delete file test_file_util.copy.c");
            not_reached();
        } else {
         ...
         */
        if (!is_empty("test_file_util.copy.c")) {
            errp(135, __func__, "just deleted file test_file_util.copy.c is not empty");
            not_reached();
        }
    ...
    if (stat("test_ioccc/test_file_util.o", &in_st) != 0) {
        errp(136, __func__, "failed to stat test_ioccc/test_file_util.o");
        not_reached();
    }
    ...
    if (stat("test_file_util.copy.o", &out_st) != 0) {
        errp(137, __func__, "failed to stat test_file_util.copy.o");
        not_reached();
    }

    if (!is_mode("test_file_util.copy.o", in_st.st_mode)) {
        err(138, __func__, "test_file_util.o st_mode != test_file_util.copy.o st_mode: %04o != %04o",
                in_st.st_mode & ~S_IFMT, out_st.st_mode & ~S_IFMT);
        not_reached();
    } else {
    ...
    if (size1 <= 0 || size1 != size2 || size1 != in_st.st_size || size1 != out_st.st_size) {
        err(139, __func__, "size_if_file(\"file_util.o\") != size_if_file(\"test_file_util.copy.o\") or <= or != stat(2) st_size!");
        not_reached();
    } else {
    ...
    if (unlink("test_file_util.copy.o") != 0) {
        errp(140, __func__, "unable to delete file test_file_util.copy.o");
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "successfully deleted copied file test_file_util.copy.c");
    }
    ...
    if (!is_empty("test_file_util.copy.o")) {
        errp(141, __func__, "just deleted file test_file_util.copy.o is not empty");
        not_reached();
    }
    ...
    ent = read_fts("test_ioccc", -1, &cwd, &fts);
    if (ent == NULL) {
        err(158, __func__, "read_fts() returned a NULL pointer on \"test_ioccc\" for directories");
        not_reached();
    } else {
        do {
            char *p = fts_path(ent);
    ...
    ent = read_fts("test_ioccc", -1, &cwd, &fts);
    if (ent == NULL) {
        err(169, __func__, "read_fts() returned a NULL pointer on \"test_ioccc\"");
        not_reached();
    } else {
        do {
            char *p = fts_path(ent);
    ...
    if (name == NULL) {
         ...
         */
        err(178, __func__, "didn't find \"test_ioccc\" in paths array");
        not_reached();
    } else {
    ...
    paths_found = find_paths(paths, "test_ioccc", -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(201, __func__, "found unexpected socket under test_ioccc/");
        not_reached();
    }
    ...
    paths_found = find_paths(paths, "test_ioccc", -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(202, __func__, "found unexpected character device under test_ioccc/");
        not_reached();
    }
    ...
    paths_found = find_paths(paths, "test_ioccc", -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(203, __func__, "found unexpected block device under test_ioccc/");
        not_reached();
    }
    ...
    paths_found = find_paths(paths, "test_ioccc", -1, &cwd, false, &fts);
    if (paths_found != NULL) {
        err(204, __func__, "found unexpected FIFO under test_ioccc/");
        not_reached();
    }
    ...
    if (paths_found != NULL) {
         ...
         * make it an error
         */
        err(206, __func__, "found unexpected file type under test_ioccc/");
        not_reached();
    }
