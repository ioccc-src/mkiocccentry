 ...
 */
char *
default_handle(char const *name, int author_num, int entry_num, char const *ioccc_id)
{
    char *ret;		/* malloc string to return */

    /*
     * firewall
     */
    if (name == NULL || ioccc_id == NULL) {
	err(172, __func__, "passed NULL arg");
	not_reached();
    }
    if (author_num < 0 || author_num >= MAX_AUTHORS) {
	err(173, __func__, "author_num: %d < 0 or >= %d", author_num, MAX_AUTHORS);
	not_reached();
    }
    if (entry_num < 0 || entry_num >= MAX_ENTRY_NUM) {
	err(174, __func__, "author_num: %d < 0 or >= %d", entry_num, MAX_ENTRY_NUM);
	not_reached();
    }

    /*
     * XXX - code stub for now
     */
    errno = 0;		/* pre-clear errno for errp() */
    ret = strdup("XXX");
    if (ret == NULL) {
	errp(175, __func__, "strdup(\"XXX\") failed");
	not_reached();
    }
    return ret;
}
